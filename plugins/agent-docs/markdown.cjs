...
function renderMarkdown(html, { permalink, siteUrl, routes }) {
  const $ = load(html);
  // Restrict conversion to the article; missing markup should fail the build
  // instead of silently exporting navigation or an empty page.
  ...
  // Only published documentation links get Markdown destinations. Assets and
  // external links keep their targets, including query strings and anchors.
  ...
  converter.addRule("source-code", {
    ...
    replacement: (_content, node) => {
      ...
      // A fence longer than any backtick run in the example prevents nested
      // Markdown examples from prematurely closing their own code block.
  ...
  // Preserve Docusaurus IDs: Markdown renderers may derive different heading
  // slugs, especially for API fields, which would break existing deep links.
