...
const NotFound = () => {
  ...
  const requestedUrl = typeof window !== 'undefined' ? window.location.pathname : "";
  const matched = !!requestedUrl.match(regex)
  if(matched){
    // redirect user to the latest version of docs
    const availableVersions = context.globalData["docusaurus-plugin-content-docs"].default.versions;
