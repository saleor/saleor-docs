...
| **Snapshots and backups** | Cloud users create snapshots on demand | Saleor automatically creates daily snapshots |
...
Clicking **Promote** converts your sandbox environment to production. The promotion typically causes about five minutes of downtime while Saleor restores the database.
...
| Commerce configuration | Included: channels, warehouses, shipping configuration, tax configuration, menus, site settings, models, and metadata stored in Saleor. |
| Extensions | Included: app installations, webhooks, plugins, and their database configuration. Saleor disables them when restoring the snapshot to a different domain. Review them before enabling them again. |
...
The **Clear database** action removes all business-related records, including orders, products, categories, customer accounts, vouchers, and gift cards. You cannot undo this action.
...
### Replace the Database with a Sample
...
::::warning
Deletion is permanent. You cannot restore the environment after deleting it.
...
If your environment is automatically deleted because of inactivity, Saleor creates a final snapshot before deletion.
You can restore the data without contacting support by following [Create a Sandbox from a Snapshot](#create-a-sandbox-from-a-snapshot).
