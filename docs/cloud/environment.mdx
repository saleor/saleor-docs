...
:::info
All extensions (apps, webhooks, and plugins) are going to be disabled
when the snapshot is restored in another environment (with a different domain)
e.g, during environment duplication or replicating production data to staging.

You must log in to the Saleor Dashboard and re-enable them manually.  
:::
