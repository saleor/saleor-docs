...
If you are using [Playground](./developer-tools.mdx) to access the API, you can authorize your requests by providing the JWT token in the _HTTP HEADERS_ tab. To do that, paste the following JSON structure into the tab, replacing the token with your real token:
