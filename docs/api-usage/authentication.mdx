...
When integrating with Saleor, one of the first decisions is which authentication mechanism to use.

- **Public access (no token)**: Some GraphQL operations are public and require no authentication at all.
This is the appropriate choice when your integration needs no permissions, such as retrieving public product data for a simple storefront.

- **User-based authentication**: When a user is present (e.g., a storefront with customer login or dashboard staff user session), you typically use:

  - [OIDC](#user-authentication-using-openid-connect) – the recommended method for user login
  - [tokenCreate](#email-and-password) – based on JWT credentials

- **App-based authentication**: If you are building machine-to-machine integrations, you use App Tokens, issued to a [Saleor App](#app-authentication). 

  However, apps cannot install or manage other Apps. Those actions require a human user.

- **Dashboard extension authentication**: Dashboard extensions receive a special dashboard extension JWT passed automatically via the [App SDK](/developer/extending/apps/developing-apps/app-sdk/overview.mdx).


```mermaid
flowchart TD
    A{Do you need permissions?} -->|No| X[No token needed - public API]
    A -->|Yes| C{Is there a user present?}
    C -->|No| D{Do you need to manage or install Apps?}
    C -->|Yes| F{Are you building a dashboard extension?}
    D -->|No| E[Use an App with a App Token]
    D -->|Yes| Y[fa:fa-ban Forbidden! You need to have a user present]
    F -->|No| G[Prefer OIDC, \`tokenCreate\` as fallback]
    F -->|Yes| H[Use the App SDK, you will be passed a special access token]
```
...
However, if your account was first created by Saleor or you set a password using the "forgot password" feature, 
both SSO and email/password logins may be available.
:::
...
`tokenCreate` returns the same token shape for both customers and staff users. The decoded token includes an `is_staff` claim that reflects the role of the underlying [`User`](/developer/users/user-model.mdx), but **permissions themselves are not embedded in the token** — they are resolved server-side on each request from the user's permission groups and direct permissions.

Treat `is_staff` as informational only. Do not use it on the client to make authorization decisions; rely on the response of the actual operation and on the [`me`](#fetching-information-about-the-currently-authenticated-user) query (which exposes `userPermissions` and `permissionGroups`) when you need to drive UI by role.
...
To prevent abuse of the password reset functionality, Saleor uses password reset throttling. You can only request single password reset link per user per 15 minutes.
If you self-host Saleor, you can modify this limit by setting the `RESET_PASSWORD_LOCK_TIME` environment variable.

## Password Login Mode

Saleor allows administrators to control whether password-based authentication is available. This is useful for organizations transitioning staff to OIDC/SSO — you can gradually restrict password login for staff while optionally keeping it available for customers.

The `passwordLoginMode` setting on `Shop` supports three modes:

### Modes

| Mode | Customers | Staff | API-observable behavior |
| --- | --- | --- | --- |
| `ENABLED` (default) | Normal login | Normal login | `tokenCreate` issues tokens for both roles as usual |
| `CUSTOMERS_ONLY` | Normal login | Staff can still log in, but they get no permissions | `tokenCreate` succeeds for staff, but the issued token resolves no staff permissions — operations requiring `MANAGE_*` permissions will fail as if the caller were a customer |
| `DISABLED` | Blocked | Blocked | `tokenCreate` and `tokenRefresh` return `DISABLED_AUTHENTICATION_METHOD`; password-related mutations are blocked |

:::info
`ENABLED` is the default. Existing deployments are unaffected unless explicitly changed.
:::


### Configuring the Mode

Use the [`shopSettingsUpdate`](/api-reference/shop/mutations/shop-settings-update.mdx) mutation to change the mode:
...
:::caution
Password-authenticated admins **cannot** set the mode to `CUSTOMERS_ONLY` or `DISABLED`.
Only OIDC-authenticated admins can restrict password login.
Attempting otherwise returns a `PASSWORD_AUTH_RESTRICTION` error.
:::

### Affected Mutations

- [`tokenCreate`](/api-reference/authentication/mutations/token-create.mdx) — returns an error when `DISABLED`; in `CUSTOMERS_ONLY`, staff can still log in, but the token has no staff permissions
- [`tokenRefresh`](/api-reference/authentication/mutations/token-refresh.mdx) — returns an error when `DISABLED`; in `CUSTOMERS_ONLY`, staff can still log in, but the token has no staff permissions
- [`passwordChange`](/api-reference/users/mutations/password-change.mdx) — blocked when `DISABLED`
- [`setPassword`](/api-reference/users/mutations/set-password.mdx) — blocked when `DISABLED`
- [`requestPasswordReset`](/api-reference/users/mutations/request-password-reset.mdx) — blocked when `DISABLED`

### Error Codes

| Code | Type | Returned when |
| --- | --- | --- |
| `DISABLED_AUTHENTICATION_METHOD` | `AccountErrorCode` | A password mutation is called when mode is `DISABLED` |
| `PASSWORD_AUTH_RESTRICTION` | `ShopErrorCode` | A password-authenticated admin tries to restrict the mode |

## Blocking Storefront Traffic

**Added in Saleor 3.23.**

By default the GraphQL API is public — anonymous clients and logged-in customers can call it directly. Setting `Shop.allowStorefrontTraffic` to `false` turns that off: only **apps** (app tokens) and **staff users** can reach the API.

This is meant for deployments that put a proxy or a backend-for-frontend in front of the whole API, where that layer holds app or staff credentials and is the only client talking to Saleor directly.

### Example Use Case

Advanced, non-standard setups where customer flows must be controlled by your own backend logic. Some operations can be proxied to Saleor as-is, while others need enrichment or extra protection to keep business rules valid.

For example, a multi-tenant storefront where each customer may only shop in certain sub-shops. Saleor alone cannot express that constraint, so the backend must resolve which channels a given customer is entitled to and rewrite or reject the request accordingly. If customers could also call the API directly, they would be able to bypass that logic and order from a sub-shop they have no access to. Blocking storefront traffic makes the backend the only way in, so the rule always applies.

| Caller | `allowStorefrontTraffic: true` (default) | `allowStorefrontTraffic: false` |
| --- | --- | --- |
| App token | allowed | allowed |
| Staff user | allowed | allowed |
| Customer (non-staff) | allowed | **HTTP 401** |
| Anonymous or invalid token | allowed | **HTTP 401** |

Rejected requests are never executed. The response is:

```json title="HTTP 401"
{
  "errors": [
    {
      "message": "Storefront traffic is not allowed.",
      "extensions": { "code": "STOREFRONT_TRAFFIC_NOT_ALLOWED" }
    }
  ]
}
```

A [batched request](/api-usage/usage-limits.mdx) is rejected as a whole, with a single 401.

### Configuring the Setting

Use the [`shopSettingsUpdate`](/api-reference/shop/mutations/shop-settings-update.mdx) mutation. It requires the `MANAGE_SETTINGS` permission:
...
:::caution
The setting is cached, so a change takes up to **5 minutes** to reach every API worker.
:::

### What Else Gets Blocked

The check runs before the GraphQL document is inspected, so it rejects *every* non-privileged request regardless of the operation. That includes:

- **Authentication mutations** — [`tokenCreate`](/api-reference/authentication/mutations/token-create.mdx), [`tokenRefresh`](/api-reference/authentication/mutations/token-refresh.mdx), [`externalObtainAccessTokens`](/api-reference/authentication/mutations/external-obtain-access-tokens.mdx), [`setPassword`](/api-reference/users/mutations/set-password.mdx), [`requestPasswordReset`](/api-reference/users/mutations/request-password-reset.mdx). A customer cannot log in at all, and therefore can never become privileged enough to pass the check.
- **Introspection** — the schema is not readable without app or staff credentials. The [Playground](#authentication-with-playground) page still loads, but its introspection query fails until you set a staff or app token in the headers.

This is by design: exempting operations would require classifying the document before the check and maintaining an allowlist. Do not enable this setting if you need public login or public introspection.

:::warning
This is an access-control switch, not a DoS protection. Requests are rejected only after the body has been parsed and validated, so blocked traffic still consumes worker resources. Use a WAF or a rate limiter in front of Saleor to shed hostile load.
:::
