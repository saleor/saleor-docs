...
If you are using [Playground](./developer-tools.mdx#playground) to access the API, you can authorize your requests by providing eiher user or app token in the _HTTP HEADERS_ tab. To do that, paste the following JSON structure into the tab, replacing the token with your real token:
