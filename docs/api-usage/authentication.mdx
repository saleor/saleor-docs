...
Note: `csrf_token` field is deprecated and is not used in current authentication flow.
...
Note: when implementing a storefront, you should probably use the [`me`](#fetching-information-about-currently-authenticated-user) query instead.
...
Note: Keep in mind that you should not add the `Authorization` header with an expired access token to this request. It'll result in an error.
