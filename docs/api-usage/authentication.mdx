...
Unlike regular users, Saleor Apps have long-living tokens they can use to communicate with the API. The tokens are assigned at the App installation or creation time and must be stored securely.

In Saleor there are two types of apps:

- **Third-party apps**: external applications that are installed in Saleor. The installation is fully automated. Saleor uses a defined [App manifest](/developer/extending/apps/architecture/manifest.mdx) to gather all required information.
- **Local apps**: can be created via the GraphQL API or the Saleor Dashboard. The app is fully manageable from the dashboard. You can change assigned permissions, add webhooks, or the authentication token.

To get an auth token, you need to create or install an app. To learn how to do that, see the [Apps section](/developer/extending/apps/installing-apps.mdx).
