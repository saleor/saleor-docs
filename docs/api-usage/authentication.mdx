...
A [batched request](/api-usage/usage-limits.mdx) is rejected as a whole: the response is a single 401 carrying one such error object per batch entry.
...
The check runs before the GraphQL document is inspected, so it rejects _every_ non-privileged request regardless of the operation. That includes:
