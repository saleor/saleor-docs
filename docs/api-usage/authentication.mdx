...
:::info
`csrf_token` field is deprecated and is not used in current authentication flow.
:::

### Fetching information about currently authenticated user

You can use the [`me`](api-reference/queries/me.mdx) query to retrieve information about the currently authenticated user:
...
Example implemented in TypeScript with use of the `jose` library:
...
You can verify tokens by calling the [`tokenVerify`](api-reference/mutations/token-verify.mdx) mutation:
...
:::caution
Keep in mind that you should **not** add the `Authorization` header with an expired access token to this request. It'll result in an error.
:::
...
In the most common scenario, you'll use the `tokenCreate` mutation with user's email and password when they try logging in inside a storefront.

You'll receive an access token and a refresh token. Depending on your use-case, you might consider **storing refresh token** in `localStorage` so that the token is persisted between page reloads and browser sessions.
...
If you make a request with an expired access token, you'll receive `ExpiredSignatureError` ("Signature has expired"). You can implement a flow where your app would automatically try to refresh the access token in such cases, and then repeat the same request with the newly generated access token.

When refreshing access token fails, you should remove the refresh token from storage and ask user to log in again.

```mermaid
sequenceDiagram
  autonumber
  Browser ->> Saleor: tokenCreate()
  Saleor -->> Browser: token + refreshToken
  ...
  Saleor -->> Browser: information about user
