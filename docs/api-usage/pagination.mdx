...
:::info Cursors are not stable
Saleor does not guarantee that cursors remain valid over time. A cursor that you stored or cached can stop being recognized by the API, and the query can fail or return unexpected results.

This is not a breaking change - treat it as a normal condition. Clients must handle it by resetting pagination and starting from the first page. Do not persist cursors as long-lived identifiers.
:::
