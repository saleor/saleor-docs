...
1. Use the GraphQL `shopDomainUpdate` mutation shown below to update Saleor's shop domain configuration with the new one.
   ...
   ```
   Note that Saleor instances must be restarted for the new configuration to take effect, as shop data is cached in memory.
2. Set the [`ENABLE_SSL`](#enable_ssl) environment variable correctly.
