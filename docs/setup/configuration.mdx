...
Specifies base URL at which Saleor is hosted, such as `https://api.example.com/`. This setting takes precedence over both `ENABLE_SSL` and `Shop.domain` when generating URLs. Ensure to provide the complete URL, including the protocol (`http://` or `https://`).
...
If deploying Saleor under a specific domain, like `api.example.com`, it's crucial to ensure the system is set up to generate URLs properly. You can achieve that using `PUBLIC_URL` environment variable.
