...
Sets the maximum number of aliases that can be provided within a single GraphQL request.

**Defaults to `100`.**

Example query using aliases (uses **three** aliases):
...
Sets the maximum number of queries that can be sent at once within one GraphQL request.

**Defaults to `1`** (meaning batch queries are effectively disabled).

A batch query is a JSON array such as this one (sends **two** queries within a batch):
...
### `GRAPHQL_MUTATION_COUNT_LIMIT`

Sets the maximum number of mutations that can be sent at once within one GraphQL request.

**Defaults to `1`** - meaning sending multiple mutations in a single request isn't allowed.

For example, the following GraphQL sends **two** mutations within a single request:
