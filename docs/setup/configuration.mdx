...
When Saleor is exposed at a domain (e.g. `api.example.com`), it must be configured to generate URLs properly. Follow these steps to update the domain configuration:

1. Use the GraphQL `shopDomainUpdate` mutation shown below to update Saleor's shop domain configuration with the new one.
...
Note that Saleor instances must be restarted for the new configuration to take effect, as shop data is cached in memory. 2. Set the [`ENABLE_SSL`](#enable_ssl) environment variable correctly.

By following these steps, Saleor can be configured to generate URLs compatible with the domain it is exposed on, enabling smooth bidirectional communication.
