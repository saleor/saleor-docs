...
Since Saleor 3.20, this setting has moved to each channel's configuration and this environment variable is no longer read at runtime. It was only used once, during the upgrade to 3.20, to set the starting value for each channel — along with `TRANSACTION_BATCH_FOR_RELEASING_FUNDS`, which at the time determined whether the feature was on at all. If you upgraded from an earlier version with `TRANSACTION_BATCH_FOR_RELEASING_FUNDS` set to `0`, the feature came out of the upgrade turned off for your channels. To change the setting now, use the channel's payment settings in the Dashboard, or the `channelUpdate` mutation. See [Releasing Funds for Abandoned Checkouts](/developer/checkout/lifecycle.mdx#releasing-funds-for-abandoned-checkouts) for details.

:::

Historically defined the TTL of a checkout before it was marked as abandoned and its funds released. The value is a time expression like `5m`, `5 minutes`, `5d`, `5 days`, `1w`, or `1 week`. Defaults to `6 hours`.

### `TRANSACTION_BATCH_FOR_RELEASING_FUNDS`

Controls how many transactions the background task for [releasing funds on abandoned checkouts](/developer/checkout/lifecycle.mdx#releasing-funds-for-abandoned-checkouts) processes each time it runs. Defaults to `60`.
...
:::danger
This has been removed in Saleor v3.23.0, see [Upgrading From 3.22 To 3.23] for more information.

This environment variable is dangerous to use and is only here to help for backward compatibility.
...
Consider opening a feature request instead (https://github.com/saleor/saleor/issues) so we can add support out of the box for the protocols that you need.
:::
...
[Upgrading From 3.22 To 3.23]: ../upgrade-guides/core/3-22-to-3-23.mdx
