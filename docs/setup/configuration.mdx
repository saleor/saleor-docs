...
To change the setting now, use the channel's payment settings in the Dashboard, or the `channelUpdate` mutation. See [Releasing Funds for Abandoned Checkouts](/developer/checkout/lifecycle.mdx#releasing-funds-for-abandoned-checkouts) for details.
