...
The mutation also generates a refresh token and returns it as a secure, HTTP-only cookie, and as a field (`refreshToken`) in the response. The refresh token can be used to generate new access token without the need to store the user's password, see [refreshing token](developer/users.mdx#refreshing-an-access-token). As cookies can be subject to cross-site request forgery attacks, a CSRF token is also generated that needs to be stored and presented when refreshing an access token.
...
Saleor's tokens are signed with RS256. The token verification can be done by using a public key, which can be fetched from `http(s)://<your-backend-domain>/.well-known/jwks.json`.
