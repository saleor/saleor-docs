...
In order to enable Click & Collect option in the given `Warehouse`, please select `All Warehouses` or `Local stock only`. `All Warehouses` option indicates, that ordered products can be picked up from the selected warehouse and shipped here from different `Warehouse`s. `Local stock only` means, that ordered products can be picked up from a selected warehouse only if the entire order would be fulfilled from stocks in the selected `Warehouse`.
