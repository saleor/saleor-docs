...
The channel permissions were introduced to allow limiting staff user access only to certain channels.
To ensure the proper functioning of OAuth permissions, ensure that the `Default permission group name for new staff users` is set to a permission group with no channel restrictions.
