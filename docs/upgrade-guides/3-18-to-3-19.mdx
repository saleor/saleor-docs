...
In previous versions (legacy tax flow), users could select to use the Tax app or flat rates. This means that if there are multiple tax applications or plugins enabled in your environment, Saleor will attempt to use them, which can result in taxes not being properly configured. Because of that, we added an option to select a specific method of tax calculation. To change it:
...
4. To migrate to new tax flow select one of options from dropdown below warning:
