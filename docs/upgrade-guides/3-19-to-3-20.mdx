...
:::info
By "paid" in this guide we mean that the `TransactionItems` connected to `Checkout` have fully covered [`Checkout.totalPrice`](/api-reference/checkout/objects/checkout.mdx#checkouttotalpricetaxedmoney---), which results in [`Checkout.authorizeStatus`](/api-reference/checkout/objects/checkout.mdx#checkoutauthorizestatuscheckoutauthorizestatusenum---) being set to [`FULL`](/api-reference/checkout/enums/checkout-authorize-status-enum.mdx).
...
:::tip
You can determine if checkout was automatically completed by making `checkout` query, if Saleor returns `null`, then you can assume that it was automatically completed:
