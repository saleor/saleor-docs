...
:::tip
After calling `transactionInitialize` or `transactionProcess` you can determine if `Checkout` could have been completed by checking the `TransactionEvent` object returned in the mutation response.

Transaction is considered successful in a context of a `Checkout` when it has one of the following `types`:
