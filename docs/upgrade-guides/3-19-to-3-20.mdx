...
Before you enable this feature make sure that you don't rely on any `Checkout` query or mutation that updates `Checkout` after transaction is crated (via `transactionInitialize` or `transactionProcess` mutations).
