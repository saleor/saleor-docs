...
## Prevent from overriding `UNCONFIRMED` status
Order status `UNCONFIRMED` could in some cases be overriden in previous Saleor versions. It could happen when, for an `UNCONFIRMED` order, `CHARGE` action was executed or unapproved fulfillment was created. 
Currently order status `UNCONFIRMED` will only be changed in these scenarios:
- `automaticallyConfirmAllNewOrders` is selected
- `orderConfirm` mutation is executed
- order is cancelled
