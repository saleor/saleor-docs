...
Previously, the `amount` could be set to null or 0. In Saleor 3.21, this field will contain the `amount` requested by the user.
If the user does not provide an `amount`, Saleor will use the maximum amount available on `transaction.authorizedAmount`.
