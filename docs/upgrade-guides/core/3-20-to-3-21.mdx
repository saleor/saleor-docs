...
Instead of recalculating taxes externally, the system will return the **pre-stored prices** from the database.

#### Affected queries:
The following queries will return prices stored in the database:
- `checkouts(...)`
- `me.checkouts(...)`
- `checkoutLines(...)`
...
#### Example queries:
Here are examples of queries that **do not** trigger external tax calculations and will instead return the stored prices:

##### Fetching checkouts for authenticated user:
```graphql
{
  me {
    checkouts(first: 10) {
...
:::note
The `checkout` query will behave as it did previously. If the taxes are expired, external calls will be made to calculate them.
:::
