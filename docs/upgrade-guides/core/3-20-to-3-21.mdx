...
In previous versions, orders that had any unapproved fulfillment and no returns, were given status `PARTIALLY_FULFILLED`. This happened even when no items were actually fulfilled. 
...
:::note
Statement above also applies to `UNCONFIRMED` orders. In previous versions, adding fulfillment to `UNCONFIRMED` order will result in status `PARTIALLY_FULFILLED`.
:::