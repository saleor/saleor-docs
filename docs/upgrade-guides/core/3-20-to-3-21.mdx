...
## Strict authorization token validation

Saleor will enforce rigorous validation for all types of authorization tokens, including Saleor-generated tokens, tokens used by the OIDC plugin, and app tokens. In previous versions, when Saleor received an invalid token, it would return a `200 OK`response with a query executed as an anonymous user. This behaviour has been changed to return a `401 Unauthorized`. This change may affect your custom apps that rely on the previous behavior, also storefronts that send invalid tokens to Saleor could be affected. If you have any custom logic that relies on receiving a `200 OK` response for invalid tokens, you will need to update it before upgrading to version 3.21.

### Examples executed with invalid token

#### Query with required permission
Query:
```graphql
query ShopQuery {
  shop {
...
mutation {
  channelCreate(
    input: {
      ...
      currencyCode: USD
      defaultCountry: PL
      orderSettings: { useLegacyLineDiscountPropagation: true }
    }
  ) {
    channel {
      id
...
## Behavior change in `accountRegister` mutation

#### Before Saleor 3.21

When calling `accountRegister` for an already existing user, Saleor returned an error response, for example:
...
To prevent user enumeration attacks, the `accountRegister` mutation no longer returns an error when the user already exists.
Instead, it always returns a user object. The id field is always set to an empty string:
...
If your application needs to verify whether a user with the given email already exists in Saleor, you must perform an additional [query](/api-reference/users/queries/user.mdx).
This step needs to be performed by app with proper permissions.
