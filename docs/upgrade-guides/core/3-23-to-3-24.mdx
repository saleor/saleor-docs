...
1. Ensure your apps (manifests) don't have `MANAGE_APPS` permission. 
2. Recommended: manually update apps (`appUpdate` mutation) to remove `MANAGE_APPS` permission and test if they work while still on 3.23. This way you can make sure everything works as expected, otherwise Saleor will do that automatically.