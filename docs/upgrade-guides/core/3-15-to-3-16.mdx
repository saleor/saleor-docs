...
:::danger
If you are making requests on behalf of your customers (for example if you use a proxy or have a backend-for-frontend architecture), make sure to provide `customerIpAddress` parameter in [`transactionInitialize`](/api-reference/payments/mutations/transaction-initialize#customeripaddressstring) and [`transactionProcess`](/api-reference/payments/mutations/transaction-process#customeripaddressstring) mutations:
