...
:::info
By "paid" in this guide we mean that the `TransactionItems` connected to `Checkout` have fully covered [`Checkout.totalPrice`](/api-reference/checkout/objects/checkout.mdx#total-price), which results in [`Checkout.authorizeStatus`](/developer/payments/lifecycle.mdx#authorization-status-calculation-for-checkout) being set to [`FULL`](/api-reference/checkout/enums/checkout-authorize-status-enum.mdx).
...
:::warning
If you use [`orderCreateFromCheckout`](/api-reference/orders/mutations/order-create-from-checkout.mdx) mutation with parameter [`removeCheckout: false`](/api-reference/orders/mutations/order-create-from-checkout.mdx#remove-checkout) multiple `Orders` may be created from a single `Checkout`.
