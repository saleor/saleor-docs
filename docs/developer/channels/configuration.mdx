...
- **Default country**: When products are queried, the default country is used to determine the stock availability of the product. This is only relevant when [`useLegacyShippingZoneStockAvailability`](/api-reference/miscellaneous/objects/shop.mdx#use-legacy-shipping-zone-stock-availability) is enabled.
