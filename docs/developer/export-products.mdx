...
### File
Link to download file is sent by email to requestor. If any variants fields are
specified every row in file corresponds to one variant,  otherwise single row
