...
Every row in file corresponds to one variant, if any variants fields are specified
otherwise single row corresponds to one product.
...
<Chart
  definition={`
  graph TD
    D[Export products data <br> Background Worker] -->|Success| E[Send email to user <br> with a link to <br> download csv file]
