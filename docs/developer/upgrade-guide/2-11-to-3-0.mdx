...
Besides the impact on fetching the data, you'll be required to specify channel during creating the checkouts. You'll find updated guide at [Checkout](developer/checkout.mdx) page.
...
If you use API to modify objects (for example with external app), please note that channel depending fields have dedicated mutations: updating product price requires using [productChannelListingUpdate](developer/api-reference/mutations/product-channel-listing-update.mdx) mutation.
...
To render rich text on your storefront you can use Editor.js in `read-only` mode, or use [one of the libraries](https://github.com/editor-js/awesome-editorjs) recommended by its authors.
...
Saleor 3.0 extends possibilities for presenting your products to the customers. Now you can not only use photos but also videos. The old `image` attribute and `ProductImage` objects are left for backward compatibility but are marked as deprecated. If you want to leverage all new features, use `ProductMedia` objects.

Read documentation for the [ProductMedia](developer/products.mdx#media).

## Removed fields from the Shop object

`Shop.homepageCollection` field has been removed. The recommended way of getting a specific collection data in such a situation is to create a new collection and using the `collection` query with the desired slug, for example:
...
## Error handling
The new version unifies error handling across the API. All mutation responses have an `errors` field, which contains issues information. For example:
...
Full list of available events can be found [here](developer/extending/apps/asynchronous-webhooks.mdx#available-webhook-events).
