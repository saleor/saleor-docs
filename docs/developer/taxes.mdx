...
- `taxAppId` - Specify which Tax App should be responsible for tax calculation when tax apps are used as a calculation strategy. To be able to see, which apps can be assigned please check [Shop.availableTaxApps](/api-reference/miscellaneous/objects/shop.mdx#code-style-fontweight-normal-shopbavailabletaxappsbcodeapp--).
  Besides tax apps, you can assign plugins to `taxAppId`. To assign a plugin you need to provide the value: `plugin:PLUGIN_ID`. Please note that plugins will be deprecated.
