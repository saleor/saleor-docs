...
- `taxAppId` - Specify which Tax App should be responsible for tax calculation when tax apps are used as calculation strategy. To be able to see, which apps can be assigned please check [Shop.availableTaxApps](/api-reference/miscellaneous/objects/shop.mdx#code-style-fontweight-normal-shopbavailabletaxappsbcodeapp--)

Additionally, the following properties can be overridden for specific countries within the channel: `chargeTaxes`, `taxCalculationStrategy`, `displayGrossPrices`, `taxAppId`. This allows for handling various situations where a store operates in different countries with different tax rules.
...
If you use `TAX_APP` calculation strategy and decide not to set `taxAppId` then Saleor will try to iterate through all installed Apps and return the first result.
