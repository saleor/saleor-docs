...
- **Use weighted tax for shipping** - When enabled, shipping tax is calculated as a weighted average of product tax rates. Available only when using the `FLAT_RATES` tax calculation strategy.
...
2. **Weighted rate** - Calculate shipping tax as a weighted average of product tax rates (available **only** when using the `FLAT_RATES` tax calculation strategy)
