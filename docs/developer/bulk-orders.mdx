...
Since this operation is more powerful than regular order management, it is not automatically assigned to staff users or apps with `MANAGE_ORDERS` permission. The mutation requires a new permission:` MANAGE_ORDERS_IMPORT`.

### Input details

#### User (`OrderBulkCreateUserInput`)

To find a user, the mutation accepts one of the following identifiers: `id`, `email` or `external_reference`. If the user associated with an order doesn’t exist in Saleor database, user's email must be provided as a reference in the order instance.

#### Delivery method (`OrderBulkCreateDeliveryMethodInput`)

Orders in Saleor can be shipped to customers or collected directly from warehouse. To identify the type of delivery either `warehouseId` or `shippingMethodId` must be provided. There can also be scenario, when an order doesn’t require physical delivery. It happens when all order’s lines have `isShippingRequired` flag set to `false`. In this case, `OrderBulkCreateDeliveryMethodInput` can be omitted.

The input also accepts denormalized names of delivery methods (`warehouseName`and `shippingMethodName` fields) and tax class (`taxClassName`).

If `shippingPrice` is not provided, Saleor will fetch the current shipping method price from database.

#### Order (`OrderBulkCreateInput`)

The mutation allows to set `Order.number` value to any string, as long as it is unique. By default, when new order is created, Saleor generates a number in sequence to store a unique order number. If an integer value is passed to the `number` field, Saleor updates the current value in the underlying database sequence.
...
The input also accepts denormalized names of variant (`variantName`), product (`productName`) and tax class (`taxClassName`).
...
- `REJECT_EVERYTHING` (default): if a single error occurs, in at least one of the orders, the whole mutation fails and no data is saved.
- `REJECT_FAILED_ROWS`: the mutation saves only those orders, which do not produce any error
- `IGNORE_FAILED`: if an error occurs for some order, Saleor will try to save it partially and skip the invalid part, if it is possible. Please note, that some of errors ignore this policy and disqualify whole order:
  - order number is not unique
  - invalid billing address
  - channel can’t be resolved
  - delivery method can’t be resolved (if shipping is required)
  - user can’t be resolved, unless user email is provided as an identifier
