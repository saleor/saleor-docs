...
4. The [payments](/developer/payments/overview.mdx) are processed unless the `Channel` setting of the checkout has [`allowUnpaidOrders`](/api-reference/miscellaneous/objects/order-settings#allowunpaidordersboolean) setting enabled or checkout's total is `0`. If you need to bypass this setting, you use [`orderCreateFromCheckout`](/api-reference/orders/mutations/order-create-from-checkout.mdx).
