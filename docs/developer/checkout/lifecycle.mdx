...
:::note
The checkout deletion task is triggered by [celery beat scheduler](developer/running-saleor/task-queue.mdx).
...
:::note
The releasing funds for abandoned checkouts task is triggered by [celery beat scheduler](developer/running-saleor/task-queue.mdx).
This feature will not work without task queue configuration. 
:::
