...
2. Delivery options and addresses are valid. [Learn more](/developer/checkout/address.mdx).
...
4. The [payments](/developer/payments/overview.mdx) are processed unless the `Channel` setting of the checkout has [`allowUnpaidOrders`](/api-reference/miscellaneous/objects/order-settings.mdx#allow-unpaid-orders) setting enabled or checkout's total is `0`. If you need to bypass this setting, you use [`orderCreateFromCheckout`](/api-reference/orders/mutations/order-create-from-checkout.mdx).
...
In versions prior to 3.20, this behavior was controlled by the environment variables `CHECKOUT_TTL_BEFORE_RELEASING_FUNDS` and `TRANSACTION_BATCH_FOR_RELEASING_FUNDS`, and applied to every channel the same way. Since 3.20, it's a setting you configure per channel instead — see below. `TRANSACTION_BATCH_FOR_RELEASING_FUNDS` still exists, but its job changed: it no longer turns the feature on or off, it just controls how many transactions the background task processes per run.

If you're upgrading from an older version: when you migrate to 3.20, each channel's settings start out based on your old environment variables. If `TRANSACTION_BATCH_FOR_RELEASING_FUNDS` was set to `0` (feature off), the feature will come out of the upgrade turned off for all your channels — you'll need to switch it on per channel afterwards if you want to keep using it.

:::

Each channel exposes the following settings under [`Channel.paymentSettings`](/api-reference/miscellaneous/objects/payment-settings.mdx):
...
These are readable and writable via the [`channelUpdate`](/api-reference/channels/mutations/channel-update.mdx) mutation, and in the Dashboard under **Configuration → Channels → [channel] → Payments & checkout**.
...
For any [`transactionItem`](/api-reference/payments/objects/transaction-item.mdx) with processed funds (`authorizedAmount` or `chargeAmount`) assigned to an abandoned checkout **in a channel with `releaseFundsForExpiredCheckouts` enabled**, Saleor will trigger the release action.
