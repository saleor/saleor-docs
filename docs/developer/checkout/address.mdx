...
2. Use the [accountSetDefaultAddress](/api-reference/users/mutations/account-set-default-address.mdx) mutation to update your default shipping or billing address.
3. Call [`checkoutShippingAddressUpdate`](/api-reference/checkout/mutations/checkout-shipping-address-update.mdx) and/or [`checkoutBillingAddressUpdate`](/api-reference/checkout/mutations/checkout-billing-address-update.mdx) to apply the selected address to the current checkout.
...
After choosing the shipping address, use the [`deliveryOptionsCalculate`](/api-reference/shipping/mutations/delivery-options-calculate.mdx) mutation to explicitly fetch available delivery methods.
...
Shipping methods can be internal (defined in the Saleor Dashboard) or [external](/developer/extending/webhooks/synchronous-events/shipping.mdx) (coming from a third party).

See [delivery method problems](/developer/checkout/problems.mdx#checkoutproblemdeliverymethodstale) for how to handle stale or invalid delivery states.

#### Click And Collect Points

Collection points are [warehouses](/api-reference/products/objects/warehouse.mdx) that allow picking up orders, e.g., for a click & collect flow.
...
- `deliveryMethodId`: the shipping method ID or Warehouse ID (`delivery` from the `deliveryOptionsCalculate` mutation or `availableCollectionPoints` field of the [`Checkout`](/api-reference/checkout/objects/checkout.mdx) object).
