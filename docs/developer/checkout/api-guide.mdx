...
Notice that the return type in `checkout.deliveryMethod` can be either `Warehouse` or `ShippingMethod`, so we need to use [union type](https://graphql.org/learn/schema/#union-types).

Learn more about [delivery methods](/developer/checkout/address#selecting-the-delivery-method.mdx).
...
- [`customerAttach`](api-reference/checkout/mutations/customer-attach.mdx) - Attach a user to the checkout.
- [`customerDetach`](api-reference/checkout/mutations/customer-detach.mdx) - Detach a user from the checkout.
- [`shop.availablePaymentGateways`](/api-reference/miscellaneous/objects/shop.mdx) - Get available payment gateways.
