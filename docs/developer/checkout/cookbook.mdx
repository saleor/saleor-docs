...
You can use a phone number instead of an email address to identify customers. While Saleor always requires email to create orders, you can use the `email` field as a phone number or other semantic meaning. For example: `123456789@yourdomain`. Make sure always to use **domain names you own**, to avoid leaking user data.
