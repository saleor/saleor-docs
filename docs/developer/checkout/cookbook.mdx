...
- Orders are split into fulfillments paid separately to each vendor.
...
- Listen to [`webhook`](/developer/extending/webhooks/overview.mdx) such as [`ORDER_CREATED`](/developer/extending/webhooks/synchronous-events/order.mdx#order-created) to process the fields
...
Checkout lines can have custom prices set by the app


## Using phone number instead as identity

In some cases, you might want to use a phone number instead of an email address for customer identification. While Saleor always requires email to create orders, you can email field as a phone number or other semantic meaning. For example: `123456789@phone`.
...
If your admins need to manage subscriptions, you can use Saleor's dashboard to create a dedicated UI in the Saleor dashboard that will talk to your subscription service.
