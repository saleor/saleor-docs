...
- Eager: Mutation will call `*_CALCULATE_TAXES` event during the mutation execution. That means mutation will run slower and the returned value will have latest updates prices values.
...
sequenceDiagram
    ...
    Saleor-->>Saleor DB: Mark prices to be "dirty"
    Saleor->>Storefront: Ok<br/> { errors: [] }

    Storefront->>Saleor: mutation checkoutLineUpdate { errors }
    Saleor-->>Saleor: Prices are dirty already
    Saleor->>Storefront: Ok<br/> { errors: [] }
    ...
    Saleor->>Saleor DB: Mark prices to be "not dirty"
...
Following Checkout mutations are eager:
- TODO

Other mutations are lazy
