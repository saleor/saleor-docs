...
- Using Payment Apps (recommended): The communication between the storefront and the payment app goes through
  Saleor. Based on the responses from the payment provider, Saleor creates or updates transactions with the [proper statuses and balances automatically](developer/payments/lifecycle.mdx#recalculations-of-amounts).
