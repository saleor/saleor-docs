...
Anyone who knows this id can query or modify the checkout, regardless of whether it was assigned to a customer.

### Public Access

When you know the checkout id, the following is available without authentication:
...
Certain fields are never exposed publicly. They require either staff permissions or ownership:

- **user**: only returned for the authenticated owner (customer) or staff
- **transactions, storedPaymentMethods** – require payment/checkout staff permissions
- **privateMetadata** – staff only
