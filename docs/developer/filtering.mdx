...
The new filtering approach was introduced in **Saleor 3.11**. For now, it's only
...
New filtering allows specifying more complex conditions with the use of `AND` and `OR` operators,
combined with operations like equal (`eq`), one of, and range. It's available under the `where` option,
for now, only on attributes query.
...
- It's allowed to filter by flat fields like it was before, as well as specifying operators,
  but this approach cannot be mixed. ([See the example](#mixing-flat-field-filter-and-operator-and-or)).
- Only one operator can be specified on each level. ([See the example](#mixing-operators-on-the-same-level)).
- In the new approach we decided to add a filter option for each field of the given type.
  Because of that, the `search` filter is moved to the top level. ([See the example](##search-filtering)).
- The filter input type matches the property name in the API type.
- The string, enum, date, and all numeric type fields have specific input types
  with available operations options. Currently there is option to filter by specific
  value ([example](#filtering-by-a-specific-value)) and list of values ([example](#filtering-by-a-list-of-values)).
- Only one operation is allowed to specify. Only `eq` or `oneOf`, providing both will raise an error. ([See the example](#mixing-different-operations)).
- It's not allowed to provide `filter` and `where` options at the same time. ([See the example](#mixing-filter-and-where-options)).
...
The `AND` operator allows defining a list of conditions that must be fulfilled.
...
The `OR` operator allows defining a list of conditions that at least one must be fulfilled
...
There is still a possibility to search objects. The search is available on the root level.
