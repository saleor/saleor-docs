...
Each attribute has an input type defined upon attribute creation. Input types determine the data type you can enter as the attribute values in the dashboard. Available input types are defined in the [`AttributeInputTypeEnum`](developer/api-reference/enums/attribute-input-type-enum.mdx) enum. Below is the list of available input types:
...
Attributes assigned to an object are represented by the [`SelectedAttribute`](developer/api-reference/objects/selected-attribute.mdx) GraphQL type. Below is the example of fetching attributes assigned to a product:
