...
Configure these constraints on the attribute using the [`AttributeCreate`](/api-reference/attributes/mutations/attribute-create) or [`AttributeUpdate`](/api-reference/attributes/mutations/attribute-update)
...
See the [guide](/developer/attributes/api#setting-allowed-reference-types) on configuring reference attributes with restricted choices.
