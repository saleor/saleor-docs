...
For example, the attribute **T-shirt Size** might have values **S - L** while the attribute **Shoe Size** has values **34-46**. These can be used in different product types without creating attributes from scratch for each shoe or t-shirt product type.

Using dropdown attributes also helps maintain consistency across your store.

## Attributes vs. Metadata Difference

Saleor uses both Attributes and [Metadata](/api-usage/metadata.mdx) in various places. Attributes have a strict structure with data types, validation, and schema. Metadata is a simple `key: value` structure without types. You might wonder whether you need both, or which to use where?

Attributes:

- Intended to be filled in by people
- Strictly typed
- The schema is strict so storefronts can depend on this value being of a specific shape (e.g., a field for number of pages will not contain a photo of jeans)
- Attributes are typically used in the UI layer, presented to customers, merchandisers, or admins

Metadata:

- Intended to be filled by an app, automation, or integration through the API
