...
Saleor's tokens are signed with RS256. The token verification can be done by using a public key, which can be fetched from `http://<your-backend-domain>/.well-known/jwks.json`.
