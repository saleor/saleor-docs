...
Depending on the configuration of your Saleor backend instance, registering a new customer account may be a single-step operation, or it may require email confirmation to activate the account (see [`ENABLE_ACCOUNT_CONFIRMATION_BY_EMAIL`](setup/configuration.mdx#enable_account_confirmation_by_email)).
...
Saleor API uses the [JSON Web Token](https://jwt.io/) authentication mechanism.
...
The mutation also generates a refresh token and returns it as a secure, HTTP-only cookie and as a field (`refreshToken`) in the response. The refresh token can be used to generate a new access token without the need to store the user's password; see [refreshing token](developer/users.mdx#refreshing-an-access-token). As cookies can be subject to cross-site request forgery attacks, a CSRF token is also generated that needs to be stored and presented when refreshing an access token.

For more details about expiration time of the token, see [token expiration time](setup/configuration.mdx#jwt_ttl_access).
...
For more details about expiration time of the `refreshToken`, see [refresh token expiration time.](setup/configuration.mdx#JWT_TTL_REFRESH)
...
The [`me`](api-reference/queries/me.mdx) query allows you to get information about currently logged in user.
