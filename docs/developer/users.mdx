...
The mutation also generates `refreshToken` and returns it as a secure, http-only cookie - `refreshToken` and as a field in the response. `refreshToken` can 
be used to re-generate an expired authentication token, see [refreshing token.](developer/users.mdx#refreshing-a-token)

For more details about expiration time of the token, see [token expiration time.](developer/running-saleor/configuration.mdx#JWT_EXPIRATION_DELTA)
...
```
:::note
You can verify authentication token as well as the refresh token.
:::
...
The `tokenRefresh` mutation will try to fetch token from the secure, http-only cookie - `refreshToken`. 
If it fails then it will use `refreshToken` provided as an input of mutation. If `refreshToken` 
is valid it will re-generate an authentication token and return it in the response of mutation.

For more details about expiration time of the `refreshToken`, see [refresh token expiration time.](developer/running-saleor/configuration.mdx#JWT_REFRESH_EXPIRATION_DELTA)

```graphql {2}
mutation {
  tokenRefresh(refreshToken: "<your-token>") {
    token
    accountErrors{
      code
    }
...
```
### Deactivating all user's tokens

The `tokensDeactivateAll` mutation will invalidate all user's tokens (access and refresh). The mutation is avaiable only for logged in users.
