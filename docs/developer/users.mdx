...
The `tokenRefresh` mutation will generate a new access token when provided with a valid refresh token and a matching CSRF token.
If the refresh token is not provided as an argument, the code will try to read it from a cookie set by the `tokenCreate` mutation.

The mutation takes the following input fields:

- `refreshToken`: the refresh token, if not specified will be read from browser cookies.
...
```graphql {2}
mutation {
  tokenRefresh(refreshToken: "<your-refresh-token>", csrfToken: "<your-csrf-token>") {
    token
    accountErrors {
      code
    }
