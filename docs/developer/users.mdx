...
The mutation generates also `refreshToken` and returns it as a secure, http-only cookie - `refreshToken` and as a field in the response. `refreshToken` can 
be used to re-generate an expired authentication token:
