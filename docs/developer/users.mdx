...
Saleor's tokens are signed with RS256. The token verification can be done by using a public key. The key can be fetched from `<your-backend-domain>/.well-known/jwks.json` (`http://127.0.0.1:8000/.well-known/jwks.json`).
