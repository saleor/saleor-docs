...
Creating a new account requires specifying a channel. It is needed because you can configure different email templates on a by-channel basis.

User accounts are not restricted to a single channel. For example, a user registered in channel `europe` can still log in and make a purchase in channel `usa`.
