...
The discount from rules with `checkoutAndOrder` predicate is automatically applied on checkout
or draft order when object met the certain conditions.
Only one promotion rule discount that gives the best saving for the customer is applied.
The checkout and order discount can be mixed together. In such case, the discount
is calculated based on discounted price.
...
Assume that checkout has only on product that is on catalogue discount, and the discounted price is **$12**.
The best promotion rule for this checkout is giving **10%** discount on subtotal price.
In such case the checkout and order discount will be **10%** from **$12** which gives **$1.2**.
...
Checkout and order predicates allow to definition of the conditions for checkout and draft order objects based
on their attributes. Currently only possible to define the conditions based on total and subtotal price.

Catalogue and checkout and order promotion discounts can be mixed, so for checkout/draft order qualification the discounted price
(after catalogue discount) is used
