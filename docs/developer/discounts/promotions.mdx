...
- checkout and order promotions - applied on checkout / draft order when object fulfill
  the rule conditions.
...
### Checkout and order promotions

Discounts from rules with the checkoutAndOrder predicate are automatically applied during checkout or draft order when certain conditions are met. Only the discount from the promotion rule that provides the maximum savings for the customer is applied. The checkout and order discounts can be combined, and in such cases, the discount is calculated based on the discounted price.
...
- `TOTAL_DISCOUNT` - applies the discount on total price
...
In this scenario, the checkout and order discount will be **10%** from **$12** resulting in a savings of **$1.2**.

:::note
For now, the checkout and order promotions are applicable only on checkouts, and don't to draft orders.
:::
...
- **Checkout and order predicate:** Defines the conditions to be met by checkout or draft order. When the conditions are met, the discount is applied automatically.
...
- **Reward type:** (Applicable only for checkout and order promotions) Describe what kind of reward is given when the promotion rule conditions are met.
...
Reward type is applicable only for checkout and order promotions and specify what kind of reward
will be given when the defined conditions are met.
Currently, only discount `SUBTOTAL_DISCOUNT` and `TOTAL_DISCOUNT` is available.
...
### Checkout and order predicate

The checkout and order predicatesenable the definition of conditions for checkout and draft order objects based on their attributes. Currently, it is only possible to define conditions based on the total and subtotal price.

Catalogue and checkout and order promotion discounts can be combined. Therefore, during checkout or draft order qualification, the discounted price (after applying any catalogue discount) is utilized.

### Checkout and order discount examples
...
- **$5** discount applied when the total price is above **$50** applied on total price
...
- `checkoutAndOrderPredicate`: defines the conditions to be met to apply checkout and order discount.
  Allows defining complex nested conditions with `AND` and `OR` operators.
...
#### Creation of checkout and order promotion rule

The following example demonstrates the creation of a checkout and order promotion rule that
gives **10%** total price discount on checkout/draft order that subtotal price is greatest
than or equal to 20.
...
## Processing checkout with a applied checkout and order promotion

## Catalogue and checkout and order promotions

Catalogue and checkout and order promotions can be mixed together. In such case the
checkout and order discount is applied on discounted price.
...
## Checkout and order Promotion and Voucher

Checkout and order promotion cannot be mixed with Vouchers.
When voucher code is added, the discount from checkout and order promotion is not applied.
