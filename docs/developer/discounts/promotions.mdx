...
The type of rule is determine by the promotion to which the rule is assigned.
...
The order discounts can be combined with catalogue discount, and in such cases, the discount is calculated based on the discounted price.
...
:::note
For now, the order promotions are applicable only on checkouts, and not on draft orders.
:::
...
Base prices are the prices after applying catalogue promotions and do not include vouchers or order discounts.
...
Catalogue level predicates allow to define conditions for variants, products, collections and categories.
