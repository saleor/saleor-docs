...
- `cataloguePredicate`: defines the conditions to be met to apply catalogue discount.
  **Required for rule that belong to `CATALOGUE` promotion.**
...
- `orderPredicate`: defines the conditions to be met to apply order discount.
  **Required for rule that belong to `ORDER` promotion.**
  Allows defining complex nested conditions with `AND` and `OR` operators.
...
#### Following chart is showing an order in which discounts apply:

```mermaid
graph TD
    A(Base undiscounted price) --> B[Apply catalogue discounts]
    B --> Z{Voucher code added?}
    Z --> |No| K[Apply `checkoutAndOrder` discount]
    K --> E
...
#### Below chart is showing the process of applying `ORDER` promotion discount

```mermaid
graph TD
    A(Base discounted price) --> B[Collect all `ORDER` promotion rules that apply]
    B --> C[Choose the promotion rule hat gives the best saving for user]
    C --> D[Apply chosen promotion rule on discounted price]
```
