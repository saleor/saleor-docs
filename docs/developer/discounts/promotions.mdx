...
Discounts from rules with the `ORDER` predicate are automatically applied during checkout or draft order when certain conditions are met.
...
- `predicateType`: define the type of predicate. Determine if the promotion rule is `catalogue` or `order` type.
...
and set `CATALOGUE` as predicate type.
...
and set `ORDER` as predicate type.
...
## Processing checkout with a applied order promotion
