...
For gift reward, the promotion rule must specify the available gifts as product variants
...
it can be defined only for promotion with `ORDER` type.
For this kind of promotion rule, the `rewardValue` and `rewardValueType` is empty.
...
## Completing checkout with gift reward
...
We can see extra line added to checkout which is marked with `isGift` flag.
