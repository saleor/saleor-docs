...
To create a promotion use [`promotionCreate`](/api-reference/discounts/mutations/promotion-create.mdx) mutation.
...
To add a promotion rule to the promotion use [`promotionRuleCreate`](/api-reference/discounts/mutations/promotion-rule-create.mdx) mutation.
...
[PromotionEventsEnum](/api-reference/discounts/enums/promotion-events-enum.mdx).
...
In the case of a variant on sale, the applied discount is visible only on the order and lines
