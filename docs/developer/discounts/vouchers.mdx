...
Vouchers are a way to apply discounts to orders. They can be used to reduce the cost of a checkout.
Vouchers can be applied to the entire order or to specific products. They can also be limited to a minimum quantity of products in the checkout.
...
Another way to limit the usage of a voucher is to set the `usageLimitPerCustomer` field to `true`. In this case, the voucher code will be
available for use only once per customer.

The voucher can be also set as single-use, when `singleUse` flag is set to `true`. In this case, the codes from the voucher can be used only once and then the code will be deactivated.

Those limitation options can be combined. For example, if `usageLimit` is set to `10` and `usageLimitPerCustomer` is set to `true`, the voucher can be used by the first 10 users.
    ...
    "voucherCreate": {
      ...
      "voucher": {
        ...
        "codes": {
          ...
          "pageInfo": {
            "startCursor": "WyIyMDIzLTEwLTIzIDA4OjIwOjEyLjIyNjQxMCswMDowMCIsICJjb2RlMyJd",
            "endCursor": "WyIyMDIzLTEwLTIzIDA4OjIwOjEyLjIyNjM4MyswMDowMCIsICJjb2RlMSJd",
            "hasNextPage": false,
...
You can learn more about vouchers codes export [here](developer/export/export-voucher-codes.mdx).
