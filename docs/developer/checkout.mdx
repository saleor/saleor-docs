...
The charts below show the workflow of a checkout process.
...
This feature is currently in a **Feature Preview** state. This part of Saleor is not yet complete
and subject to change but is available to experiment and provide feedback.

:::

Transaction stores details of payment transactions attached to an order or checkout.
...
:::
The [transactionUpdate](developer/api-reference/mutations/transaction-update.mdx) mutation allows to update the details of a transaction.
...
### Handling an action request for transaction

Action request is called when a staff user or an app triggers a request of action that should be called for a given transaction.
If your app should also process payment request actions (like charge, refund, or void) triggered by staff users on
Saleor dashboard side or by app, make sure that your app is subscribed to
