...
- `availableCollectionPoints`: a list of available places of self-pickup. It is calculated based on given country and available Stocks (for Warehouses which `clickAndCollectOption` field is not set to `DISABLED`). For further click&collect feature explanation see [Warehouse settings](dashboard/configuration/warehouses.mdx#clickcollect-related-settings)
