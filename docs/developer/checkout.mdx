...
If Checkout is assigned to the user, only this user and staff user with permission `MANAGE_CHECKOUTS` can query its data. Checkouts without assigned user can be queried modified using token, without additional permissions. 
