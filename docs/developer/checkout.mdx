...
- `input`: [PaymentInput](developer/api-reference/inputs/payment-input.mdx) object:
  ...
  - `partial`: when set to True, multiple payments can be attached to a single checkout. Creating a payment with partial=False will cancel all previous payments.
...
There are two kind of payments:

- `Full payments`: only one payment per checkout is needed and it must cover the whole amount. The argument `partial` must be `False`.
- `Partial payments`: multiple payments are allowed per checkout, each can cover a fraction of the total amount. All payments together must cover the total amount. The argument `partial` must be `True`.

Here is a couple of notes related to the flow:

- If some of the partial payments are already processed and then subsequentely was created a full one, the partial payments will get refunded.
- If there is a full payment that is being processed (e.g. during 3DS) and a new partial payment has been created, then the full payment will be abandoned.
...
This mutation allows to process an individual payment.
It can be useful with multiple payments and should be called between
[`checkoutPaymentCreate`](developer/api-reference/mutations/checkout-payment-create.mdx) and [`checkoutComplete`](developer/api-reference/mutations/checkout-complete.mdx).
Unlike the [`checkoutComplete`](developer/api-reference/mutations/checkout-complete.mdx), this operation doesn't create an order.

The [`checkoutPaymentComplete`](developer/api-reference/mutations/checkout-payment-complete.mdx) mutation requires the following input:

- `token`: the token of the checkout to complete.
- `paymentId`: The ID of a payment that need to be processed.
- `paymentData`: client-side generated data required to finalize the payment.

It returns the following output:

- `checkout`: related `Checkout` object.
- `payment`: a `Payment` object that has been processed.
