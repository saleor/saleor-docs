...
The below process describes the key milestones in the checkout process flow in Saleor. Additional steps may also occur along the way; however, the purpose of this instruction is to deliver a base reference for the user to work with. We assume that at this stage, you have already completed the steps included in the [GraphQL](getting-started/graphql.mdx) section of this chapter and that you are familiar with the basic setup of the Saleor GraphQL API.

You can run the code snippets in this section in the [Playground](getting-started/graphql) or your preferred HTTP client.
...
To modify a line, use [checkoutLinesUpdate](developer/api-reference/mutations/checkout-lines-update.mdx).

Currently, [CheckoutLineUpdateInput](api-reference/inputs/checkout-line-update-input) supports `variantId` and `lineId` parameters to operate on the selected line object.
...
If the `variantId` parameter is used in line input and the variant exists in multiple lines, [checkoutLinesUpdate](developer/api-reference/mutations/checkout-lines-update.mdx) will return an error. In that case, you are required to use `lineId`.
...
Depending on the selected payment gateway, you will use the payment provider's form, which can be integrated with Saleor or be redirected to an external payment page. The payment gateway returns information if the payment is successful, along with tokenized credit card payment information. This token is then used to run the [`checkoutPaymentCreate`](developer/api-reference/mutations/checkout-payment-create.mdx) mutation.
...
The transaction has status authorized, and payment was made with a credit card. The actions that can be called from Saleor are: `VOID` and `CHARGE`.
...
:::
The [transactionUpdate](developer/api-reference/mutations/transaction-update.mdx) mutation allows to update the details of the transaction.
...
Anonymous and unfinished checkouts are deleted after a specified time from their last modification:
