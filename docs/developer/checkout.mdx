...
## Address Validation
The checkout's mutations that accepts an address as an input have field that can turn off address validation. This allows to assign partial or not fully valid address to the checkout. Providing country code is mandatory for all addresses regardless of the rules provided in this input.

The address [validation](developer/api-reference/inputs/checkout-address-validation-rules.mdx) input has two boolean fields: 
- `checkRequiredFields` - Determines if an error should be raised when the provided address doesn't have all the required fields. Default set to true.
- `checkFieldsFormat` - Determines if an error should be raised when the provided address doesn't match the expected format. Default set to true.
- `enableFieldsNormalization` - Determines if Saleor should apply normalization on address fields. Example: converting city field to uppercase letters. Default set to true.

#### checkoutCreate
The [`checkoutCreate`](developer/api-reference/mutations/checkout-create.mdx) mutation has optional input for providing shipping and billing addresses. By disabling an address validation there is a possibility to provide only part of an address during checkout creation. 
...
The [`checkoutShippingAddressUpdate`](developer/api-reference/mutations/checkout-shipping-address-update.mdx) mutation has optional field for controling a shipping address validation: [validationRules](developer/api-reference/inputs/checkout-address-validation-rules.mdx).
...
The [`checkoutBillingAddressUpdate`](developer/api-reference/mutations/checkout-billing-address-update.mdx) mutation has optional field for controling a billing address validation: [validationRules](developer/api-reference/inputs/checkout-address-validation-rules.mdx).
...
:::note
The fields of shipping and billing addresses will be normalized (if needed) when  finalizing checkout by calling [checkoutComplete](developer/api-reference/mutations/checkout-complete.mdx) mutation.
:::
...
The payment flow that uses a Saleor's payment gateway or payment synchronous webhooks to process a payment.
