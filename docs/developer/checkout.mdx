...
## Address Validation
The checkout's mutations that accept an address as an input have a field that can turn off the address validation. This allows assigning a partial or not fully valid address to the checkout. Providing country code is mandatory for all addresses regardless of the rules provided in this input.

The address [validation](developer/api-reference/inputs/checkout-address-validation-rules.mdx) input has two boolean fields:
- `checkRequiredFields` - Determines if an error should be raised when the provided address doesn't have all the required fields. Set to `true` by default.
- `checkFieldsFormat` - Determines if an error should be raised when the provided address doesn't match the expected format. Set to `true` by default.
...
#### checkoutCreate
The [`checkoutCreate`](developer/api-reference/mutations/checkout-create.mdx) mutation has an optional input for providing shipping and billing addresses. If you want to provide only a part of the address, you can disable the address validation.

The mutation accepts [validationRules](developer/api-reference/inputs/checkout-validation-rules.mdx) as an [input](developer/api-reference/inputs/checkout-create-input.mdx) field.
...
The [`checkoutShippingAddressUpdate`](developer/api-reference/mutations/checkout-shipping-address-update.mdx) mutation has an optional field for controlling the shipping address validation: [validationRules](developer/api-reference/inputs/checkout-address-validation-rules.mdx).
...
The [`checkoutBillingAddressUpdate`](developer/api-reference/mutations/checkout-billing-address-update.mdx) mutation has an optional field for controlling the billing address validation: [validationRules](developer/api-reference/inputs/checkout-address-validation-rules.mdx).
...
:::note
The shipping and billing addresses need to be valid, when finalizing checkout by calling [checkoutComplete](developer/api-reference/mutations/checkout-complete.mdx) mutation.
:::

:::note
The fields of shipping and billing addresses will be normalized (if needed) when completing the checkout by calling [checkoutComplete](developer/api-reference/mutations/checkout-complete.mdx) mutation.
:::
...
### Adding same product to multiple lines
...
:::
Saleor allows to force new line using `forceNewLine` flag. This allows to create multiple lines with same variant.
...
- [checkoutLinesAdd](developer/api-reference/mutations/checkout-lines-add.mdx)
...
If the variant is already added to the checkout and exist only in one line, Saleor will increase the corresponding line's quantity.
...
If the variant is already added to the checkout and exist in more than one line, Saleor will create new line with provided quantity.

:::

### Updating line

To modify line, use [checkoutLinesUpdate](developer/api-reference/mutations/checkout-lines-update.mdx).

Currently, [CheckoutLineUpdateInput](developer/api-reference/inputs/checkout-line-update-input) supports `variantId` and `lineId` parameters to operate on selected line object.
Since version `3.6` where [adding same product to multiple lines](checkout#adding-same-product-to-multiple-lines) feature was added we recommend use `lineId` parameter, `variantId` will be deprecated in the future.

:::caution

Is not allowed to use **variantId** and **lineId** in same line input.

If **variantId** parameter is used in line input and variant exists in multiple lines [checkoutLinesUpdate](developer/api-reference/mutations/checkout-lines-update.mdx) will return error - **lineId** should be used in that case.
...
- [checkoutLinesAdd](developer/api-reference/mutations/checkout-lines-add.mdx) - when adding
  a variant that has already been added to checkout, the previously existing line is overridden - the quantity is incremented and the price is updated.,
...
If user A completes their checkout within the given time, temporary allocation will be converted into a permanent one. However, if they don't complete their order within the time limit, their stock reservation will expire and the two items in their checkout will become available for other users to add to their own checkouts.
...
The payment flow uses a Saleor's payment gateway or payment synchronous webhooks to process a payment.

### Payment

Depending on the selected payment gateway, you will either use the payment provider's form which can be integrated with Saleor, or the payment gateway will direct you to an external payment page. The payment gateway returns information if the payment is successful, along with tokenized credit card payment information. This token is then used to run the [`checkoutPaymentCreate`](developer/api-reference/mutations/checkout-payment-create.mdx) mutation.
...
This feature is currently in a **Feature Preview** state. That means this part of Saleor is not yet complete
but is available to experiment with and provide feedback, and is subject to change.
...
Transaction stores details of payment transaction attached to order or checkout.


The [transactionCreate](developer/api-reference/mutations/transaction-create.mdx) mutation takes the following arguments:
...
The following example shows how the [transactionCreate](developer/api-reference/mutations/transaction-create.mdx) mutation 
is used to create a new transaction.
The transaction has status authorized, payment was made with a credit card. The actions that can be called from Saleor are: `VOID` and `CHARGE`. 
...
:::
The [transactionUpdate](developer/api-reference/mutations/transaction-update.mdx) mutation allows to update a details of transaction.
...
The following example shows how the [transactionUpdate](developer/api-reference/mutations/transaction-create.mdx) mutation 
...
:::note
During an update of transactions, all funds that go to a new state should be subtracted from the previous state. 
...
An action request is called when a staff user or an app triggers a request of an action that should be called for a given transaction.
...
sequenceDiagram
    ...
    App ->>+ Saleor: Call transactionUpdate <br>to update transaction with<br> a result recieved after <br>requested action
...
In order not to overload the database, unfinished checkouts are deleted.
