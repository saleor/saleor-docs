...
The pre-requirements for creating an order from checkout:
...
If these are satisfied, the checkout is transformed into an order, and the customer receives a confirmation email.
The operation requires permission `HANDLE_CHECKOUTS` and can be called only by App. Calling `checkoutPaymentCreate` and `checkoutComplete` is not needed for this approach. 
The created order can be marked as a paid by staff customer/app with `MANAGE_ORDERS` permission.

The below chart shows a proposed flow where this operation can be used.
```mermaid
sequenceDiagram
    Customer->>+Checkout App: Finalize an order
    Checkout App ->>+ Saleor: Create order from checkout
...
This feature was introduced in **Saleor 3.3**.
...
- `transactionEvent`: Data that defines a transaction event.
...
The transaction has status authorized, payment was made with a credit card. The actions that can be called from Saleor are: `VOID` and `CAPTURE`. 
...
The updated status of the transaction is `Captured`. The available action is REFUND. 
The authorized funds are captured, so amountAuthorized is 0. amountCaptured is 99USD
...
mutation {
  transactionUpdate(
    ...
    transaction: {
      ...
      amountCaptured: { currency: "USD", amount: 99 },
    }
    transactionEvent: {
      status: SUCCESS
      name: "Captured credit card"
      reference: "PSP-ref123.capture"
    }
  ) {
    transaction {
      id
