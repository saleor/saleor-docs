...
- `availableCollectionPoints`: a list of available places of self-pickup. It is calculated based on given country and available Stocks (for local Warehouse).

Please note, that selecting `collectionPoint` or `shippingMethod` is mandatory. In other words, setting `deliveryMethod` is mandatory.
