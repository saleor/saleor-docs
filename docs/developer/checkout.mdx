...
The version `3.6` featured [adding the same product to multiple lines](checkout#adding-same-product-to-multiple-lines). Since then, we recommend using the `lineId` parameter, as `variantId` will be deprecated in the future.

:::caution

Is not allowed to use `variantId` and `lineId` in the same line input.

If the `variantId` parameter is used in line input and the variant exists in multiple lines, [checkoutLinesUpdate](developer/api-reference/mutations/checkout-lines-update.mdx) will return an error. In that case, it is required to use `lineId`.
