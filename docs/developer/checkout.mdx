...
:::info
This feature is dedicated to [third-party](api-reference/enums/app-type-enum.mdx#code-style-fontweight-normal-apptypeenumbthirdpartybcode) apps.
:::
...
sequenceDiagram
  Front->>+Saleor: Fetch <br>checkout.availablePaymentGateways
	...
	Front ->>+ Saleor: TransactionInitialize<br>(like checkoutPymentCreate)<br> with amount and<br>payment gateway ID, data
  ...
  Saleor ->>+ PaymentApp: webhook transaction-initialize-session
	PaymentApp -->>- Saleor: mandatory data for init the frontend
	Saleor ->> Saleor: TransactionEvent <br>created/TransactionEvent<br>request updated
	Saleor -->>- Front: TransactionItem created
...
All details related to processing and managing payments can be found here: FIXME:

:::note
When the checkout is fully paid, the webhook CHECKOUT-FULLY-PAID will be triggered.
:::
