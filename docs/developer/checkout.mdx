...
- **Shipping Methods**: the way orders will be sent. E.g., DHL courier, postal service, self-pickup.
...
Running [checkoutLineDelete](developer/api-reference/mutations/checkout-line-delete.mdx) mutation will remove whole line, no matter the quantity:
...
Address can be assigned using [`checkoutShippingAddressUpdate`](developer/api-reference/mutations/checkout-shipping-address-update.mdx) mutation:
...
More information about address validation can be found on [address validation page](developer/address.mdx).
...
After choosing the shipping address, API can be queried for available methods in the user area:
...
Available payment gateways can be listed from `Checkout.availablePaymentGateways` field. Depending on the chosen gateway, you may need additional configuration. For example, public `api_key` used by frontend payment library. To obtain that data, run:
...
The purpose of the operation is to ensure this checkout is correct. The validation consist of checking:
