...
:::note
To avoid leaking precise stock information, the `quantityAvailable` field will not be greater than the maximum orderable threshold configured in the site settings.
:::

:::note
If `checkoutLineLimit` in the site settings is set to `null`, precise stock information will be exposed in the `quantityAvailable` field. What is more, if for given productVariant `trackInventory` is set to `false` and `checkoutLineLimit` is set to `null`, `quantityAvailable` field will also be `null`.
:::
## Stock quantities

For creating integrations with an external stock management system, you will need to use stock values not restricted by checkout line limit (can be set in Site Settings in dashboard or via `settingsUpdate` mutation).
