...
Warehouse is chosen based on argument `address`. If argument is ommited, API will return highest available quantity.
...
For creating integrations with external stock management system, you will need stock values not restricted by `MAX_CHECKOUT_LINE_QUANTITY`. 
...
To clarify difference between `quantityAvailable` and stocks, we will query both for the product variant: 
