...
## Multichannel and products

Customers can only fetch products from available channels. Because listing channels is only available for staff users, you will need to provide channel slugs to you storefront.
Besides availability, you can define more parameters on channel basis using [ProductChannelListing](developer/api-reference/objects/product-channel-listing.mdx):
- publicationDate
- isPublished
- visibleInListings
...
For variants theres analogue [ProductVariantChannelListing](developer/api-reference/objects/product-variant-channel-listing.mdx):
- price
- costPrice
- margin

[Learn more about multichannel](developer/channels.mdx).
...
`channel` argument can be skipped only if user has `is_staff` flag.
...
```graphql
{
  products(first: 2, channel: "default-channel", sortBy: {field: NAME}) {
    edges {
      node {
        id
        name
