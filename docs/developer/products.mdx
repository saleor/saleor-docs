...
To fetch a single product, use the [`product`](developer/api-reference/queries/product.mdx) query, which requires only one input field:
...
When using Saleor with tax plugins such as Vatlayer, you can fetch prices including taxes specific to a country. To do that, pass the `address` parameter in the `pricing` field, including the country code for which the tax should be calculated. The `address` parameter is available in the `Product.pricing` and `ProductVariant.pricing` fields.
...
Stock availability of products is defined for each variant and warehouse (see [Warehouses](/dashboard/configuration/warehouses.mdx) and [Checking inventory](/dashboard/catalog/products.md#checking-inventory) sections to see how to configure it in the Dashboard). There are two fields that allow you to fetch the stock information in the public API.
...
- the `quantityAvailable` is a sum of stock quantities from all available stocks,
  in this case, the sum is bigger than the `MAX_CHECKOUT_LINE_QUANTITY` so the maximum
  value is returned - 50 pcs available to buy
- `stocks` contain a full list of stock quantities in every of the warehouses

:::note
When the product has a track inventory turned off, the `quantityAvaible` will be always
equal to `MAX_CHECKOUT_LINE_QUANTITY` value.
:::
...
support provided address. When the address is not provided the highest quantity
...
As a response we get the quantity 20 as the `americas` is the only warehouse that is
available in the `US`.
...
The collection point warehouses are treated as a magic single-warehouse shipping zone.

- In the case of a local collection point warehouse, the calculated quantity will be equal
  to the quantity of this local warehouse.
- In the case of a global collection point warehouse the quantity will be equal to the sum
