...
Besides availability, you can define more parameters on a channel basis using [ProductChannelListing](api-reference/objects/product-channel-listing.mdx):
...
For variants, there's [ProductVariantChannelListing](api-reference/objects/product-variant-channel-listing.mdx):
...
The [`products`](api-reference/queries/products.mdx) query gives the ability to filter the results. Use the optional [`filter`](api-reference/inputs/product-filter-input.mdx) argument. Some of the filters that are available here are:
...
In [`products`](api-reference/queries/products.mdx) you can also sort the results using two [`sortBy`](api-reference/inputs/product-order.mdx) arguments:
...
To fetch a single product, use the [`product`](api-reference/queries/product.mdx) query, which requires only one input field:
...
:::note
`stocks` field in the [`ProductVariant`](api-reference/objects/product-variant.mdx) object require `MANAGE_PRODUCTS` permission.
:::
