...
To fetch the exact available quantity, use the `ProductVariant.quantityAvailable` field:
...
The `address` parameter behaves in the same way as in the `Product.isAvailable` fields above.
