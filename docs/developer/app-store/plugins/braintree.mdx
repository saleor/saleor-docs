...
sequenceDiagram
    ...
    Saleor ->> Braintree: Pass through unencrypted payment data*

    Braintree ->> Saleor: Pass unencrypted payment data* from Braintree
    Saleor ->> Saleor database: Update transaction model (non sensitive)
    Saleor ->> Saleor database: Save unencrypted payment data* from Braintree
    Saleor ->> User via storefront: Pass through unencrypted payment data* from Braintree
