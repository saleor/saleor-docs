...
sequenceDiagram
    ...
    User via storefront ->> Saleor: Pass unencrypted payment data*
    create participant Saleor database
    Saleor ->> Saleor database: Store internal transaction model (non sensitive)
    Saleor ->> Stripe: Pass through unencrypted payment data*

    Stripe ->> Saleor: Pass unencrypted payment data*
    Saleor ->> Saleor database: Update transaction model (non sensitive)
    Saleor ->> Saleor database: Save unencrypted payment data*
    Saleor ->> User via storefront: Pass through unencrypted payment data*
    ...
    User via storefront ->> Saleor: Pass encrypted sensitive payment data** and unencrypted payment data*
    Saleor ->> Saleor database: Store internal transaction model (non sensitive)
    Saleor ->> Stripe: Pass through encrypted sensitive payment data**  and unencrypted payment data*

    Stripe ->> Saleor: Pass unencrypted payment data* and encrypted payment data**
    Saleor ->> Saleor database: Update transaction model (non sensitive)
    Saleor ->> Saleor database: Save unencrypted payment data* and encrypted sensitive payment data**
    Saleor ->> User via storefront: Pass through unencrypted payment data* and encrypted sensitive payment data** 
  ```


*Passes non-PCI DSS regulated payment information, such as card type, last four digits of the card number and card expiration date.

**Passes encrypted credit card number and other encrypted information (cardholder data and sensitive authentication data), the decryption key is only accessible by Stripe, data is impossible for Saleor to read.
