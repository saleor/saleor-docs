...
sequenceDiagram
    ...
    Stripe ->> Saleor: Pass unencrypted payment data* with client secret from Stripe
    Saleor ->> Saleor database: Update transaction model (non sensitive)
    Saleor ->> Saleor database: Save unencrypted payment data* without client secret from Stripe
    Saleor ->> User via storefront: Pass through unencrypted payment data* with client secret from Stripe
