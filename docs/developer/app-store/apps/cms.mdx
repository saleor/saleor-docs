...
- The app considers [Product Variant](/api-reference/products/objects/product-variant.mdx) as a main synchronized entity. Every created item in the CMS will be a single, unique variant of the product.
- The app requires you to create a model in the CMS platform, but you can map Saleor product fields to the CMS fields.
- The app will synchronize product variants on the following [events](/developer/extending/webhooks/asynchronous-events.mdx):
  - `PRODUCT_UPDATED`
