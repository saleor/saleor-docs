...
- The app considers [Product Variant](https://docs.saleor.io/docs/3.x/api-reference/products/objects/product-variant) as a main synchronized entity. Every created item in the CMS will be a single, unique variant of the product.
- The app requires you to create a model in the CMS platform, but you can map Saleor product fields to the CMS fields.
- The app will synchronize product variants on the following [events](https://docs.saleor.io/docs/3.x/developer/extending/apps/asynchronous-webhooks):
  - `PRODUCT_UPDATED`
...
The app uses REST API, by default enabled in Payload. Typically, it's available under `/api` path.
You need to put this URL in the configuration form.

Then, read about [authorization in Payload](https://payloadcms.com/docs/authentication/config). It's not required but recommended to set up authorization between the app and Payload.

To do that, you need to create a user collection with `useAPIKey` field enabled. This will expose an infinite API key in Payload admin panel. You need to paste it to the "User API key" field in the config form.

The user collection used for authentication has a slug (set from the coded configuration). You need this slug to be set in the "Authenticated user slug" field in the config form.
...
The product variant collection slug must be set in the form as well - as a collection slug field.
...
Please leave the "Authenticated user slug" and "User API key" fields empty.
...
Warning - fields correctness is not validated. Ensure you set correct values.
