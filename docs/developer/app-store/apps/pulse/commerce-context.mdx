...
Whether the shopper allowed the storefront to read the storage that `marketing` and `session` come from.
...
| `not_required` | No shopper cookie (a till, an import) | `marketing` / `session` present when data existed |
| `unknown`      | Writer has no consent primitive       | any                                               |
