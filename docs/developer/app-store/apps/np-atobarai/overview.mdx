...
- Split payments are not supported. App works **only** if a single completed Atobarai transaction exists for the checkout/order. The app filters transactions by status, keeping only those with `CHARGE_SUCCESS` or `AUTHORIZATION_SUCCESS` events. If there are zero or more than one completed transactions, the app skips fulfillment reporting and adds an order note explaining the reason.
- The shipment has to be fulfilled using one of the supported local carriers.
- App uses [Japanese postal code data](https://www.post.japanpost.jp/zipcode/download.html) to fill city and city area (Oaza). If postal code includes more then one record is found in the database, app will pick the first one. You need to ensure you use valid billing address and pickup point during fulfillment.
