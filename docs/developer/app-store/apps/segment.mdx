...
- Use Segment [semantic e-commerce events](https://segment.com/docs/connections/spec/ecommerce/v2/#core-ordering-overview)

## Assumptions & limitations

- The app requires a [write key](https://segment.com/docs/connections/find-writekey/) to work.
- The app is not validating if the write key is correct due to missing API on the Segment side. If the write key is incorrect, app will not inform you about it.
...
| `ORDER_FULLY_PAID` | "Order Completed"  |
...
3. Name the source you want, e.g., "Saleor Orders."
4. Copy visible _Write Key_ and paste it into the app configuration.

## Configuration

The app requires configuring only a single field - a Segment Write Key.

### Testing

The easiest way to test if the app is configured is to perform following steps:
