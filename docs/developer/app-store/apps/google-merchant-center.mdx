...
- **Attribute mapping** — fill the `brand` and `gtin` columns from Saleor attributes.
...
Two Google columns have no Saleor equivalent and can be filled from an attribute:

- **Brand** → `brand`
- **GTIN** → `gtin`

Only product attributes are offered. A variant attribute value wins over a product attribute value with the same attribute. An unmapped column is left out of the file completely, rather than shipped empty.
...
- A missing translation falls back **per field**, not per product: a product with a translated name and an untranslated description publishes with both, rather than being dropped.
...
- A variant appears in a channel's feed only when it **has a price in that channel**. A variant that is not sold in the channel is left out silently and is not reported as a problem.
...
- **Configuration changes apply to the next run.** A run in flight keeps the language, URLs, title template, and attribute mapping it started with, because the earlier part of the file is already written. The exception is **Allow smaller feeds**, which is read when the run decides whether to publish.
  {/_ FLAG — NEEDS CONFIRMATION FROM THE APP TEAM. Claim: publication status is not respected.
  Evidence: the app's catalog walk is `products(first:, after:)` with no channel and no
  `where` filter (graphql/queries/fetch-products-core.graphql); the per-channel read is
  `products(channel:, where: {ids:})` (fetch-channel-pricing.graphql). Neither filters on
  `isPublished` or `visibleInListings`. In Saleor core, `ProductsQueryset.visible_to_user`
  (saleor/product/managers.py) returns every product assigned to the channel, unfiltered by
  publication, when the requestor holds MANAGE_PRODUCTS — and `ProductVariant.resolve_pricing`
  only requires both channel listings to exist. So an unpublished product with a variant price
  appears to be exported. Remove this bullet if the app filters somewhere I did not find. _/}

- **Publication status is not checked.** Any variant with a price in the channel is exported, including products that are unpublished or hidden from listings. Remove a variant's channel listing if it must not reach Google.

{/_ FLAG — CONFIRM. Claim: only `brand` and `gtin` can be mapped. Evidence:
`MAPPABLE_TSV_COLUMNS = ["brand", "gtin"]` in src/modules/feed/tsv-writer.ts, enforced by
the tRPC input schema in src/modules/config/config-form-mapping.ts. The Product Feed app's
extra mappings (color, material, pattern, size) have no counterpart here. _/}

- **Only `brand` and `gtin` are mappable.** Every other column is computed from catalog data. The `color`, `material`, `pattern`, and `size` mappings that the [Product Feed App](./product-feed.mdx) offered are not supported.
...
- **Attribute mappings for `color`, `material`, `pattern`, and `size` have no equivalent.** Only `brand` and `gtin` are mappable here.
