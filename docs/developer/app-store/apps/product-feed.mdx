...
- `Bucket Name`
- `Bucket Region`
- `Endpoint` - you can specify the endpoint URL here, for example: "https://s3.mywebsite.com". If left empty, DynamoDB defaults to AWS
- `Force path-style endpoint` - when true, force a path-style endpoint to be used where the bucket name is part of the path. [More on Path-style requests](https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html#path-style-access)
