...
:::warning
On Saleor Cloud, [Customer Emails](/developer/app-store/apps/customer-emails/overview.mdx) replaces this app for new setups. If you are self-hosting, this is the app that sends customer email.
:::
...
2. Create email templates using MJML and [Handlebars](https://handlebarsjs.com/). Additionally you can use a curated subset of [handlebars-helpers features](https://github.com/helpers/handlebars-helpers) — see [the list of allowed helpers](/developer/app-store/apps/smtp/configuration)
...
A message containing an account activation token after registration. To activate the account, your storefront should perform an API call according to [the documentation](/developer/users/customer-account.mdx#registration-with-email-confirmation).
...
Message sent after requesting a password change. The payload contains a token that has to be attached to the request to the Saleor API to change the password. To implement a storefront interface for password change, follow [the documentation](/developer/users/customer-account.mdx#resetting-the-password).
...
Message sent on the email address change request. Follow the [documentation](/developer/users/customer-account.mdx#changing-the-email-address) for more details.
...
A message with a token that's used for account deletion. To implement a storefront interface for account deletion, follow [the documentation](/developer/users/customer-account.mdx#deleting-the-account).
...
Saleor Cloud comes with the SMTP app pre-installed with a default SMTP server configuration. It can be used for testing purposes, enabling a quick start with Saleor without configuring an SMTP server.

This feature is not intended for production use and hardcodes both the template and Saleor domains into the email sender.

For production use, create your own SMTP server configuration and use it instead of the default one.
