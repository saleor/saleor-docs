...
:::tip
Remember that capture can expire after certain period of time if it is not extended. This depends on used payment method. For example, most credit cards authorizations expire after 28 days.
...
Expired payments can be re-authorized, although this somes with some risk. [See Adyen docs for more infomration](https://docs.adyen.com/account/manage-payments/#reauthorize-a-payment)
:::
...
- **`TransactionFlowStrategyEnum.CHARGE`** - Behavior of merchant account capture delay settings will be used, no change is made by app:
  ...
  - If capture delay is set to **manual**, authorization will be done insetad of a charge, which must be captuerd either in Saleor Dashboard or Adyen Dashboard
