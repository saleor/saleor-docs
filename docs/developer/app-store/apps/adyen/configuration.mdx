...
### Use customer IP address

The Adyen configuration form includes a **"Use customer IP address"** checkbox. When enabled, the app forwards the customer's IP address (received from Saleor's `customerIpAddress` field) to Adyen as the [`shopperIP`](https://docs.adyen.com/api-explorer/Checkout/71/post/sessions#request-shopperIP) field. Adyen can then use this IP for fraud detection and risk evaluation.

This setting is **enabled by default** for newly created configs. If your config was created before May 2026, you should revisit it and enable the setting.

Enabling this setting is **highly recommended**. Without it, there is a high risk of abuse, and Adyen's fraud detection will not work effectively.

:::caution
If your storefront uses a server-side proxy to call Saleor mutations, Adyen may receive the proxy's IP address instead of the actual customer's IP. This can cause fraud detection to incorrectly flag or block unrelated sessions that share the same IP. To avoid this, either ensure your proxy forwards the real customer IP in the `customerIpAddress` mutation field, or call the payment mutations directly from the storefront.

Read more about this in the [Transactions rejected due to fraud](./overview.mdx#transactions-rejected-due-to-fraud) section.
:::
