...
- App will return `CHARGE_ACTION_REQUIRED` or `AUTHORIZATION_ACTION_REQUIRED` as the transaction result when the response from Adyen to [initialize transaction session webhook](/developer/extending/webhooks/synchronous-events/transaction#initialize-transaction-session) or [process transaction session webhook](/developer/extending/webhooks/synchronous-events/transaction#process-transaction-session) webhooks does not include `pspReference` or `action`. This is because Saleor don't require `pspReference` in app response for this transaction result.
