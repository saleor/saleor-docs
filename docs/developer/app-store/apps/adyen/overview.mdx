...
### Broken channel-configuration mapping

App stores it's configuration assigned to specific Saleor channel.

If you change the assignment, all enqueued notifications (pending notifications from Adyen) that were related to such configuration will be not found by the app, causing the request to fail.

Before you change the configuration mapping, ensure that Adyen webhooks are not pending.
