...
Adyen app sends [`customerIpAddress`](/api-reference/payments/objects/transaction-initialize-session.mdx#customeripaddressstring) it receives from Saleor `TRANSACTION_INITIALIZE_SESSION` and `TRANSACTION_PROCESS_SESSION` to Adyen as [`shopperIP` field](https://docs.adyen.com/api-explorer/Checkout/71/post/sessions#request-shopperIP).
