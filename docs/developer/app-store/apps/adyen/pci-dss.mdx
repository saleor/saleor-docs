...
sequenceDiagram
    ...
    Adyen API ->> Adyen App: Provide non-ecrypted payment data and encrypted payment data
    Adyen App ->> Saleor: Pass non-ecrypted payment data and encrypted payment data  Adyen
    Saleor ->> Saleor database: Update transaction model (non sensitive)
    Saleor ->> User via storefront: Pass through non-ecrypted payment data and encrypted payment data 
