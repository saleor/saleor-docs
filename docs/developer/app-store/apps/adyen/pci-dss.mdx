...
sequenceDiagram
    ...
    Adyen API ->> Adyen App: Provide unencrypted payment data* and encrypted payment data**
    Adyen App ->> Saleor: Pass unencrypted payment data* and encrypted payment data** from Adyen
    Saleor ->> Saleor database: Update transaction model (non sensitive)
    Saleor ->> User via storefront: Pass through unencrypted payment data* and encrypted payment data** 
...
*Passes non-PCI DSS regulated payment information, such as customer ip, delivery address fields (street, city, state/province, postal code, country), additional data about freight amount, duty amount with total tax amount
**Passes encrypted credit card number and other information (cardholder data and sensitive authentication data), the decryption key is only accessible by Adyen, data is impossible for Saleor to read.
