...
## Can I change transaction payment method within single transaction?
...
At this point, the only way to complete checkout will be have “Allow unpaid orders” flag enabled.
...
But - if a customer (storefront) tries to complete the checkout before webhook arrives, it will fail. To ensure this flow is smooth, we recommend to integrate transactionProcessSession. This webhook will speed up resolving the flow and if needed - storefront can earlier show problem with payment.
