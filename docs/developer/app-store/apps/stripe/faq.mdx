...
App will always handle Stripe webhook, so _eventually_ status will be delivered anyway. If you rely on automatic checkout completion, this should be enough for you - you can show to the customer status from Stripe directly, and webhook will resolve the final state asynchronously.
