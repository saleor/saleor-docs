...
:::note
To configure the app, logged user must have HANDLE_PAYMENTS and MANAGE_APPS permissions
:::
...
:::warning
App will manage it’s webhooks. Do not disable or remove webhooks on Stripe side when the app is supposed to work - it will fail.

App will not prune it’s webhooks when uninstalled or disabled. If you want to remove the app, delete configurations first, or remove webhooks in Stripe manually
:::

App tries to detect if an event is coming from the removed app. If possible, app will try to remove such a webhook.
