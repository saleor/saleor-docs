...
It can set the following statuses on Saleor transaction in case of successful processing:
- `CHARGE_SUCCESS`
...
In some cases it can fail to process and return following data to Storefront
...
When app is being configured, it creates a webhook in Stripe. Webhook is registered for following events in Stripe:
