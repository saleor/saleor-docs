...
This webhook is executed when staff user wants to charge the payment that was authorized. It will call Stripe API to capture Payment Intent and update Saleor transaction with `CHARGE_SUCCESS` status when capture is successful or `CHARGE_FAILURE` when there is an error from Stripe API.
...
This webhook is executed when staff user wants to refund the payment that was charged. It will call Stripe API to create refund and return Stripe refund id. Actual status of refund will be set in app Stripe webhook when handling `charge.refund.updated` event based Stripe refund status:
