...
- This guide is assuming React-based UI. Follow Stripe documentation for implementation in other frameworks.
- This guide hardcodes currency to be USD. If you storefront implement multiple currencies, you need to ensure you properly convert the amount to cents and use Stripe currency format.
...
In this guide we will render a part of UI that will have "PAY" button rendered and working.
...
Prepare a following GraphQL mutation:
...
- "PAY" button visible, but doing nothing once clicked.

# Submitting payment

Next step is to create the payment intent using Stripe SDK and Saleor Transaction mutations together. First, let's create the mutation query:
...
Next step will be allowing Stripe to confirm the payment. Depending on the Payment method, Stripe may require few steps, including redirecting to the dedicated external pages.
Hence we need to generate the `return_url` param, telling Stripe where to redirect after the payment is completed.

The redirection URL may be specific to your storefront, but let's prepare some example:
...
However, we don't know if Stripe has told Saleor about the payment yet! Let's have a quick recap of the flow: Stripe will send webhook with the payment status to Saleor.
But, this is an async operation - so we can't be sure if the webhook has been received yet.

If storefront (automatically, or by user action) calls `checkoutComplete` too early, Saleor will not allow to perform the operation. Only `SUCCESS` or `PROCESSING` statuses are allowed to complete the checkout.
...
With this mutation, App will fetch latest Payment Intent status from Stripe and update Saleor about it. We can't be sure if ProcessSession mutation was before or after Stripe Webhook - but it's not a problem.
Saleor will ignore the event if reported second time.

If payment has failed before, your storefront should handle error (either based on direct Stripe Payment Intent status, or Saleor transaction status) and display it to the user - probably asking to retry the payment.
...
Saleor will automatically generate Idempotency Key and the App will pass it to Stripe. If you want to control the Key usage, like setting your own, you can pass it to [the mutation](../../../../api-reference/payments/mutations/transaction-initialize)