...
You may want to migrate from the [Stripe Plugin](../../plugins/stripe) to the App. This document will help you with that.
...
The `paymentData` and `redirectUrl` input should not be used anymore, to transfer data to the App, use `data` fields on the transaction webhooks.
