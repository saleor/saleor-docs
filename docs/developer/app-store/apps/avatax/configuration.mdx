...
- _Automatically distributed discounts_ for lines containing products
- _Price reduction discounts_ for shipping line
...
When the _Tax exemption status metadata_ option is enabled in the AvaTax App configuration, the app will update the checkout and order public metadata entry with key `avataxExemptionStatus`.
