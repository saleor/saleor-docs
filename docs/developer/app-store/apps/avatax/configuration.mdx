...
You can override the `shipFrom` address per checkout or order by setting **private metadata** with key `avataxShipFromAddress`. The value must be a JSON string with the following fields:
