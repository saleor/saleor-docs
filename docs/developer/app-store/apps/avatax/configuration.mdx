...
- [Tax exemption status metadata](#tax-exemption-status-metadata) - a flag that indicates whether the app should write information about applied tax exemption to checkout public metadata.
...
When enabled, the AvaTax App will write information about tax exemption applied by AvaTax to the checkout **public metadata**, under the `avataxExemptionStatus` key.
...
When the *Tax exemption status metadata* option is enabled in the AvaTax App configuration, the app will update the checkout public metadata entry with key `avataxExemptionStatus`.
...
To read it from Saleor, you can query the checkout `metafield`:
