...
- Initial index of existing customers stored in Saleor and sending them to the provider contact list
- Creating a new contact in the provider, when a new Customer is created or updated in Saleor
...
- Have `MANAGE_APPS` permission

:::

## Installation

The App can be installed from Saleor App Store. To install the App, you must have `MANAGE_USERS` and `MANAGE_APPS` permissions.

## Configuration

The App is configured per provider. You can configure more than one provider - the App will work with at least one provider set up

### Mailchimp configuration

#### Authorization

App uses Mailchimp OAuth flow, meaning it will ask you to log in with Mailchimp and connect the App with your Saleor account. You can always disconnect the App from Mailchimp dashboard.

After successful authorization, App will store a token used to connect with a Mailchimp API.
...
The App will set a customer in Mailchimp when customer is created (`CUSTOMER_CREATED` event) and updated (`CCUSTOMER_UPDATED` event).

#### Customers bulk sync

If you install the App and already have previously created customers, you would likely want to perform initial index of customers and send the to the provider.

You can do that from the App.

:::caution

Customers synchronization **works on the frontend side**, that means App **must remain opened** during this operation.
...
App will create contacts with "Non-subscribed" status (`transactional` in the API).

#### Built-in fields

App will automatically map the customer email (mandatory), first (`FNAME`) and last (`LNAME`) names (optional)

#### Tags

Your campaigns likely rely on segments based on custom fields and tags.
We probably need consent to run marketing for each user.

The App allows this workflow via tags in the customer's [PrivateMetadata](https://docs.saleor.io/docs/3.x/developer/metadata)

The App will parse metadata items with the key `mailchimp_tags` and extract tags that will be added to each contact.
These tags can be used to create segments and bulk-modify customers in Mailchimp.

Tags must be a form of **array of strings**, for example `["tag 1", "tag 2"]`.
