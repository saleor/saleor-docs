...
<AppMetadata
  ...
  githubUrl="https://github.com/saleor/example-app-crm"
/>
...
The App can be installed from [Saleor App Store](developer/app-store/apps/overview.mdx). To install the App, you must have `MANAGE_USERS` and `MANAGE_APPS` [permissions](developer/permissions.mdx#app-permissions).
...
One way to model this with Saleor is using [PrivateMetadata](api-usage/metadata.mdx) for each customer.
...
1. Follow the [_Self-hosting Saleor Apps_ article](/developer/extending/apps/developing-apps/self-hosting-saleor-apps).
