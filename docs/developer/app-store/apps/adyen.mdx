...
To use some payment methods inside Adyen Drop-in you might have to implement the following events:
...
Adyen's orders and requests for balance check are not saved in Saleor, each payment linked to the Adyen order will be stored as a separate transaction in Saleor.

Orders link transactions on Adyen level. For example if the user cancels the order (by removing a gift card payment inside drop-in)
every payment linked to that order will be refunded or voided.

:::

#### onBalanceCheck

To make a request to `/paymentMethods/balance` endpoint use `paymentGatewayInitialize` mutation:

```graphql
mutation SendDataToApp($checkoutId: ID!, $data: JSON) {
  paymentGatewayInitialize(
...
```json
{
  "action": "checkBalance",
  "paymentMethod": {
    /** The following properties are passed from Drop-in: */
    "type": "giftcard",
    "brand": "givex",
    "encryptedCardNumber": "...",
...
the `onBalanceCheck` implementation could look like this (depending on your GrpahQL client):

```ts
    async onBalanceCheck(resolve, reject, data) {
      const {
        paymentGatewayInitialize: { gatewayConfigs },
      } = await client.request(BalanceCheck, {
        checkoutId,
...
the response received from Saleor with data from Adyen app will look like this:
...
To make a request to `/orders` endpoint use `paymentGatewayInitialize` mutation and pass the following `data`:
...
the `onOrderCreate` implementation could look like this (depending on your GrpahQL client):

```ts
    async onOrderCreate(resolve, reject, data) {
      const {
        paymentGatewayInitialize: { gatewayConfigs },
      } = await client.request(CreateOrder, {
        checkoutId,
...
the response received from Saleor with data from `/orders` endpoint will look like this:
...
Once the order is created you can complete the payment. Pass the data received from drop-in in `onSubmit` method.
It will include `order` property - Adyen App will charge the customer with the amount specified in order's data.

For example: if you have only 20 EUR left on gift card, and the order's total amount is 50 EUR, `order` data will firstly charge the gift card with available amount (20 EUR) and the 2nd payment method with amount left inside the order (50 - 20 = 30 EUR).

#### onOrderCancel

This endpoint is called once the user removes a payment method in pending Adyen order (e.g. there was already a partial charge for gift card, but user decides to use a different payment method).

To make a request to `/orders/cancel` endpoint use `transactionProcess` mutation:

```graphql
mutation CancelOrder($id: ID!, $data: JSON!) {
...
The response from that mutation will look like this:
