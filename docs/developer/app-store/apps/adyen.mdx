...
You can install Adyen app directly from your Saleor Dashboard. Go to **Apps** section and click **Install** button next to Adyen app. Follow the instructions on the screen. After the installation is complete, select Adyen app from the list of installed applications.

#### Using Adyen API credentials

To configure Adyen application you first need to configure Adyen.

#### Creating new API Credentials

To create new Adyen API credentials over to Adyen dashboard, then **Developers -> API credentials**. Click **Create new credential** and choose **Web service user**.
...
Go back to the Adyen app configuration in Saleor Dashboard. Provide configuration name that you will be familiar with. In **API Key** field provide key from **Server settings -> Authentication** section. Select **TEST** environment.

:::caution
The configuration will not work until you save API credentials in Adyen dashboard.
:::

Save API credentials in Adyen dashboard, head over to Adyen app configuration and click **Save**. Rest of the form fields have been enabled.
...
In Adyen dashboard head over to **Developers -> Webhooks**. Click **+ Webhook** button and choose **Standard webhook**. Provide description for the webook.
...
In **Events**, apart of events selected by default, select `EXPIRE`. Deselect `ORDER_CLOSED` and `ORDER_OPENED` events.
...
Generate new **HMAC key** in **Security -> HMAC Key** section and copy it.
