...
```
:::note
The command `python manage.py populatedb --createsuperuser` will fill the database with sample data. It is NOT supposed to be executed in a production environment!
:::

Both commands create an admin account with the email: `admin@example.com` and password: `admin`.

Once the Saleor Dashbord app has been created on Heroku, you can log in via the URL `<Heroku-app name>.herokuapp.com` using the admin account.
