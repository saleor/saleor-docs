...
The RSA private key in PEM format is used for signing Saleor's JWT tokens. In the DEBUG mode, this is not necessary as Saleor will generate a temporary private key. It is required in production environments. 
generate a temporary private key. Required in production environments. 

### `RSA_PRIVATE_PASSWORD`

The password required for loading `RSA_PRIVATE_KEY`. Default set to `None`.
