...
Controls the time which defines how long the JWT authentication token will be valid. The value should be provided in seconds. Defaults to 5 minutes.


### `JWT_REFRESH_EXPIRATION_DELTA`

Controls the time which defines how long the JWT refresh token will be valid. The value should be provided in seconds. Defaults to 30 days.

### `REQUEST_EMAIL_CHANGE_TOKEN_EXPIRATION_DELTA`

Controls the time which defines how long the token for changing an email will be valid. The value should be provided in seconds. Defaults to 1 hour.

### `JWT_SECRET`

Controls the key used to encode/decode JWT token. Defaults to [`SECRET_KEY`.](developer/running-saleor/configuration.mdx#SECRET_KEY)

### `JWT_EXPIRE`

Controls the key used to determine if authentication tokens should expire or not. Defaults to `False`.
