...
The RSA private key in PEM format that is required for signing Saleor's JWT tokens. For local development, when `DEBUG==True` this is not required as Saleor will 
