...
Products in Saleor have the `minimal_variant_price` field, which is used to store the currently lowest price of a product, taking into account variant prices (which may be different than the base product price) and active sales. Minimal variant price are useful when you want to [sort the products by price](developer/products.mdx#sorting).

Recalculating this value for all products may be time-consuming if there is a lot of products in the database. To do it in the background, run the following command:
