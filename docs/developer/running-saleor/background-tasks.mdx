...
## Updating minimal products’ prices

Every product in Saleor has a field `minimal_variant_price`, which is used to store the currently lowest price of a product. This field takes into account prices of all products’ variants (since variants may have different prices) and active sales. Recalculating this value for all products may be time-consuming if there is a lot of products in the database. To do it in the background, run the following command:
