...
The warehouses, shipping zones, and channels must be properly linked together
...
- channel - allow configuring your shop differently for different currencies;
  go [here](./channels.mdx) for more information
- shipping zone - allow for defining the cost of shipping in different areas of the world,
  shipping zone contains shipping methods that can be used for a given part of the world
- warehouse - contains stocks of specific products, assigned to a given channel and shipping zone,
  can be defined as the place for click and collect
...
The relations between the mentioned entities are shown in the below chart:
...
Stocks are allocated in the checkout completing process, for all lines
...
available quantity in one stock, multiple allocations are created.
...
Firstly we need to create the warehouse with use of
[`WarehouseCreate`](developer/api-reference/mutations/warehouse-create.mdx) mutation.
...
the [`WarehouseUpdate`](developer/api-reference/mutations/warehouse-update.mdx) mutation.
