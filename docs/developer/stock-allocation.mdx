...
- `all warehouses` - the global stock for a given product is used during the fulfillment
  process, the available quantity is not restricted by the quantity of this warehouse,
  and ordered products can be shipped from a different warehouse
...
In the case of checkout with the local collection point, the stock allocation can be made
only from this warehouse. If there is not enough quantity, the `InsufficientStock` error
is raised. In the case of checkout with the global collection point, the stocks are
allocated from the chosen warehouse in the first place, when there is not enough quantity
the stocks are allocated from the next warehouse with the biggest available quantity value.
