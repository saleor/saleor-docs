...
information can be queried publicly in API. Otherwise, it requires permission.

### Click-and-collect warehouses

You can define the warehouse as a collection point with the `click and collect` option.
...
only from the current warehouse. If there is not enough quantity, the `InsufficientStock` error
is raised. In the case of the checkout with the global collection point, the stocks
