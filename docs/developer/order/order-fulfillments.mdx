...
- Automatically after checkout completion (if [`channel.orderSettings.automaticallyConfirmAllNewOrders`](/api-reference/channels/objects/channel#channelordersettingsautomaticconfirmallnewordersboolean---) is enabled)
...
Here's an example of updating tracking numer for fulfillment:
