...
- Automatically after checkout completion (if [`channel.orderSettings.automaticallyConfirmAllNewOrders`](/api-reference/miscellaneous/objects/order-settings.mdx#automatically-confirm-all-new-orders) is enabled)
- Manually through the [`orderConfirm`](/api-reference/orders/mutations/order-confirm.mdx) mutation
...
[`FulfillmentStatus Enum`](/api-reference/orders/enums/fulfillment-status.mdx) represents all the possible states fulfillment can be in.
...
- **WAITING_FOR_APPROVAL** → The fulfillment is created but awaits approval before being processed.
  ...
  [`shop.fulfillmentAutoApprove`](/api-reference/miscellaneous/objects/shop.mdx#fulfillment-auto-approve).
...
- Running [`orderFulfillmentApprove`](/api-reference/orders/mutations/order-fulfillment-approve.mdx) changes status to **FULFILLED**.
- Running [`orderFulfillmentCancel`](/api-reference/orders/mutations/order-fulfillment-cancel.mdx) **removes the fulfillment**.

**If fulfillment is in FULFILLED**:

- Running [`orderFulfillmentCancel`](/api-reference/orders/mutations/order-fulfillment-cancel.mdx) changes status to **CANCELED**.
- Running [`orderFulfillmentReturnProducts`](/api-reference/orders/mutations/order-fulfillment-return-products.mdx) changes status based on the request:
  - **RETURNED** → If only a return was requested.
  - **REPLACED** → If a replacement was requested.
- Running [`orderFulfillmentRefundProducts`](/api-reference/orders/mutations/order-fulfillment-refund-products.mdx) changes status based on the request:
...
- [`OrderLine.quantityFulfilled`](/api-reference/orders/objects/order-line.mdx#quantity-fulfilled) - The number of items that have been fulfilled
- [`OrderLine.quantityToFulfill`](/api-reference/orders/objects/order-line.mdx#quantity-to-fulfill) - The number of items still pending fulfillment
...
The impact of fulfillments on stock depends on whether inventory tracking is enabled for the product variant ([`ProductVariant.trackInventory`](/api-reference/products/objects/product-variant.mdx#track-inventory)).
...
- The `allowStockToBeExceeded` parameter in the [`orderFulfill`](/api-reference/orders/mutations/order-fulfill.mdx) mutation allows creating fulfillments even when stock is insufficient
...
Fulfillments are created using the GraphQL mutation [`orderFulfill`](/api-reference/orders/mutations/order-fulfill.mdx). This operation allows you to process and ship items from your warehouses to customers.
...
- `trackingNumber` - The shipping tracking number
  ...
  - Can be updated later using the [`orderFulfillmentUpdateTracking`](/api-reference/orders/mutations/order-fulfillment-update-tracking.mdx) mutation
...
```json
{
  "id": "T3JkZXI6OTE3Yjc2NDQtY2Q0Zi00ZjcyLTkzNjktMGNhYTk4ODEyNDQy",
  "input": {
    "lines": [
      {
        "orderLineId": "T3JkZXJMaW5lOjM1YzEwNjNkLTYyNjQtNGExMi1hYzBlLWRhMzg1ZDM3ZGRhNA==",
        "stocks": [
          {
            "quantity": 2,
      ...
      {
        "orderLineId": "T3JkZXJMaW5lOmJmYzQzMDg2LTlkM2ItNGM2MS1hMGJkLTRkNGE2YmIyNWZiNw==",
        "stocks": [
          {
            "quantity": 3,
            "warehouse": "V2FyZWhvdXNlOjc1MjYwYWRjLTJjZjAtNGQ0ZC1hOTM5LTBmZGY2Y2FlYjBjMQ=="
...
Fulfillments can be canceled using the [`orderFulfillmentCancel`](/api-reference/orders/mutations/order-fulfillment-cancel.mdx) mutation. This operation is useful for handling shipping errors, customer cancellations, or when items need to be restocked.
...
Fulfillments can be approved using the [`orderFulfillmentApprove`](/api-reference/orders/mutations/order-fulfillment-approve.mdx) mutation. This operation is used when approval is required before processing a fulfillment.
...
The [`orderFulfillmentUpdateTracking`](/api-reference/orders/mutations/order-fulfillment-update-tracking.mdx) mutation allows you to update or add a tracking number to an existing fulfillment. This is useful when:
...
The [`orderFulfillmentReturnProducts`](/api-reference/orders/mutations/order-fulfillment-return-products.mdx)
...
3. **Return and Refund**
   ...
   For more details on how to handle refunds for payment created with apps, refer to the [Refunds documentation](/developer/payments/refunds.mdx).
   :::

   - Case: Processing return and refund simultaneously
   - Process:
     - Set `refund: true` in [`orderFulfillmentReturnProducts`](/api-reference/orders/mutations/order-fulfillment-return-products.mdx) to trigger automatic refund
...
:::warning
For orders paid via apps or custom integrations, follow the [Refunds documentation](/developer/payments/refunds.mdx).

For orders paid using payment plugins, use [`orderFulfillmentRefundProducts`](/api-reference/orders/mutations/order-fulfillment-refund-products.mdx) or [`orderRefund`](/api-reference/deprecated/orders/mutations/order-refund.mdx).
:::
...
1. **Refund only**
   ...
   - Plugin-based: Use [`orderFulfillmentRefundProducts`](/api-reference/orders/mutations/order-fulfillment-refund-products.mdx) to refund specific fulfilled lines
...
2. **Miscellaneous Refund**
   ...
   - Fulfillment: Already shipped
   - Plugin-based: Use [`orderRefund`](/api-reference/deprecated/orders/mutations/order-refund.mdx) to refund a custom amount (e.g., shipping costs)

3. **Cancel**
   - When: The customer cancels part of the order before it ships
   - Fulfillment: Not shipped
   - Plugin-based: Use [`orderFulfillmentRefundProducts`](/api-reference/orders/mutations/order-fulfillment-refund-products.mdx) for unfulfilled lines
...
[`Shop.fulfillmentAllowUnpaid`](/api-reference/miscellaneous/objects/shop.mdx#fulfillment-allow-unpaid)
...
[`OrderSettings.automaticallyFulfillNonShippableGiftCard`](/api-reference/miscellaneous/objects/order-settings.mdx#automatically-fulfill-non-shippable-gift-card)
...
The following explains the interaction between [`automaticallyFulfillNonShippableGiftCard`](/api-reference/miscellaneous/objects/order-settings.mdx#automatically-fulfill-non-shippable-gift-card) and [`fulfillmentAllowUnpaid`](/api-reference/miscellaneous/objects/shop.mdx#fulfillment-allow-unpaid) when:
...
The interaction between [`automaticallyFulfillNonShippableGiftCard`](/api-reference/miscellaneous/objects/order-settings.mdx#automatically-fulfill-non-shippable-gift-card) and [`fulfillmentAutoApprove`](/api-reference/miscellaneous/objects/shop.mdx#fulfillment-auto-approve) determines whether gift card fulfillments require manual approval:
