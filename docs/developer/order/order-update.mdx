...
| Confirmed orders (`UNFULFILLED`, `FULFILLED`, `RETURNED`) | Addresses, email, external references, language, metadata |
...
Order notes are stored in the order's events and will be displayed in the Saleor Dashboard's Order History section. This feature is useful for:
