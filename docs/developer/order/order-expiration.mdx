...
To set the expiration of orders, specify the number of minutes after which the orders should expire by setting [`expireOrdersAfter`](/api-reference/miscellaneous/objects/order-settings.mdx#expire-orders-after).
Additionally, the [`deleteExpiredOrdersAfter`](/api-reference/miscellaneous/objects/order-settings.mdx#delete-expired-orders-after) value can be modified. By default, this is set to 60 days, but it can be changed to anywhere between 1 and 120 days.
