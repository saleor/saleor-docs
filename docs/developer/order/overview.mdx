...
The following information can be accessed without authentication if you know the order id:

- **Basic details**: ID, number, status, created/updated timestamps, channel
- **Contents**: lines, discounts, gift cards, totals (subtotal, shipping, undiscounted total, total, balance)
- **Addresses & contact**: shipping and billing addresses, customer email, customer note
- **Delivery & fulfillment**: delivery method, shipping method name, collection point, fulfillments
...
- **User**: returned only to the authenticated owner or staff/app with `MANAGE_ORDERS`
- **Transactions**: require `MANAGE_ORDERS` and `HANDLE_PAYMENTS`
- **Events and invoices**: require `MANAGE_ORDERS`
- **Private metadata**: only staff with `MANAGE_ORDERS`
