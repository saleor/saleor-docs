...
within the configured minimum and maximum price range.
- **Weight-based**: These methods are available when the order's total weight falls 
within the configured minimum and maximum weight range.
...
:::tip
For details on using webhooks to integrate external shipping methods, 
see the [Shipping webhooks](https://docs.saleor.io/developer/extending/webhooks/synchronous-events/shipping.mdx) 
guide. Additionally, information on how shipping methods are listed 
and used during the checkout process can be found in the 
[Listing available shipping methods](https://docs.saleor.io/developer/checkout/address#listing-available-shipping-methods.mdx)
 in checkout guide.
...
2. Make the shipping method available in channels using [`shippingMethodChannelListingUpdate`](/api-reference/shipping/mutations/shipping-method-channel-listing-update.mdx):
...
1. First, set the default weight unit for your store. By default, it is set to KG, so if you operate using a different unit (e.g., LB), make sure to update it accordingly.
...
2. Use the [`shippingPriceCreate`](/api-reference/shipping/mutations/shipping-price-create) 
...
3. Make the shipping method available in channels using [`shippingMethodChannelListingUpdate`](/api-reference/shipping/mutations/shipping-method-channel-listing-update.mdx):
```graphql {2-8}
mutation {
  shippingMethodChannelListingUpdate(
...
mutation {
  shippingPriceUpdate(
    ...
    input: {addPostalCodeRules: [{end: "", start: "83-330"}], 
...
- Use [`shippingPriceExcludeProducts`](/api-reference/shipping/mutations/shipping-price-exclude-products.mdx) to add products to the exclusion list
- Use [`shippingPriceRemoveProductFromExclude`](/api-reference/shipping/mutations/shipping-price-remove-product-from-exclude.mdx) to remove products from the exclusion list
...
You can assign a tax class to shipping methods. For more information, see the [taxes guide](/developer/taxes.mdx#assigning-tax-rate-to-a-shipping-method).
...
- [`shippingPriceUpdate`](/api-reference/shipping/mutations/shipping-price-update.mdx) Use this mutation for general updates to the shipping method's name, delivery days, etc.
- [`shippingMethodChannelListingUpdate`](/api-reference/shipping/mutations/shipping-method-channel-listing-update.mdx) Use this mutation to adjust channel-specific settings like price and order total range.
- [`shippingPriceTranslate`](/api-reference/miscellaneous/inputs/shipping-price-translation-input.mdx) Use this mutation to provide translations for the shipping method's name and description in different languages.

### Metadata Management

To manage shipping rate metadata, use:
- [`updateMetadata`](/api-reference/miscellaneous/mutations/update-metadata.mdx)
- [`updatePrivateMetadata`](/api-reference/miscellaneous/mutations/update-private-metadata)
