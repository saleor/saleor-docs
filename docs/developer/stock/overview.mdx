...
:::info
The role of shipping zones in stock availability depends on the 
...
setting. See [Stock availability mode](#stock-availability-mode) for details.
:::
...
:::info
This step is only required when [`useLegacyShippingZoneStockAvailability`](/api-reference/miscellaneous/objects/shop.mdx#use-legacy-shipping-zone-stock-availability) 
is enabled. When disabled, stock availability is determined by the warehouse-channel link alone, 
and shipping zone assignment is not needed for stock purposes.
:::
...
Saleor supports two modes for determining which warehouses contribute to stock availability, controlled by the 
[`useLegacyShippingZoneStockAvailability`](/api-reference/miscellaneous/objects/shop.mdx#use-legacy-shipping-zone-stock-availability) 
setting on `Shop`. You can change this setting via the 
[`shopSettingsUpdate`](/api-reference/shop/mutations/shop-settings-update.mdx) mutation.

### Legacy mode (enabled by default for existing instances)

When `useLegacyShippingZoneStockAvailability` is `true`, stock availability is filtered by **shipping zones** 
and the customer's **destination address**. A warehouse's stock counts toward availability only if:
...
### Simplified mode (default for new instances)

When `useLegacyShippingZoneStockAvailability` is `false`, stock availability is determined solely by the 
**direct warehouse-channel link**. Shipping zones and the customer's destination address are ignored 
when calculating stock. A warehouse's stock counts toward availability if:

- The warehouse is assigned to the sales channel.

This simplifies the stock setup — you no longer need to configure shipping zones to make stock available in a channel.

#### Deprecated arguments

When simplified mode is active, the `address` argument on the following fields is ignored and deprecated:

- [`Product.isAvailable`](/api-reference/products/objects/product.mdx#is-available)
- [`ProductVariant.quantityAvailable`](/api-reference/products/objects/product-variant.mdx#quantity-available)
- [`ProductVariant.stocks`](/api-reference/products/objects/product-variant.mdx#stocks)
