...
determine the applicable taxes. Read more about [calculating taxes for click and collect](developer/taxes#example-for-click-and-collect-shipping).
...
### Warehouse Update
Once created, following warehouse fields can be updated:
- name
- slug
- email
- address
- externalReference
- isPrivate (to control visibility)
- clickAndCollectOption

:::info
Public warehouse allows you to query the stock information publicly in the API. 

Private warehouse requires permission.
:::
...
To manage warehouse metadata, use [`updateMetadata`](/api-reference/miscellaneous/mutations/update-metadata) 
or [`updatePrivateMetadata`](/api-reference/miscellaneous/mutations/update-private-metadata). 
...
To assign warehouse to shipping zone follow [these instructions](/developer/shipping/overview#assigning-warehouses-to-the-shipping-zone)
...
5. Click Save Shipping Zone.

**API**

Use the [`shippingZoneUpdate`](/api-reference/shipping/mutations/shipping-zone-update) 
...
`mutation {
  shippingZoneUpdate(
    ...
    input: {
      addWarehouses: [
        "V2FyZWhvdXNlOjJjYjY1NTk2LTRiOTEtNGRkYS1iMWY4LTA3ZjczOTFiNGMxZA=="
      ]
    }
  ) {
    shippingZone {
...
## Stocks
Stocks represent the inventory relationship between warehouses and product variants, tracking the quantity of each variant available at specific locations. This relationship is crucial for inventory management, order fulfillment, and stock level monitoring.
...
#### Get All Stocks
You can use the [`stocks`](/api-reference/products/queries/stocks) query to get all stocks or apply a search filter to get only stocks for variants or a warehouse with a specific name.
...
#### Get Single Stock
To retrieve detailed information about a specific [`stock`](/api-reference/products/queries/stock) entry:
...
To view all stock entries for a specific [`warehouse`](/api-reference/products/queries/warehouse), including detailed product information and quantity:

```graphql
query WarehouseStocks{
  warehouse(id: "V2FyZWhvdXNlOjI1YzdlZDNlLTljYjQtNDUzNi04ZGNjLTgzZTdlNDIzZTNjMQ=="){
    stocks(first:10){
      totalCount
      edges{
        node{
