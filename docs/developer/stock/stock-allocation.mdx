...
to provide the expected allocation.

:::info
The allocation conditions described below apply when 
...
is enabled (legacy mode). When disabled, shipping zones are not considered — stocks are allocated 
from any warehouse assigned to the channel. See [Stock availability mode](/developer/stock/overview.mdx#stock-availability-mode) for details.
:::

Below you can find the explanation of the terms that will be required to understand this guide properly:
...
- **Legacy mode only:** the stock's warehouse has a shipping zone that includes the shipping address
...
When [`useLegacyShippingZoneStockAvailability`](/api-reference/miscellaneous/objects/shop.mdx#use-legacy-shipping-zone-stock-availability) 
is disabled, the shipping zone condition is skipped — allocation considers all warehouses 
assigned to the channel regardless of the customer's address.
