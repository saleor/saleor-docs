...
The warehouses and channels must be linked to provide the expected allocation. 
...
- warehouse - contains stocks of specific products, assigned to a given channel,
...
is enabled, an additional condition applies: the stock's warehouse must have a shipping zone 
that includes the shipping address (or billing address if the shipping address is not set) country code, 
and this shipping zone must be available in the channel in which the order is created. 
See [Legacy Stock Availability](/developer/stock/overview.mdx#legacy-stock-availability) for details.
:::
