...
- If `trackInventory: true`, Saleor reduces and validates stock on checkout 
or draft order, and blocks sales when stock runs out.
- If `trackInventory: false`, Saleor will not validate or reduce inventory — 
useful when stock is managed in an external system.
