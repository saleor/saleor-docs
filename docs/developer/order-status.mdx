...
- Expire: the status will automatically change to `EXPIRED` if the order is older than [`channel.orderSettings.expireOrdersAfter`](../api-reference/miscellaneous/objects/order-settings#ordersettingsexpireordersafterminute-) minutes. This will happen automatically only if no transaction attempts are made.
