...
- `paymentGateways` - A list of payment gateways to initialize. If not provided, all apps subscribed to the webhook [PAYMENT_GATEWAY_INITIALIZE_SESSION](api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#code-style-fontweight-normal-webhookeventtypesyncenumbtransaction_initialize_sessionbcode) will be used.
