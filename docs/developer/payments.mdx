...
The below process describes the key milestones in the payment flow in Saleor.
...
- By using a [Saleor Payment App](extending/overview#apps-recommended)
- By using a [Saleor App with existing Checkout App](extending/overview#apps-recommended).
...
- `paymentGateways` - A list of payment gateways to initialize. If not provided, all apps subscribed to the webhook `PAYMENT_GATEWAY_INITIALIZE_SESSION` will be used.
...
The mutation will trigger the `PAYMENT_GATEWAY_INITIALIZE_SESSION` webhook. For more details about this webhook, please refer to its documentation. FIXME:

### Initialize transaction

To initiate payment processing, call the `transactionInitialize` mutation. The `data` provided in the `paymentGateway` field will be sent to the payment app, and Saleor will return the `data` received from the app as a response. Based on the Payment App's response, Saleor will create or update the transaction with the appropriate status.

The `transactionInitialize` mutation requires the following arguments:
...
The mutation will trigger the `TRANSACTION_INITIALIZE_SESSION` webhook. For more details about this webhook, please refer to its documentation.
...
- `AUTHORIZATION_ACTION_REQUIRED` or `CHARGE_ACTION_REQUIRED` indicates that additional action is required from the
  customer to finalize the payment. Calling `transactionProcess` will be required to pass the result of the action to the app.
...
If `transactionInitialize` or `transactionProcess` returns `transactionEvent.type` equal to `AUTHORIZATION_ACTION_REQUIRED` or `CHARGE_ACTION_REQUIRED`, additional
steps from the customer are required. To provide the app with the result of the additional action, call the `transactionProcess` mutation.

The `transactionProcess` mutation requires the following arguments:
...
The mutation triggers the `TRANSACTION_PROCESS_SESSION` webhook. For more details about this webhook, refer to its documentation. FIXME:
...
- `AUTHORIZATION_ACTION_REQUIRED` or `CHARGE_ACTION_REQUIRED` indicates that additional action is required from the customer to
  finalize the payment. Calling `transactionProcess` will be required to pass the result of the action to the app.
- `AUTHORIZATION_SUCCESS` indicates that funds have been authorized.
- `AUTHORIZATION_FAILURE` indicates that authorization has failed.
- `AUTHORIZATION_REQUEST` indicates that authorization is in a pending state.
- `CHARGE_SUCCESS` indicates that funds have been charged.
...
## Saleor App with existing Checkout App
...
The following example shows how you can use the [transactionCreate](../api-reference/mutations/transaction-create) mutation to create a new transaction.
...
- Transactions attached to the checkout are accessible via the [transactions](../api-reference/objects/checkout#code-style-fontweight-normal-checkoutbtransactionsbcodetransactionitem--) field.
- Transactions attached to the order are accessible via the [transactions](../api-reference/objects/order#code-style-fontweight-normal-orderbtransactionsbcodetransactionitem--) field.
...
The [transactionUpdate](../api-reference/mutations/transaction-update) can be called only by the transaction owner.
The owner is the User or App that created the given transaction.
:::

The following example shows how you can use the [transactionUpdate](../api-reference/mutations/transaction-create) mutation to update the transaction.
...
report a new transaction event. The newly created event will be used to [re-calculate](#re-calculations-of-transaction-amounts) the transaction's amounts.
...
The following example shows how the [transactionEventReport](../api-reference/mutations/transaction-event-report) mutation is used to report an event
...
The provided data will be used to create a new [TransactionEvent](../api-reference/objects/transaction-event) object that will be included in the
...
- `alreadyProcessesed` - Defines if the reported event hasn't been processed earlier.
  ...
  different amount, the error with code [INCORRECT_DETAILS](../api-reference/enums/transaction-event-report-error-code#code-style-fontweight-normal-transactioneventreporterrorcodebincorrect_detailsbcode)
  will be raised.
...
When action is processed asynchronously on the payment provider side, the app should call the [transactionActionRequest](../api-reference/mutations/transaction-request-action)
...
The app immediately receives the status of the requested action. It can provide the details of the action in the response to the received Saleor webhook. The following webhook events
can accept action details in the response:: [TRANSACTION_CHARGE_REQUESTED](../api-reference/enums/webhook-event-type-sync-enum#code-style-fontweight-normal-webhookeventtypesyncenumbtransaction_charge_requestedbcode),
[TRANSACTION_CANCELATION_REQUESTED](../api-reference/enums/webhook-event-type-sync-enum#code-style-fontweight-normal-webhookeventtypesyncenumbtransaction_cancelation_requestedbcode),
[TRANSACTION_REFUND_REQUESTED](../api-reference/enums/webhook-event-type-sync-enum#code-style-fontweight-normal-webhookeventtypesyncenumbtransaction_refund_requestedbcode).
...
The re-calculation will differ based on the value of the [TransactionEventTypeEnum](../api-reference/enums/transaction-event-type-enum) provided as the field `type`
