...
The first approach is used by default. It means that the order can be only created after the checkout is fully paid. See [`allowUnpaidOrders`](api-reference/miscellaneous/objects/order-settings#code-style-fontweight-normal-ordersettingsballowunpaidordersbcodeboolean--) setting for more details.
