...
Saleor distinguishes two different approach of processing a payment:

- By using a [plugin](extending/overview#plugins) embedded in the Saleor code.
- By using a [Saleor App](extending/overview#apps-recommended).
...
- [Saleor payment app](developer/payments.mdx#processing-a-payment-with-payment-app) - The communication between the storefront and payment app goes through Saleor.
  ...
  Saleor can determine the current status of the payment and update it on its own side.
...
### Processing a payment with Payment App
