...
or [User.storedPaymentMethod](api-storefront/users/objects/user.mdx#code-style-fontweight-normal-userbstoredpaymentmethodsbcodestoredpaymentmethod--)
...
As a response, the mutation will return a `success` status and an optional `message`` returned by the app.
