...
The available action is `REFUND`. The authorized funds are charged, so `amountCharged` is equal to 99USD.
...
The updated status of the transaction is `CHARGED`.
The authorized funds are charged, so `amountAuthorized` is 0. `amountCharged` is equal to 99USD.
...
`transaction.chargedAmount` (`transaction.chargedAmount += amount`)
...
`transaction.refundedAmount` (`transaction.refundedAmount += amount`)
...
(`transaction.canceledAmount += amount`).
