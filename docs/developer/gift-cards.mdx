...
## Tax handling and application during checkout

Gift cards are applied to the checkout total after tax calculation. This means:
...
To track how much each gift card contributed to the checkout:
1. Query the checkout's `subtotalPrice` and `shippingPrice` to get the total before gift cards
2. Compare with `totalPrice` to see the final amount after gift cards
3. The difference shows the total reduction from all gift cards

Example query to get detailed checkout information:
...
## Balance changes and order completion

Gift card balances are only updated after the order is completed:
- During checkout, the `currentBalance` remains unchanged
- After order completion, the used amount is deducted from the gift card's balance
- If an order is cancelled, the gift card balance is restored
- If multiple gift cards were used, each one's balance is updated proportionally to its contribution
...
- There's no way to guarantee the gift card hasn't been used or shared
:::

If a customer wants to return a physical gift card product (unused gift card code):
1. The gift card must not have been fulfilled yet
2. The return process follows standard product return procedures
3. The gift card code will be deactivated to prevent future use

For more information about returns, see [Returns](api-reference/orders/returns.mdx).