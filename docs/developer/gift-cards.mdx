...
:::note
Gift Cards API may be limiting for some use cases like split payments or refunds.

We suggest to use an App that implements gift cards as a payment methods via Transaction API
:::
...
There is no way to refund a gift card directly. You can create a new gift card with adjusted amount and send it to the customer again.