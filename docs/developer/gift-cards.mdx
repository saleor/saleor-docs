...
Running `transactionInitialization` mutation results in creating an authorization transaction.

### Updating Authorization Transactions

If gift card is already authorized to a checkout but `amount` has to be updated run `transactionInitialize` with an update `amount` value.
