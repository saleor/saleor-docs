...
Using [Transaction API](/developer/payments/overview) you can use built-in payment gateway dedicated for consuming gift cards created in Saleor.

### Applying Gift Cards
To use a gift card in checkout, run the [`transactionInitialize`](/api-reference/payments/mutations/transaction-initialize) mutation.
...
### Authorization Transaction Cancellations

Upon successful transaction initialization requested amount of funds from gift card will be authorized to be charged upon order confirmation.

Cancelling authorization transaction can be performed by:
- Calling `transactionRequestAction` with `CANCEL` action
- Applying gift card to another checkout

### Charge Transaction Refunds

Upon completing a checkout and confirming an order Saleor will attempt to charge funds from the gift card.
...
:::important
It's not recommended to use Gift Cards this way.
You can disable legacy use of Gift Cards in Checkout by unsetting `allowLegacyGiftCardUse` flag in Channel configuration.
:::
...
- If an order is cancelled, the gift card balance is not restored
  ...
  - If you used Transaction API for applying gift card you can also [refund](#gift-card-charge-refunds) gift card transactions
