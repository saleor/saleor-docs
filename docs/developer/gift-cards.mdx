...
Firstly, you have to create a product type with `GIFT_CARD` kind (use [productTypeCreate](/developer/api-reference/mutations/product-type-create.mdx) for that), and then create a product (use [productCreate](/developer/api-reference/mutations/product-create.mdx) mutation)
...
To use a gift card in the checkout just run [checkoutAddPromoCode](/developer/api-reference/mutations/checkout-add-promo-code.mdx)
