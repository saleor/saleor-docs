...
- by setting `orderSettings.allowUnpaidOrders` for a given `Channel` to `true` and calling `checkoutComplete` without any payment/transaction object
- by using the `orderCreateFromCheckout` mutation with first order then payment flow.
