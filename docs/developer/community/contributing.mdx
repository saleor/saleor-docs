...
and must have `count_queries` argument.
...
### API
- Use `id` for mutation inputs instead od `model_name_id`
...
`ADDED_IN_X` at the beginning of the description and `PREVIEW_FEATURE` label at the end.
All labels can be found in `saleor/graphql/core/descriptions.py`.

When we would like to remove the API field, we marked it first as `DEPRECATED`.
In a field definition, there is a dedicated argument for that: `deprecation_reason`,
but in the case of arguments, we are using `DEPRECATED_IN_X_INPUT` label in the description.
...
Example: So instead of using generic PaymentErrorCode, for PaymentCreate mutation
...
sorting input must be used in `sort_by` argument, and filter input in `filter`
...
If it resolves some GitHub issue, please linked it. Wait for all actions to be performed,
...
So, the question is, when should I add entry in a breaking changes section? 
