...
To guarantee repeatable installations, we manage the project dependencies using [Poetry](https://poetry.eustace.io/). You can find the project’s direct dependencies in `pyproject.toml`.
...
Describe what you test and what it is supposed to verify. Often, the test name is not enough to explain the expected behavior. Also, it will help to see that the test should be split into two, as it tests too much logic at once. In addition, it's beneficial in test maintenance.
...
Use [black](https://github.com/python/black/) to ensure your code is correctly formatted.
...
Use [pycodestyle](http://pycodestyle.pycqa.org/en/latest/) to ensure your code adheres to PEP 8.
