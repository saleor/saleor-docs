...
e.g. **Adding `new_field` to Saleor version 3.23:**

- In version 3.22 add `new_field` with `null=True`.
- In version 3.22 add logic in code that covers writing to `new_field`.
- In version 3.22 add migration that will fill nullable values. Please check [data migration](#data-migration)
- In version 3.23 change `new_field` to be nonnullable.
...
e.g. **Rename field `old_field_name` to `new_field_name` in Saleor version 3.23:**

- In version 3.22 add a new field `new_field_name`.
- In version 3.22 add logic in code that covers writing to both `new_field_name` and `old_field_name`.
- In version 3.22 migrate data from `old_field_name` to `new_field_name`. Please check [data migration](#data-migration)
- In version 3.23 update logic to use only `new_field_name`.
- From version 3.23 remove `old_field_name`. Please check [removing field](#removing-field).
...
e.g. **Change field type of `number` from `int` to `string` in Saleor version 3.23:**

- In version 3.22 add a new field `number_string`.
- In version 3.22 add logic in code that covers writing to both `number` and `number_string`.
- In version 3.22 migrate data from `number` to `number_string`. Please check [data migration](#data-migration)
- From version 3.23 remove `number`. Please check [removing field](#removing-field)
- In version 3.24 add a new field `number` back.
- In version 3.24 add logic in code that covers writing to both `number` and `number_string`.
- In version 3.24 migrate data from `number_string` to `number`. Please check [data migration](#data-migration)
- From version 3.25 remove `number_string`. Please check [removing field](#removing-field)
...
e.g. **Removing field `old_field` in Saleor version 3.23:**

- In version 3.23 drop the field from ORM and ensure that the field is nullable or has a default value:
...
- In version 3.24 drop the field from the database:
...
e.g. task to migrate data between orders in Saleor version 3.22 should be placed in `saleor/order/migrations/tasks/saleor3_22.py`
...
from .tasks.saleor3_22 import migration_task
