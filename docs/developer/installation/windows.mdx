...
3. Choose a secret key. We try to provide usable default values for all settings but we have decided not to provide a default for `SECRET_KEY` as someone would inevitably ship a project with the default value left in code.
...
4. Create a database user using the command line tools that came with your PostgreSQL installation:
   ...
   When asked to choose a password for the new role, use "saleor" (or see [how to configure Saleor to use a different password](developer/running-saleor/configuration.mdx#database_url)). When asked for your password, give the one you set during installation of PostgreSQL.
...
6. Migrate the database:
   ...
   :::caution
   This command creates database extensions. If you get an error related to the `CREATE EXTENSION` command, please return to user creation step above.
   :::
