...
:::info
When providing media via external image URLs (`mediaUrl`), images are fetched **asynchronously** after the mutation completes. The media objects are created immediately, but the actual images may not be available right away. See the [upgrade guide](/upgrade-guides/core/3-22-to-3-23#asynchronous-image-fetching-for-product-media) for details on handling this.
:::
