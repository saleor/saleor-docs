...
To perform bulk operations on products and variants, a user needs the `MANAGE_PRODUCTS` permission.
...
- **Attributes**: Allows to define product attributes. Each attribute should include its ID and values. See [`ProductAttributeValueInput`](/api-reference/products/inputs/product-attribute-value-input) for details.
...
#### Key fields to consider for variants:
...
- Stocks and quantities
- Channel listings and pricing
- And many more availbe variant fields
