...
Saleor does not send these emails itself. Install [Customer Emails](/developer/app-store/apps/customer-emails/overview.mdx) on Saleor Cloud, or the [SMTP app](/developer/app-store/apps/smtp/overview.mdx) if you are self-hosting (or another app subscribed to the account events).
