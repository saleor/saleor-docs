...
1. Trigger a password reset - Saleor sends an email with a secure link to set a new password.
The [`requestPasswordReset`](/api-reference/users/mutations/request-password-reset.mdx) mutation generates a unique token and includes it in the email link (`redirectUrl`).

2. Set New Password: open link from received email, and enter a new password in the Saleor dashboard.  
The dashboard calls the [`setPassword`](/api-reference/users/mutations/set-password.mdx) mutation, 
passing the user’s email, new password and the token from the link to securely update the password.

### Troubleshooting

#### Token Invalidation by Login

If a staff user is already logged in via Single Sign-On (SSO) or another session when they attempt to use a password reset link, the token may become invalid. 
This happens because the token hash is partially based on the user's last login state—if the user logs in (updating their `last_login` timestamp) after the reset token was generated, the token becomes invalid.
