...
:::note
Staff and customers share the same [`User`](/developer/users/user-model.mdx) type, distinguished by `isStaff: true`. The mutations on this page operate on users with `isStaff: true`. See [Customers and Staff as One User](/developer/users/user-model.mdx) for how the two roles relate and for the rules around promoting a customer to staff.
:::
...
:::caution
If [`passwordLoginMode`](/api-usage/authentication.mdx#password-login-mode) is set to `DISABLED`, staff password resets via `requestPasswordReset` and `setPassword` are blocked. Use OIDC for staff authentication or set the mode back to `ENABLED`.
