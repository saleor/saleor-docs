...
:::caution
If [`passwordLoginMode`](/api-usage/authentication.mdx#password-login-mode) is set to `CUSTOMERS_ONLY` or `DISABLED`, staff password resets via `requestPasswordReset` and `setPassword` may be blocked. Ensure the mode is set to `ENABLED` or use OIDC for staff authentication.
:::
