...
:::note
Customers and staff are represented by the same [`User`](/developer/users/user-model.mdx) type. The mutations on this page operate on users with `isStaff: false`. For the relationship between the two roles — including how a customer can be promoted to staff — see [Customers and Staff as One User](/developer/users/user-model.mdx).
:::
...
Besides the built-in fields, `customerUpdate` can also assign a [customer type and set attribute values](/developer/users/customer-types.mdx) on the account.
...
:::tip
Metadata is freeform key-value data. For structured fields that are validated against a schema and filterable in the `customers` query — such as loyalty tiers or company details — use [customer types and attributes](/developer/users/customer-types.mdx) instead.
:::
