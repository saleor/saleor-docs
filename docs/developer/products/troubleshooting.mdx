...
2. **Variant is enabled**: At least one `Variant` exists is set to `Available` for respective channel. `Price` needs to be higher than zero.
...
4. In the product variant settings check if `Warehouses` of the shipping zone has stock. Only warehouses enabled in the shipping zone are considered for stock availability.

### Product can't be added to a checkout

Checkout is always channel-specific. Products must be available in the respective channel and shipped to the channel's default country (check the channel's settings). Follow steps [Product can't be added to an order](#product-cant-be-added-to-an-order) to troubleshoot.
