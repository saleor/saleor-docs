...
To obtain product variants, query the `productVariants` field on the `Product` type:
<Tabs>
<TabItem value="Query">
```graphql
...
:::note
To avoid leaking precise stock information, the `quantityAvailable` field will not be greater than the maximum orderable threshold 
which can be configured through the mutation [`shopSettingsUpdate`](/api-reference/shop/mutations/shop-settings-update.mdx).
:::

## Stock quantities

For creating integrations with an external stock management system, you will need to use stock values not restricted by 
[`Shop.limitQuantityPerCheckout`](/api-reference/miscellaneous/objects/shop.mdx#limit-quantity-per-checkout).
...
- The `quantityAvailable` is a sum of stock quantities from all available stocks.
  In this case, the sum is higher than the `Shop.limitQuantityPerCheckout`, so the maximum
...
be equal to the `Shop.limitQuantityPerCheckout` value.
:::
...
|--------|-------------|--------------|
| [Product Type](/developer/products/overview.mdx#product-types) | Defines whether the product is shippable (physical vs. digital) and which attributes (e.g., size, color) it supports. | [`productTypeCreate`](/api-reference/products/mutations/product-type-create.mdx) |
| [Category](/developer/products/overview.mdx#categories-and-collections) | Organizes products into a hierarchical tree structure, often used for navigation in the storefront (e.g., "Clothing > Men > Shirts"). | [`categoryCreate`](/api-reference/products/mutations/category-create.mdx) |
| [Collection (Optional)](/developer/products/overview.mdx#categories-and-collections) | Groups products for marketing purposes or curated storefront sections (e.g., “Summer Sale” or “Best Sellers”). | [`collectionCreate`](/api-reference/products/mutations/collection-create.mdx) |
| [Channel](/developer/channels/overview.mdx) | Controls the product's visibility, availability, pricing, and publication status across different sales channels (e.g., online store, marketplace). | [`channelCreate`](/api-reference/channels/mutations/channel-create.mdx) |
| [Warehouse](/developer/stock/overview.mdx) | Required to track stock levels and assign inventory to product variants. | [`createWarehouse`](/api-reference/products/mutations/create-warehouse.mdx) |

### Step-by-Step: Creating a Product and Variant

1. [`productCreate`](/api-reference/products/mutations/product-create.mdx)
...
2. [`productChannelListingUpdate`](/api-reference/products/mutations/product-channel-listing-update.mdx)
...
3. [`productVariantCreate`](/api-reference/products/mutations/product-variant-create.mdx)
...
4. [`productVariantChannelListingUpdate`](/api-reference/products/mutations/product-variant-channel-listing-update.mdx)
...
5. [`productVariantStocksCreate`](/api-reference/products/mutations/product-variant-stocks-create.mdx)
...
[`productMediaCreate`](/api-reference/products/mutations/product-media-create.mdx) – Creates a media object (image or external URL) associated with a product.
...
:::info
When using `mediaUrl` with an image URL, the image is fetched **asynchronously** in the background. The mutation returns immediately, but the image may not be available right away. Address returned in the `url` field returns HTTP 503 while the image is being downloaded. Clients should handle this by retrying with appropriate backoff. See the [upgrade guide](/upgrade-guides/core/3-22-to-3-23#asynchronous-image-fetching-for-product-media) for details.
:::
...
[`variantMediaAssign`](/api-reference/products/mutations/variant-media-assign.mdx) – There is no dedicated upload mutation for variant media. Assign uploaded product media to specific variants (e.g., color-specific images).
...
[`productMediaReorder`](/api-reference/products/mutations/product-media-reorder.mdx) - Allows to change ordering of the product media.
...
[`productMediaUpdate`](/api-reference/products/mutations/product-media-update.mdx) Update media description
