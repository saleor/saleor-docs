...
If the user logged in by OAuth is a staff user, he will be assigned to the default permission group if set.
The user permissions are the sum of permissions granted from the OAuth and from the Saleor groups that
...
:::important
The data in Saleor are also protected by the channel permissions, so to ensure that the staff
logged to Saleor by OAuth will have the correct right, you should create the permission group
without channel restrictions on Saleor side, and set to group as `default permission group name for new staff users`.
:::
