...
- profile
- email
- offline*access - \_optionally, when you enable refreshing the token*
...
Executing [`externalAuthenticationUrl`](api-reference/mutations/external-authentication-url.mdx) mutation will prepare special URL which will redirect user to requested page after successfull authentication. After redirection `state` and `code` fields will be added to the URL. [`externalObtainAccessTokens`](api-reference/mutations/external-obtain-access-tokens.mdx) will require both.
...
- `input` [`JSONString`](api-reference/scalars/json-string.mdx)
...
The [`externalObtainAccessTokens`](api-reference/mutations/external-obtain-access-tokens.mdx) mutation will generate requested access tokens. The mutation takes the following input fields:
...
The [`externalRefresh`](api-reference/mutations/external-refresh.mdx) mutation will generate new access tokens when provided with a valid refresh token.
If the refresh token is not provided as an argument, the plugin will try to read it from a cookie set by the [`tokenCreate`](api-reference/mutations/token-create.mdx) mutation. In that case, a matching CSRF token is required.
...
You can prepare the logout URL by calling [`externalLogout`](api-reference/mutations/external-logout.mdx) mutation.
...
When OAuth permissions are enabled, Saleor will try to fetch [own's permissions](developer/available-plugins/openid-connect.mdx#oauth-permissions) from `access_token.scope`. In case when token's `scope` doesn't have any Saleor's permissions,
