...
The Adyen plugin allows you to process customer transactions via [Adyen](https://www.adyen.com).
...
The next step is to choose Adyen as the payment gateway that we want to use to process the checkout.
...
- `order` - Optional Order object. `Null` when Saleor was not able to finalize checkout or when additional action is required from the customer.
...
:::caution
When the notification is received for an inactive payment or there will be any issue
with order creation, the payment is refunded or voided (in case the payment wasn't charged yet).
:::

## Enabling native 3DS2

Saleor uses redirect 3D Secure 1 and 2 by default. [Enabling native 3D Secure](dashboard/configuration/plugins/adyen.md#enabling-the-adyen-integration) will add the `additionalData.allow3DS2: true` flag
