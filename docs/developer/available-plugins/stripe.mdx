...
Description of `checkoutPaymentCreate` mutation can be found [here](developer/checkout.mdx#selecting-a-payment-method)
...
```graphql
mutation{
  checkoutComplete(checkoutId: "ID", storeSource:true, paymentData:"{\"setup_future_usage\": \"off_session\"}"){
    order{
      id
      userEmail
      created
...
```graphql
mutation{
  checkoutComplete(checkoutId: "ID", storeSource:true, paymentData:"{\"payment_method_id\": \"pm_id\", \"off_session\": false}"){
    order{
      id
      userEmail
      created
