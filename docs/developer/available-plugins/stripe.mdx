...
:::note
Saleor 3.1 changes the way of saving card in payment gateways.
Instead of `checkoutComplete`, use `checkoutPaymentCreate` mutation.


Description of the `checkoutPaymentCreate` mutation can be found [here](developer/checkout.mdx#selecting-a-payment-method).
:::

##### Saleor 3.0 (the old method)

Use `paymentData.setup_future_usage` to store payment method for future payments with `checkoutComplete`
