...
The OIDC plugin supports two way of integration:
 - Saleor as a client for authorization server - authentication flow is handled by Saleor API.
 - Saleor as a resource server - grant users access using Authorization Code Flow.
...
In resource server flow, Saleor API only verifies if token included in request is valid. Operations like login, logout or token refresh have to be handled directly by auth provider API.
