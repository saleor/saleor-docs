...
After being called by [`PluginManager`](developer/extending/plugins.mdx), the plugin will fill the template using event payload and send a message via SMTP or Sendgrid.

## Available events
[WebhookEventTypeEnum](developer/api-reference/enums/webhook-event-type-enum.mdx) contain all availabe events.
