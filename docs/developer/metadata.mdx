...
By default, `metafields` (`privateMetafields`) will return all metadata that the object has assigned. You can use the optional `keys` argument to select the fields you want to return. We highly recommend that you only fetch the fields your application understands as it limits the amount of information being needlessly transferred.
