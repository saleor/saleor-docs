...
Since version 3.19, Saleor can skip address validation entirely. To enable this, set the `skipValidation` flag to `true` in the `AddressInput`.
This disables all validation except for the `country` field, which still requires a valid `CountryCode` enum.

Please note that using the `skipValidation` flag may require extra permissions, and not all mutations support this feature.
