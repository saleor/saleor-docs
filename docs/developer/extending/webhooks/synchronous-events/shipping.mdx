...
The `currency` returned by the app must match the checkout's currency. If they do not match, the corresponding shipping method will not be displayed to the customer.

:::

:::warning
 The following queries will not return external shipping methods:
  - `checkouts(...)`
  - `me.checkouts(...)`
  - `checkoutLines(...)`
...
:::warning
 The following queries will return non filtered shipping methods:
  - `checkouts(...)`
  - `me.checkouts(...)`
  - `checkoutLines(...)`
  - `orders(...)`
