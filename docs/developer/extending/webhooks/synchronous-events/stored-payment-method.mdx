...
This feature is dedicated to [third-party](/api-reference/apps/enums/app-type-enum.mdx#thirdparty) apps.
...
customer requested a field [User.storedPaymentMethod](/api-reference/users/objects/user.mdx#storedpaymentmethodsstoredpaymentmethod), [checkout.storedPaymentMethods](/api-reference/checkout/objects/checkout.mdx#storedpaymentmethodsstoredpaymentmethod).

The webhook expects to return a list of payment methods that are assigned to the customer. The payment
methods from the webhook response will be returned as a response for [User.storedPaymentMethod](/api-reference/users/objects/user.mdx#storedpaymentmethodsstoredpaymentmethod), [checkout.storedPaymentMethods](/api-reference/checkout/objects/checkout.mdx#storedpaymentmethodsstoredpaymentmethod) fields.
...
Saleor will send a
[LIST_STORED_PAYMENT_METHODS](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#list_stored_payment_methods)
...
You can find more details about building webhook subscription queries in [Subscription Webhook Payloads](../subscription-webhook-payloads.mdx).
...
A synchronous webhook [STORED_PAYMENT_METHOD_DELETE_REQUESTED](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#stored_payment_method_delete_requested) is called when the user requests the deletion of the stored payment method. User can request the deletion of the stored payment method by triggering the mutation [storedPaymentMethodRequestDelete](/api-reference/payments/mutations/stored-payment-method-request-delete.mdx).
More details about the flow can be found in the [transactions overview](/developer/payments/overview.mdx).

### Request

The example below shows a sample webhook subscription defined for
[STORED_PAYMENT_METHOD_DELETE_REQUESTED](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#stored_payment_method_delete_requested)
webhook:
...
The synchronous [PAYMENT_GATEWAY_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment_gateway_initialize_tokenization_session) webhook is triggered when a customer requests the initialization of the payment gateway to tokenize a payment method. This webhook is activated by executing the [paymentGatewayInitializeTokenization](/api-reference/payments/mutations/payment-gateway-initialize-tokenization.mdx) mutation.

The webhook payload includes details about the [customer](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx#useruser), [channel](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx#channelchannel), and [data](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx#datajson). The [data](/api-reference/payments/mutations/payment-gateway-initialize-tokenization.mdx#datajson) field contains the JSON data provided as input in the [paymentGatewayInitializeTokenization](/api-reference/payments/mutations/payment-gateway-initialize-tokenization.mdx) mutation.
...
The synchronous [PAYMENT_METHOD_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment_method_initialize_tokenization_session) webhook is triggered when a customer requests the tokenization of a payment method. This webhook is activated by executing the [paymentMethodInitializeTokenization](/api-reference/payments/mutations/payment-method-initialize-tokenization.mdx) mutation.

The webhook payload includes details about the [customer](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#useruser), [channel](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#channelchannel), and [data](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#datajson). The [data](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#datajson) field contains the JSON data provided as input in the [paymentMethodInitializeTokenization](/api-reference/payments/mutations/payment-method-initialize-tokenization.mdx) mutation.
...
Saleor will send a [PAYMENT_METHOD_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment_method_initialize_tokenization_session) webhook using the `PaymentMethodInitializeTokenizationSession` subscription type or with a predefined payload in case of a missing subscription query.

For detailed information on building webhook subscription queries, please refer to [Subscription Webhook Payloads](../subscription-webhook-payloads.mdx).
...
The `result` field is an enum value of [PaymentMethodTokenizationResultEnum](/api-reference/payments/enums/payment-method-tokenization-result.mdx). In case of a failure `result`, the `error` message will be attached to the [error](/api-reference/payments/objects/payment-method-initialize-tokenization.mdx#errorspaymentmethodinitializetokenizationerror) returned by the [paymentMethodInitializeTokenization](/api-reference/payments/mutations/payment-method-initialize-tokenization.mdx) mutation.

## Process additional actions for payment method tokenization

`PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION`

The synchronous [PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment_method_process_tokenization_session) webhook is triggered when a customer requests to process additional actions required for tokenizing the payment method. This webhook is activated by executing the `paymentMethodProcessTokenization` mutation.
...
Saleor will send a [PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment_method_process_tokenization_session) webhook using the [PaymentMethodProcessTokenizationSession](/api-reference/payments/objects/payment-method-process-tokenization-session.mdx) subscription type or with a predefined payload in case of a missing subscription query.
...
The `result` field is an enum value of [PaymentMethodTokenizationResultEnum](/api-reference/payments/enums/payment-method-tokenization-result.mdx). In case of a failure `result`, the `error` message will be attached to the [error](/api-reference/payments/objects/payment-method-process-tokenization.mdx#errorspaymentmethodprocesstokenizationerror) returned by the [paymentMethodProcessTokenization](/api-reference/payments/objects/payment-method-process-tokenization.mdx#errorspaymentmethodprocesstokenizationerror) mutation.
