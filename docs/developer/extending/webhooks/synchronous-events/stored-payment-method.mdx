...
customer requested a field [User.storedPaymentMethods](/api-reference/users/objects/user.mdx#stored-payment-methods), [checkout.storedPaymentMethods](/api-reference/checkout/objects/checkout.mdx#stored-payment-methods).

The webhook expects to return a list of payment methods that are assigned to the customer. The payment
methods from the webhook response will be returned as a response for [User.storedPaymentMethods](/api-reference/users/objects/user.mdx#stored-payment-methods), [checkout.storedPaymentMethods](/api-reference/checkout/objects/checkout.mdx#stored-payment-methods) fields.
...
Saleor will send a
[LIST_STORED_PAYMENT_METHODS](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#list-stored-payment-methods)
...
[STORED_PAYMENT_METHOD_DELETE_REQUESTED](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#stored-payment-method-delete-requested)
webhook:
...
The synchronous [PAYMENT_GATEWAY_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment-gateway-initialize-tokenization-session) webhook is triggered when a customer requests the initialization of the payment gateway to tokenize a payment method. This webhook is activated by executing the [paymentGatewayInitializeTokenization](/api-reference/payments/mutations/payment-gateway-initialize-tokenization.mdx) mutation.

The webhook payload includes details about the [customer](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx#user), [channel](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx#channel), and [data](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx#data). The [data](/api-reference/payments/mutations/payment-gateway-initialize-tokenization.mdx#data) field contains the JSON data provided as input in the [paymentGatewayInitializeTokenization](/api-reference/payments/mutations/payment-gateway-initialize-tokenization.mdx) mutation.
...
Saleor will send a [PAYMENT_GATEWAY_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment-gateway-initialize-tokenization-session) webhook using the [PaymentGatewayInitializeTokenizationSession](/api-reference/payments/objects/payment-gateway-initialize-tokenization-session.mdx) subscription type or with a predefined payload in case of a missing subscription query.
...
The synchronous [PAYMENT_METHOD_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment-method-initialize-tokenization-session) webhook is triggered when a customer requests the tokenization of a payment method. This webhook is activated by executing the [paymentMethodInitializeTokenization](/api-reference/payments/mutations/payment-method-initialize-tokenization.mdx) mutation.

The webhook payload includes details about the [customer](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#user), [channel](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#channel), and [data](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#data). The [data](/api-reference/payments/objects/payment-method-initialize-tokenization-session.mdx#data) field contains the JSON data provided as input in the [paymentMethodInitializeTokenization](/api-reference/payments/mutations/payment-method-initialize-tokenization.mdx) mutation.
...
Saleor will send a [PAYMENT_METHOD_INITIALIZE_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment-method-initialize-tokenization-session) webhook using the `PaymentMethodInitializeTokenizationSession` subscription type or with a predefined payload in case of a missing subscription query.
...
The `result` field is an enum value of [PaymentMethodTokenizationResultEnum](/api-reference/payments/enums/payment-method-tokenization-result.mdx). In case of a failure `result`, the `error` message will be attached to the [errors](/api-reference/payments/objects/payment-method-initialize-tokenization.mdx#errors) returned by the [paymentMethodInitializeTokenization](/api-reference/payments/mutations/payment-method-initialize-tokenization.mdx) mutation.

## Process additional actions for payment method tokenization

`PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION`

The synchronous [PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#payment-method-process-tokenization-session) webhook is triggered when a customer requests to process additional actions required for tokenizing the payment method. This webhook is activated by executing the `paymentMethodProcessTokenization` mutation.
...
The `result` field is an enum value of [PaymentMethodTokenizationResultEnum](/api-reference/payments/enums/payment-method-tokenization-result.mdx). In case of a failure `result`, the `error` message will be attached to the [errors](/api-reference/payments/objects/payment-method-process-tokenization.mdx#errors) returned by the [paymentMethodProcessTokenization](/api-reference/payments/objects/payment-method-process-tokenization.mdx) mutation.
