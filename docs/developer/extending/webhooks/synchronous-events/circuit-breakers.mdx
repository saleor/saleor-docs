...
The breaker will trip again if the number of errors exceeds the threshold and minumum count. The conditions on which breaker re-enters open state are controlled by another two constants:
...
Note that all of circuit breaker events (both success and failures) have a certain TTL (time to live) and are stored in Redis. On each transition, the events are purged, so each state has a fresh start and is not contaminated by older events.
...
However it will **not** block any requests. This is useful for testing the breaker without affecting the webhooks berhavior.
...
flowchart TD
    A["CLOSED (working)"]
    A --> C{failure min count > 100, threshold > 35%?}
    C -->|No| C{failure min count > 100, threshold > 35%?}
    C -->|Yes| D["OPEN (tripped)"]
    D ---|"Cooldown"| E["HALF OPEN"]
    E --- F{failure min count > 20, threshold recovery > 30%?}
    F ---|"Yes"| D
