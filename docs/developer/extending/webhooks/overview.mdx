...
Both sync and async webhooks requests are time-limited. Saleor will wait maximum 20s for full HTTP request round trip, which leaves approx 18s to process the data and respond.
...
- For asynchronous webhooks use job queues like SQS. "Accept" response from the queue is fast and the rest of the processing should be managed on the external system like app.
...
- 20s that Saleor keeps the request is a ceiling value, but it should not be treated as an accepted standard. We recommend to rethink synchronous event targets to process events as quick as possible.
