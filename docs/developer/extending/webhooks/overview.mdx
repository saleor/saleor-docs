...
Webhooks can be created using the Saleor Dashboard, Saleor GraphQL API or Saleor CLI. All of these methods trigger [`appCreateWebhook`](../api-reference/graphql-api/apps.md#appcreatewebhook) mutation.

Check [Creating a Webhook](./creating-a-webhook.md) page for more details.
...
You can read more about GraphQL subscriptions in the [GraphQL Subscriptions](./graphql-subscriptions.md) section.

### Webhook headers

In addition to the payload several headers included:
...
Saleor gives you the option to add custom headers to the request (available since version 3.12). `Authorization*` and `X-*` keys are allowed, with limitations of 5 headers per webhook and 998 characters per header. To check how to add `customHeaders` go to [Creating webhook](/docs/developer/extending/webhooks/creating.mdx).
...
You can read more about asynchronous events in the [Asynchronous events](./asynchronous-events.md) section and about synchronous events in the [Synchronous events](./synchronous-events.md) section.
