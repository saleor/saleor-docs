...
Saleor gives you the option to add custom headers to the request (available since version 3.12). `Authorization*` and `X-*` keys are allowed, with limitations of 5 headers per webhook and 998 characters per header. To check how to add `customHeaders` go to [Creating webhook](./creating.mdx).
...
Both synchronous and asynchronous webhook requests are time-limited. Saleor will wait a maximum of 20 seconds for the full HTTP request round trip, which leaves approximately 18 seconds to process the data and respond.

Some operations can contain more than one synchronous webhook invocation. Saleor can't guarantee successful processing if a summary of the entire GraphQL request exceeds 30 seconds.

You can follow some good practices to avoid webhook timeouts:

- For asynchronous webhooks use job queues like SQS. "Accept" response from the queue is fast and the rest of the processing should be managed on the external system like an app.
- Be careful with cold starts. Serverless environments can take a few seconds to start with the first request in a time period.
- The 20 seconds that Saleor keeps the request is a ceiling value, but it should not be treated as an accepted standard. We recommend rethinking synchronous event targets to process events as quickly as possible.
