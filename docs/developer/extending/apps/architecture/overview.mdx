...
The app manifest is a JSON file that describes the application details, including its name and version. It lists the app's [permissions](/developer/permissions.mdx#app-permissions), [Dashboard mounting points](/developer/extending/apps/extending-dashboard-with-apps.mdx), webhooks, and optional app-token registration URL.

## App Requirements

Every installable app must publish a [manifest URL](/developer/extending/apps/architecture/app-requirements.mdx#manifest-url). A [register URL](/developer/extending/apps/architecture/app-requirements.mdx#register-url) is required only when the manifest includes `tokenTargetUrl`. Apps without a backend can use the [frontend-only architecture](/developer/extending/apps/architecture/app-requirements.mdx#frontend-only-apps).
