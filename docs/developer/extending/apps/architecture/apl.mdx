...
APL is the core component of communication between the Saleor and the App.
...
Saleor provides a simple persistence with App Metadata. Still, if App doesn't have a token and domain saved (which is typical for serverless architecture), it can't also call Saleor to reach Metadata. Hence need for simple, fast storage to save auth data.

Saleor doesn't require App to use any specific technology for persistence. That's why there are many implementations of APL available in the app-sdk package. And writing a custom one is easy.

# Communication diagram

This diagram explains flow how APL is used:
...
Follow diagram describes example of external webhook, that calls the App and should send data to Saleor
...
Writing custom APL is easy - you only need to implement `APL` interface from app-sdk. An [example of a custom Redis APL is here](https://github.com/saleor/saleor-app-sdk/blob/main/docs/apl.md#example-implementation)

# Developing an App without APL

Technically, a simple App can be developed without APL. The token can be created via graphQL mutation or in Dashboard. The App can also decide to work only with a single Saleor instance - as a custom extension only for a single store.
...
However, some features in app-sdk are meant to be used together with APL interface. That's why it's recommended to follow this pattern.
