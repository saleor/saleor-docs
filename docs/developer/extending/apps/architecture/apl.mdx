...
Saleor apps can work in both single and multi-tenant modes. Single-tenant apps can hardcode the Saleor domain as well as a token, but they can perform only as an extension for one Saleor instance. That is how [App Template Gallery](dashboard/app-templates-gallery.md) works. If you wanted to use another Saleor instance, you would need to deploy it separately (with corresponding environment variables).
