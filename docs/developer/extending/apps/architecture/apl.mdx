...
Saleor provides a simple persistence with the app metadata. But, if the app doesn't save the token and the domain, it is unable to call Saleor to retrieve the metadata. Hence need for simple storage to save the auth data.
...
Saleor apps can work in both single and multi-tenant modes. Single-tenant apps can hardcode the Saleor domain as well as a token, but they can perform only as an extension for one Saleor instance. That is how [App Template Gallery](https://docs.saleor.io/docs/3.x/dashboard/app-templates-gallery) works. If you wanted to use another Saleor instance, you would need to deploy it separately (with corresponding environment variables).
...
- [Vercel](https://vercel.com/dashboard) (single tenant)
...
You are free to write your own APL. You just need to make sure it follows the `APL` TypeScript interface from [app-sdk](https://github.com/saleor/saleor-app-sdk).
