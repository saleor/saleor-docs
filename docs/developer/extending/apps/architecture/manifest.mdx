...
- `webhooks`: List of webhooks that will be set. For details, please look at [asynchronous webhooks](developer/extending/webhooks/asynchronous-events.mdx), [synchronous-webhooks](developer/extending/webhooks/synchronous-events/overview.mdx) and [webhooks' subscription](developer/extending/webhooks/subscription-webhook-payloads.mdx) (be aware that subscription queries are required in manifest) sections.
