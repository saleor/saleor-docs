...
:::info
The `tokenTargetUrl` field is optional. When omitted, Saleor does not create or send an app token during installation. A [frontend-only app](/developer/extending/apps/architecture/app-requirements.mdx#frontend-only-apps) can omit its register endpoint and APL, while still using the short-lived access token that the Dashboard provides for client-side API calls.
:::
