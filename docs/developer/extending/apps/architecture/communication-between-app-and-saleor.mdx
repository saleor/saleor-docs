...
To read more about synchronous events, check [documentation](developer/extending/webhooks/synchronous-events/key-concepts.mdx).

## App to Saleor API

Apps can also communicate with Saleor GraphQL API. You can declare the access scope with the app's [permissions](developer/extending/apps/overview#permissions).
