...
Apps are only visible to users with the `MANAGE_APPS` permission.
...
Given that the user can open the app with just the `MANAGE_APPS` permission, we can not assume the presence of other permissions. Luckily, we can check if the user has the required permissions before we execute the `orders` query.
...
```jsx
import { useAppBridge } from "@saleor/app-sdk";
import { useQuery } from "grapghql-client-library"; // made up library
