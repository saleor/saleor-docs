...
To distinguish between two types of users: the ones who can use the app and the ones who can manage it, let's use an example of Orders App that sends orders to an external service. The app requires the `MANAGE_ORDERS` permission.
...
You can check if the user has the required permissions using the [`useAppBridge` hook](developer/extending/apps/developing-apps/app-sdk/app-bridge.mdx#useappbridge-hook):
...
On the server-side of our app, the app scope applies. It means that as long as the app has the `MANAGE_ORDERS` permission, it can execute the `orders` query.

If you want to render the table for all the users, not just the ones with the `MANAGE_ORDERS` permission, you need to execute the `orders` query on the server-side and pass the result to the client-end.
