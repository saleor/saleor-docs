...
Given that the user can open the app, we can not assume the presence of other permissions. Luckily, we can check if the user has the required permissions before we execute the `orders` query.
