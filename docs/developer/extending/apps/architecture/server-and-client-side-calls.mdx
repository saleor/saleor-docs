...
AppToken is an infinite token generated when the App is being installed. If `tokenTargetUrl` is set in the [App Manifest](./manifest.mdx), 
Saleor will send the token to that URL and the App needs to store it in a safe place to consume the Saleor API. 
Official Saleor Apps and [app-sdk](../developing-apps/app-sdk/overview.mdx) uses [APL concept](./apl.mdx) to store the token.
If `tokenTargetUrl` is not provided in the manifest, no token is sent to the app during installation. This is suitable for apps that don't 
need access to protected API, such as static apps that only serve an iframe in the Dashboard.
