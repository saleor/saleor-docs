...
An AppToken is an infinite token generated during installation only when `tokenTargetUrl` is set in the [App Manifest](./manifest.mdx).
Saleor sends the token to that URL, and the app must store it securely to consume the Saleor API.
Official Saleor Apps and [app-sdk](../developing-apps/app-sdk/overview.mdx) use the [APL concept](./apl.mdx) to store the token.
If `tokenTargetUrl` is not provided, Saleor neither creates an AppToken nor sends a registration request. A [frontend-only app](./app-requirements.mdx#frontend-only-apps) does not need an AppToken because it can use an AccessToken for client-side requests.
