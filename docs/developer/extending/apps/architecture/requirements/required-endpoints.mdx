...
- Method: `GET`
- Response - `JSON` with a valid [App Manifest](https://docs.saleor.io/docs/3.x/developer/extending/apps/manifest)
...
To learn more about what Manifest consists of, please check the [Manifest docs](https://docs.saleor.io/docs/3.x/developer/extending/apps/manifest) or explore its [type](https://github.com/saleor/saleor-app-sdk/blob/main/src/types.ts#L67).
...
While we suggest `/api/register` path, it can be configured in [Manifest](https://docs.saleor.io/docs/3.x/developer/extending/apps/manifest) with the `tokenTargetUrl` field.
