...
- A **full App** has a backend in addition to any Dashboard UI. If the backend needs an AppToken for server-side API calls, define `tokenTargetUrl`, implement the Register URL, and store the token in an [Auth Persistence Layer (APL)](/developer/extending/apps/architecture/apl.mdx).
