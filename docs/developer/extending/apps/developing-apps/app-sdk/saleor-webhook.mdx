...
implemented for both Next.js and Request / Response [Web API](https://developer.mozilla.org/en-US/docs/Web/API/Response)

## Common configuration

Both `SaleorSyncWebhook` and `SaleorAsyncWebhook` contain a similar API with few differences.

### Constructing Webhook instance in Next.js `/pages`
...
### Constructing Webhook instance for Web API (in Next.js `/app`)

Similar to Next.js implementation, create new API route directory, e.g. `/app/api/webhooks/order-created/` in it create two files `route.ts` and `webhook.ts`. `route.ts` file will have API endpoint implementation and `webhook.ts` will declare Saleor webhook. This is done because route can only export HTTP method named functions, otherwise it would result in build error. This API route will be available at endpoint `/api/webhooks/order-created`.

:::info
This example is specific for [Next.js Route Handlers](https://nextjs.org/docs/app/building-your-application/routing/route-handlers) but `fetch-api` handlers should work regardless of used framework.

All methods have `Request => Response` signature, which means that any framework using Fetch API objects should work with `@saleor/app-sdk`

For examples in other frameworks check [awesome-saleor repository](https://github.com/saleor/awesome-saleor)
:::

```typescript
// File: /app/api/webhooks/order-created/webhook.ts
...
export const POST = orderCreatedWebhook.createHandler((request, ctx) => {
  // Your webhook implementation goes here
};
...
import { orderCreatedWebhook } from "../webhooks/order-craeted/webhook";
...
export default webhook.createHandler((req, res, context) => {
...
export const POST = webhook.createHandler((request, context) => {
  // Note: Resoponse.json is static method on Web API Response interface
...
From version 0.50.0 of `@saleor/app-sdk` have new field: `schemaVersion` inside context for `createHandler`:
