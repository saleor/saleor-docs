...
type AppBridgeState = {
    ...
    /**
     * Depending on the mount, app extension may receive additional fields what is the state of the form
     */
    formContext: {
...
`subscribe(eventType, callback)` - can be used to listen to particular [event type](#available-event-types). It returns
an `unsubscribe` function, which unregister the callback.
...
`unsubscribeAll(eventType?)` - unregister all callbacks of provided type. If no type was provided, it will remove all
event callbacks.
...
| `formPayload`   | Fired when Dashboard loads an extension in the context of user-editable form. See [form handling](./form-handling.mdx) |
...
| `formPayloadUpdate`           | Form state that Dashboard will apply.                                                                                                                                   | See [form handling](./form-handling.mdx)                                                                                                                                                                      |
