...
Not, `/const` doesn't exist anymore. All headers are exported from `/headers` path.
...
`ctx.buildResponse` was removed from Webhook's context. Now to create a typed response from sync webhook, you can use
`buildSyncWebhookResponsePayload` function.
...
`requiredEnvVars` parameter no longer is accepted in `SaleorApp` constructor. Please manually validate env variables on the app level.
...
AppBridge now uses `Crypto` API built-in modern browsers. This require localhost or https to work.

When creating `MetadataManager`, `deleteMetadata` callback is required now.