...
Saleor requires the following endpoints for a standalone app:
...
- Register endpoint - If your app needs to access to protected API, you must provide `tokenTargetUrl` in the manifest 
and implement a register endpoint to handle the `POST` request with the token. 
[Read more](developer/extending/apps/installing-apps.mdx#installation-using-graphql-api)
...
export default createManifestHandler({
  manifestFactory({ request, appBaseUrl, schemaVersion }) {
    return {
      name: "My Saleor App",
      // tokenTargetUrl is optional. Omit it if your app doesn't need the token.
