...
App SDK mainly targets serverless environments, like Vercel or AWS Lambda. It provides a set of handlers that can be used to build Saleor apps.
...
- Protected handler - Endpoints meant to be allowed only by the App's frontend (from Dashboard).
...
import { createManifestHandler } from "@saleor/app-sdk/handlers/<PLATFORM>";
