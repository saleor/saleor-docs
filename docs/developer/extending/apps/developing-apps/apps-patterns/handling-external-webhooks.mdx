...
Some applications can be dealing with more than one Saleor instance. In that case, you are going to need a way to distinguish which one is the target of the webhook payload. One way of doing that is to add a domain or unique id to the endpoint, for example `/api/webhooks/status-update?domain=demo.saleor.io`.
