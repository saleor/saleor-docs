...
```graphql
mutation {
  appProblemDismiss(input: { byApp: { ids: ["QXBwUHJvYmxlbTox"] } }) {
    errors {
      field
      message
      code
...
```graphql
mutation {
  appProblemDismiss(input: { byApp: { keys: ["warehouse-api-health"] } }) {
    errors {
      field
      message
      code
...
```graphql
mutation {
  appProblemDismiss(input: { byStaffWithIds: { ids: ["QXBwUHJvYmxlbTox"] } }) {
    errors {
      field
      message
      code
...
mutation {
  appProblemDismiss(
    input: {
      byStaffWithKeys: { keys: ["warehouse-api-health"], app: "QXBwOjE=" }
    }
  ) {
    errors {
      field
...
```graphql
mutation {
  appProblemDismiss(input: { byApp: { keys: ["payment-gateway-health"] } }) {
    errors {
      field
      message
      code
...
mutation {
  appProblemDismiss(
    input: { byApp: { keys: ["payment-gateway-connectivity"] } }
  ) {
    errors {
      field
      message
