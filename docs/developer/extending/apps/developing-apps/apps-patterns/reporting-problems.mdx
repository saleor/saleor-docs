...
| `key`                | `String` | Yes      | —       | Identifier for the type of problem. Used for aggregation and dismissal by key.                           |
| `aggregationPeriod`  | `Int`    | No       | `60`    | Time window in minutes for aggregating problems with the same key. Set to `0` to disable aggregation.    |
| `criticalThreshold`  | `Int`    | No       | —       | When set, the problem is marked as `isCritical: true` once its `count` reaches this value.               |
...
Both apps and staff users can dismiss problems.

### Dismiss by IDs

```graphql
mutation {
  appProblemDismiss(ids: ["QXBwUHJvYmxlbTox"]) {
    app {
      id
    }
    errors {
...
Dismiss all problems matching the given keys:

```graphql
mutation {
  appProblemDismiss(keys: ["warehouse-api-health"]) {
    app {
      id
    }
    errors {
...
`ids` and `keys` cannot be used together in a single call. Exactly one of them is required.

:::

### App vs. staff callers

| Caller | `ids`                           | `keys`                                        | `app` argument  |
| ------ | ------------------------------- | --------------------------------------------- | --------------- |
| App    | Dismiss own problems by ID      | Dismiss own problems by key                   | Not allowed     |
| Staff  | Dismiss by ID (app is inferred) | Dismiss by key (requires `app` argument)      | Required for `keys` |

Staff users need the `MANAGE_APPS` permission and can only dismiss problems for apps within their permission scope.

Staff dismissal by keys example:

```graphql
mutation {
  appProblemDismiss(keys: ["warehouse-api-health"], app: "QXBwOjE=") {
    app {
      id
    }
    errors {
...
Problems are available on the `App` type. You can query them through any query that returns an `App`:
...
| `dismissed`    | `Boolean!`  | Whether the problem was dismissed.                                 |
| `dismissedBy`  | `UserOrApp` | The app or staff user who dismissed the problem, or `null`.        |
...
When the underlying issue is resolved, your app should dismiss the related problems so the Dashboard reflects the current state. Use `appProblemDismiss` with the `keys` argument to clear all problems of a given type:

```graphql
mutation {
  appProblemDismiss(keys: ["payment-gateway-health"]) {
    app {
      id
    }
    errors {
...
mutation {
  appProblemDismiss(keys: ["payment-gateway-connectivity"]) {
    app {
      id
    }
    errors {
...
| Querying `problems` | Standard `App` query permissions | Available wherever the `App` type is returned.          |
