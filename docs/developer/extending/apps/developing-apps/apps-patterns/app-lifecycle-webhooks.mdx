...
`APP_STATUS_CHANGED` fires on both activation and deactivation. Treat deactivation as a _pause_ rather than a removal — the app installation still exists and can be re-activated, so prefer disabling background work over deleting tenant data. Use `APP_DELETED` for destructive cleanup.
