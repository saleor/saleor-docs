...
The issue might be caused by the CORS policy set up in the Saleor Cloud dashboard. Navigate to the environment details of your instance and check `Allowed API origins` section. If option `Selected origins` is selected, you have two possible solutions:

- Add your application URL to the list of allowed origins.
- Change the implementation of your application to make GraphQL requests from the server side only.
