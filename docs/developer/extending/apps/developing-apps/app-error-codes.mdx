...
- 💡 **Debugging Tips**:
  - Check the `requiredSaleorVersion` string in the manifest (e.g., `^3.22` or `>=3.22 <4.0`).
...
- 🧐 **Cause**: This error can arise from a variety of issues, often related to the manifest content or app assets:
    1.  **Manifest values**:
        ...
        *   **Required Saleor version ([`manifest.requiredSaleorVersion`](/developer/extending/apps/architecture/manifest.mdx))**: The value is not a parseable semantic versioning string (e.g., `"~=3.0"`, `"^3.22"`).
