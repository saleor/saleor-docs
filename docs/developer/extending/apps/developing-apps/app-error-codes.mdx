...
- 💡 **Debugging Tips**:
  - Confirm the `manifestUrl` (and any icon URLs if the error points there) are correct and publicly accessible *by your Saleor server*.
  ...
  - Perform network diagnostics from the environment where your Saleor instance runs. For example, try to `curl` or `ping` the `manifestUrl` from that environment to check reachability and the HTTP response.
...
- 💡 **Debugging Tips**:
  ...
  - Verify your current Saleor instance version (this can often be found in your Saleor Dashboard or through your deployment details). Ensure it is compatible with the app's requirements.
...
- 💡 **Debugging Tips**:
  - Review the manifest's `webhooks` section, specifically the `customHeaders` for each webhook. It must be a flat JSON object where both keys and values are strings (e.g., `{"X-My-Header": "value", "Authorization": "Bearer ..."}`).
  - **Header Naming**: Pay close attention to allowed header names. Keys are treated case-insensitively and must either:
  ...
  - For details on how `customHeaders` are defined when creating or updating webhooks via the API, see also the [`WebhookCreateInput`](/api-reference/webhooks/inputs/webhook-create-input.mdx) documentation.

## `AppErrorCode.GRAPHQL_ERROR`

- 🧐 **Cause**: A general GraphQL processing error occurred during an app-related operation; used as a fallback when no more specific AppErrorCode applies.

- 💡 **Debugging Tips**:
  - Check the error message for details from the underlying GraphQL error.
  - Review the API request and context in which the error occurred.
