...
- `webhooks`: List of webhooks that will be set. For details, please see [asynchronous webhooks](developer/extending/apps/asynchronous-webhooks.mdx), [synchronous-webhooks](developer/extending/apps/synchronous-webhooks/key-concepts.mdx) and [webhooks' subscription](developer/extending/apps/subscription-webhook-payloads.mdx) (be aware that subscription queries are required in manifest) sections.
