...
On `*_DETAILS_WIDGETS` mounts (for example, the product details sidebar), widgets are stacked in a scrollable column. From Dashboard 3.23.7 onward, an app can report its content height so each iframe grows or shrinks with the UI instead of staying in a fixed box — see [Sizing sidebar widgets](/developer/extending/apps/developing-apps/app-sdk/app-bridge#sizing-sidebar-widgets) (`@saleor/app-sdk` 1.9+).
