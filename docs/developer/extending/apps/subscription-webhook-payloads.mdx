...
To build a subscription query you can use any GraphQL interface (like [GraphQL Playground](api-usage/playground.mdx)). Just like with building a GraphQL query, you can use prompts and build the whole subscription from our API. Payload generation will use resolvers/permissions that are normally used in our API.
Inside the query, you must define the event type for which the payload will be generated. The event type must be supported by our subscription mechanism (see the [list of supported events](api-reference/unions/event.mdx)).
