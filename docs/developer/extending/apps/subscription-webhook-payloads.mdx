...
building a GraphQL query you will be able to use prompts from our API.
...
Events that are currently subscribable:
LINK
:::warning
...
Since subscriptions uses Saleor API to define webhook payload, the App that has the webhook subscribed needs all the permissions that would normally be required when querying for specified data.
