...
When enabling payment webhooks, first you need to subscribe to the `PAYMENT_LIST_GATEWAYS` event. This event is called whenever Saleor lists available payment gateways, either in the API or internal use. This webhook needs to return information about payment methods that are defined in your app.
...
You can also query for the available payment gateways in the context of a specific checkout. This is useful if payment methods depend on the current checkout (e.g. user’s shipping country etc.)
...
- `PAYMENT_PROCESS` - when the `checkoutComplete` mutation is run for the first time, payment is created and waits for the user's confirmation.
- `PAYMENT_CONFIRM` - when the `checkoutComplete` mutation is run for the second time, payment is confirmed.
  Depending on a payment gateway, multiple events can be triggered.
...
Read more about checkout mutations [here](developer/checkout.mdx).
