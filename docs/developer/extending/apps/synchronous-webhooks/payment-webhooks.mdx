...
First, you need to create an App to react to payment webhooks. You can read more about apps [here](developer/extending/apps/index.mdx).
...
The response fields match the `GatewayResponse` data class used in Saleor to represent a response from a payment gateway; you can read more about these fields [here](/developer/extending/payment-gateways.mdx#gatewayresponse).
