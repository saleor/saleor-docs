...
No matter which way of installing you choose, if your App provides a `tokenTargetUrl` in its manifest, it must expose the registration endpoint. Saleor Core will contact this endpoint during installation and send the auth token. If your application returns an HTTP status 200, installation is marked as successful, and staff users will be able to interact with it. If `tokenTargetUrl` is not set, Saleor skips the token exchange step and the app is installed without receiving an auth token.
...
:::info
The token exchange step (POST to `tokenTargetUrl`) is only performed when `tokenTargetUrl` 
is provided in the app manifest. If omitted, Saleor completes the installation 
without sending an auth token to the app.
:::
