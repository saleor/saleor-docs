...
Imagine you have an app that notifies an external service whenever a new order is created. The app registers the `ORDER_CREATED` webhook based on the [subscription webhook payload](/developer/extending/webhooks/subscription-webhook-payloads) from the `OrderCreatedSubscription.graphql` file:
...
The subscription dictates what data will be sent to the app along with the webhook. The app can then execute logic utilizing the subscription payload. In our case, it will be notifying the external service about the new order:
...
Whenever your app starts using a new field from the subscription, you must update the app's [webhook `query` field](../../../api-reference/webhooks/inputs/webhook-update-input#code-style-fontweight-normal-webhookupdateinputbquerybcodestring-).
:::
...
If you are using [`@saleor/app-sdk`](/developer/extending/apps/developing-apps/app-sdk/overview) for app development, you can use some of its helpers in your migration script.
...
Where you get `authData` will depend on your [APL](/developer/extending/apps/developing-apps/app-sdk/apl) implementation.
...
A production-ready migration process can be more extensive. For example, you can first deactivate the old webhook, create a new one, confirm it's working, and only then remove the old webhook. This gives you option to roll back the migration if something goes wrong.

If you need the full picture view on migration scripts, feel free to peak into [saleor/apps repository](https://github.com/saleor/apps/tree/main/apps/taxes/scripts/migrations).
