...
sidebar_label: Asynchronous webhooks
---
...
- send order confirmation to company chat.
- synchronize stocks with an external warehousing system.
- update Elasticsearch or Algolia index after product description change.
- trigger frontend SSR for new pages.

## Permissions

Managing webhooks is available for users with `MANAGE_APPS` permission. The app can also create and modify its own webhooks without the need for additional permissions.
...
Protocol is chosen based on `targetUrl` scheme.
...
If success response is not received, it will retry its request after a delay time in case of temporary service unavailability. 
