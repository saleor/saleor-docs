 ...
 * if `secretKey` wasn't set for a webhook - the RS256 signature based on our private key used also to sign JWT tokens and the payload (available since Saleor 3.5, before payload from webhooks without `secretKey` set were not signed)
