...
Asynchronous webhooks can be used for reacting to the events emitted by the Saleor. Worker instance will send event payload via the chosen protocol. Your app should acknowledge that payload was received. The result of app operations is not sent back to the Saleor. If you need to send data back to the user (for example, calculated shipping cost), you should use [synchronous webhook](developer/extending/apps/synchronous-webhooks/key-concepts.mdx).
