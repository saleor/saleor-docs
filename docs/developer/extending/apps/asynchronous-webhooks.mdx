 ...
 * if `secretKey` was set for a webhook - the HMAC sha256 header based on it and the payload
 * if `secretKey` wasn't set for a webhook - the RS256 signature based on our private key used also to sign JTW tokens and the payload (available since Saleor 3.5, before payload from webhooks without `secretKey` set were not signed)
...
- Saleor-Signature - defines signature to indicate that the request is veriferable
