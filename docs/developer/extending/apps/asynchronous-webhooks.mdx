 ...
 * @param {string} args.payload Raw body
 */
function verifyWebhook(args) {
  return (
    crypto
      .createHmac("sha256", process.env.SALEOR_APP_WEBHOOK_SECRET_KEY)
      .update(args.payload)
      .digest("hex") === args.signature
  );
}
```
