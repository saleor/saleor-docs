...
- If your payment provider requires additional operation before the payment is even initialized (for example, to render the drop-in), you might be interested in reading about the [`PAYMENT_GATEWAY_INITIALIZE_SESSION`](developer/payments/overview.mdx#initialize-payment-gateway) webhook.
