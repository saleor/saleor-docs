...
`PRODUCT VARIANT BACK IN STOCK` is trigger always when any product variant with previously non-available stock,
 will get new supply in any warehouse.
...
`PRODUCT VARIANT OUT OF STOCK` is trigger always when any product variant will have no available stock for any warehouse.
