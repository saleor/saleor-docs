...
`TRANSACTION_ACTION_REQUEST` is called after requesting a change in transaction state (called by staff user or by an app). 
The `action` object contains all details related to the requested action. 
The `transaction` contains all details related to `TransactionItem` object for which the requested action should be called.
