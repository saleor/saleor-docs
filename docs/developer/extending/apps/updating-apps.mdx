...
Currently, Saleor doesn't support automatic apps updates. Which means if the permissions, webhooks or any other manifest field changes, they will not be automatically fetched nor reflected in Saleor.

However, there are a few ways how to update the app manually, after it is installed:

### Reinstall the app

Reinstalling the app is probably the fastest way to update the app's manifest. To do that, you need to first remove the old one, then install it again.
...
- App must support multiple installations - the new token will be generated and app must be able to store it.
- If app stores it's settings in the metadata (`app.privateMetadata`), it will be removed and app must be configured again.
...
You can use this mutation to update these fields after the app was installed
