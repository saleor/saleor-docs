...
Synchronous webhooks for taxes allow Saleor Apps to control tax calculation for both checkouts and orders. Tax apps can also influence how checkout/order-level discounts are distributed among individual lines.

### Configure tax codes

Saleor Apps can subscribe to the `FETCH_TAX_CODES` synchronous webhook. The response from the webhook will populate the list of available tax codes in the Saleor Dashboard.

Tax codes can be configured for products and product types. The app-specific tax code is stored in the metadata of the corresponding object. In order to avoid collisions, Saleor stores it under keys of the format `{app.identifier}.code` and `{app.identifier}.description`.

The easiest way is to edit those tax codes using your Saleor Dashboard, although apps can use metadata mutations to build their own interface for configuring tax codes.
...
Taxes are calculated based on the discounted base prices. The sync webhooks are called once shipping methods are resolved (including shipping webhooks described above) and once discounts are calculated.

Please note that Saleor does not push checkout/order-level discounts onto individual lines. It is a responsibility of the app to decide how the discount should affect individual lines/

For checkouts, calculated taxes are cached for 1 hour or until invalidated by a mutation that might affects base prices, discounts or the billing/shipping address.

For orders, calculated taxes are cached until invalidated by triggering a mutation.
...
Example request for `CHECKOUT_CALCULATE_TAXES`:

```json
```

Example request for `ORDER_CALCULATE_TAXES`:

```json
[
  {
    "type": "Order",
    "id": "T3JkZXI6MDVhNzI2ODktMDFkMS00YTRhLTkyZjYtODE5MWJmOGY1YjFh",
    "channel": {
      "type": "Channel",
      "id": "Q2hhbm5lbDox",
      "slug": "default-channel",
    ...
    "shipping_address": {
      ...
      "last_name": "",
      "company_name": "",
      "street_address_1": "Manhattan Lane",
      "street_address_2": "",
      "city": "NEW YORK",
      "city_area": "",
      "postal_code": "10001",
    ...
    "discounts": null,
    "token": "05a72689-01d1-4a4a-92f6-8191bf8f5b1a",
    "original": "T3JkZXI6Tm9uZQ==",
    "lines": [
      {
        "type": "OrderLine",
        "id": "T3JkZXJMaW5lOjQ5",
        "product_variant_id": "UHJvZHVjdFZhcmlhbnQ6MjAz",
        "allocations": [],
        "charge_taxes": true,
        "product_metadata": {
          "vatlayer.code": "standard",
          "vatlayer.description": "standard",
          "io.saleor.taxjar.code": "50202306A0002",
          "io.saleor.taxjar.description": "Soft Drinks - Carbonated - 10-19% fruit juice"
        },
        "product_type_metadata": {
          "vatlayer.code": "standard",
          "vatlayer.description": "standard",
          "io.saleor.taxjar.code": "50202306A0001",
          "io.saleor.taxjar.description": "Soft Drinks - Carbonated - 1-9% fruit juice"
        },
        "unit_price_base_amount": "3.50",
        "total_price_base_amount": "3.50",
        "undiscounted_unit_price_base_amount": "5.00",
        "undiscounted_total_price_base_amount": "5.00",
        ...
        "quantity": 1,
        "currency": "USD",
        "unit_discount_amount": "1.50",
        "unit_discount_type": "fixed",
        "unit_discount_reason": "Sale: U2FsZTo1",
        "voucher_code": null,
        "sale_id": "U2FsZTo1"
      }
    ],
    "included_taxes_in_prices": false,
    "shipping_price_base_amount": "59.17",
    "total_base_amount": "62.67",
    "undiscounted_total_base_amount": "66.17",
    "fulfillments": [],
    "collection_point": null,
    "meta": {
      "issued_at": "2022-04-03T20:26:21.430220+00:00",
      "version": "dev",
      "issuing_principal": {
        "id": "VXNlcjox",
        "type": "user"
    ...
    "private_metadata": {},
    "metadata": {},
    "created": "2022-04-03T20:21:25.805Z",
    "status": "draft",
    "user_email": "",
    "origin": "draft",
...
{
  "currency": "USD",
  "total_net_amount": "62.67",
  "total_gross_amount": "67.92",
  "subtotal_net_amount": "3.50",
  "subtotal_gross_amount": "3.50",
  "shipping_price_net_amount": "59.17",
  "shipping_price_gross_amount": "64.42",
  "shipping_tax_rate": "0.08875",
  "lines": [
    {
      "id": 49,
      "currency": "USD",
      "tax_rate": "0.08875",
      "unit_net_amount": "3.50",
      "unit_gross_amount": "3.50",
      "total_net_amount": "3.50",
      "total_gross_amount": "3.50"
    }
  ]
}
```