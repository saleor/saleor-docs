...
2. Inside it, find the `extensions` field. Then, add a new extension to it as follows:
...
- The array contains an object with five fields:
  ...
  - `permissions`: It allows blocking access from the extension based on permissions. If the users don't have the right permissions, they won't be able to display the extension. The full list of permissions is [here](developer/permissions.mdx#app-permissions).
