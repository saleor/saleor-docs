...
[The App Manifest](developer/extending/apps/manifest.mdx) serves as a source of information about the Saleor app. It includes the app name, description, URL, as well as configuration for webhooks and extensions. The data is then consumed by other Saleor parties such as the API and the Dashboard.
