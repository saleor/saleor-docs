...
All communications with customers are processed by the communication interface build on PluginsManager.

The current flow allows staff users to integrate any notification system. PluginsManager has defined notification events that are triggered by the system when the expected action occurred. The manager will call plugins that define the given event. The notification payload will be passed as a method's argument.

The communication interface implements a webhook event `NOTIFY`. Any App which subscribes to it will receive an event type and communication payload for each communication action. 

Saleor has implemented given events: 
`account_confirmation`, `account_password_reset`, `account_change_email_request`, `account_change_email_confirm`, `account_delete`, `account_set_customer_password`, `invoice_ready`, `order_confirmation`, `order_confirmed`, `order_fulfillment_confirmation`, `order_fulfillment_update`, `order_payment_confirmation`, `order_canceled` , `order_refund_confirmation`
