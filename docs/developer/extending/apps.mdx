...
### Difference between `create` and `install`
- `create`: create a custom App instance, assign explicitly provided permissions. 
The app is fully manageable. You can change assigned permissions, add webhooks, or auth_token. 
`create` can be useful for creating integration with some services which will parse the webhook payload.
- `install`: create App instance and prepare communication channel. 
This approach uses a defined App manifest to prepare all required objects. 
This approach is recommended for creating integration with a service dedicated to Saleor. 
Installed App can be activated, deactivated, deleted but cannot be modified by using a mutation dedicated to `create` actions. 
...
`appFetchManifest` mutation fetches manifest from provided `manifestUrl`. Saleor downloads, parses and returns data as a `Manifest` object.
This mutation is only available to staff users with the `MANAGE_APPS` permission.

```graphql
mutation{
  appFetchManifest(manifestUrl:"http://localhost:3000/manifest"){
    manifest{
      identifier
      version
      about
...
If there are no errors in the response, a manifest object is returned:
```json
{
  "data": {
    "appFetchManifest": {
      "manifest": {
        "identifier": "example.app.wonderful",",
        "version": "1.0.0",
        "about": "My Wonderful App is a wonderful App for Saleor.",
        "name": "My Wonderful App",
