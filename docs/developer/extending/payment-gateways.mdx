...
All of the plugins method takes `previous_value` as a second argument.
More information about `previous_value` can be found [here](plugins#base-plugin-class).

For gateway to work correctly you also need to specified `Supported currencies` in gateway configuration.
...
| `supported_currencies` | `str`  | Define currencies which are supported by gateway                                                         |
