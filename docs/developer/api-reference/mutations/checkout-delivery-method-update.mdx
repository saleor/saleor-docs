---
id: checkout-delivery-method-update
title: checkoutDeliveryMethodUpdate
---

New in Saleor 3.1. Updates the delivery method (shipping method or pick up point) of the checkout.

```graphql
checkoutDeliveryMethodUpdate(
  deliveryMethodId: ID
  token: UUID
): CheckoutDeliveryMethodUpdate

```

### Arguments

#### `deliveryMethodId` ([`ID`](/docs/developer/api-reference/scalars/id))

Delivery Method ID (`Warehouse` ID or `ShippingMethod` ID).

#### `token` ([`UUID`](/docs/developer/api-reference/scalars/uuid))

Checkout token.

### Type

#### [`CheckoutDeliveryMethodUpdate`](/docs/developer/api-reference/objects/checkout-delivery-method-update)

New in Saleor 3.1. Updates the delivery method (shipping method or pick up point) of the checkout.
