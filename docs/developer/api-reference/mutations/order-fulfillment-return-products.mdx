---
id: order-fulfillment-return-products
title: orderFulfillmentReturnProducts
---

Return products.

```graphql
orderFulfillmentReturnProducts(
  input: OrderReturnProductsInput!
  order: ID!
): FulfillmentReturnProducts

```

### Arguments

#### `input` ([`OrderReturnProductsInput!`](/docs/developer/api-reference/inputs/order-return-products-input))

Fields required to return products.

#### `order` ([`ID!`](/docs/developer/api-reference/scalars/id))

ID of the order to be returned.

### Type

#### [`FulfillmentReturnProducts`](/docs/developer/api-reference/objects/fulfillment-return-products)

Return products.
