...
Instead of assigning permissions directly to the user, we define them on a group basis. Organizing access rights in [Groups](api-reference/objects/group.mdx) helps in determining the roles of team members.
...
To create a new group, use the [permissionGroupCreate](api-reference/mutations/permission-group-create.mdx) mutation.
...
To remove a group, use the [permissionGroupDelete](api-reference/mutations/permission-group-delete.mdx) mutation:
...
The [permissionGroupUpdate](api-reference/mutations/permission-group-update.mdx) mutation takes a list of user IDs you would like to add or remove from the group. Having the same user in both lists will result in an error.
...
App permissions are defined on a per-app basis. Access can be assigned during the app installation and modified later using the [appUpdate](api-reference/mutations/app-update.mdx) mutation.
...
Available permissions are kept in the [PermissionEnum](api-reference/enums/permission-enum.mdx).
