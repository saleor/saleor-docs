...
Instead of assigning permissions directly to the user, we define them on a group basis. Organizing access rights in [Groups](developer/api-reference/objects/group.mdx) helps in determining the roles of team members.
...
The [permissionGroupUpdate](developer/api-reference/mutations/permission-group-update.mdx) mutation takes a list of user IDs you would like to add or remove from the group. Having the same user in both lists will result in an error.
...
App permissions are defined on a per-app basis. Access can be assigned during the app installation and modified later using the [appUpdate](developer/api-reference/mutations/app-update.mdx) mutation.
