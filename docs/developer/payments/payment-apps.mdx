...
Saleor then communicates with the app using synchronous webhooks, automatically [updates statuses and balances](developer/payments/overview.mdx#recalculations-of-transaction-amounts), and returns the response to the storefront. Checkout is finalized with the [`checkoutComplete`](api-reference/checkout/mutations/checkout-complete.mdx) mutation.
