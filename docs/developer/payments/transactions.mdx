...
The pspReference value on a `TransactionItem` can initially be `None`.
...
If `pspReference` is missing, the event is not considered in transaction amount recalculations.
