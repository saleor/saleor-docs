...
The [`transactionCreate`](/api-reference/payments/mutations/transaction-create.mdx) can only be called by staff users or apps with the [`HANDLE_PAYMENTS`](/api-reference/users/enums/permission-enum.mdx#handle-payments) permission.

:::

The following example shows how you can use the [`transactionCreate`](/api-reference/payments/mutations/transaction-create.mdx) mutation to create a new transaction.
...
- Transactions attached to the checkout are accessible via the [`checkout.transactions`](/api-reference/checkout/objects/checkout.mdx#transactions) field.
- Transactions attached to order are accessible via the [`order.transactions`](/api-reference/orders/objects/order.mdx#transactions) field.
...
[HANDLE_PAYMENTS](/api-reference/users/enums/permission-enum.mdx#handle-payments) permission
or by the App that created the the transaction and has [`HANDLE_PAYMENTS`](/api-reference/users/enums/permission-enum.mdx#handle-payments) permission.
...
The [`transactionEventReport`](/api-reference/payments/mutations/transaction-event-report.mdx) can only be called by staff users with
[`HANDLE_PAYMENTS`](/api-reference/users/enums/permission-enum.mdx#handle-payments) permission
or by the App that created the transaction and has [`HANDLE_PAYMENTS`](/api-reference/users/enums/permission-enum.mdx#handle-payments) permission.

:::

The following example shows how the [transactionEventReport](/api-reference/payments/mutations/transaction-event-report.mdx) mutation is used to report an event
...
The provided data will be used to create a new [TransactionEvent](/api-reference/payments/objects/transaction-event.mdx) object that will be included in the recalculation process.
...
- **Other event has the same `pspReference` but different amount**: [`INCORRECT_DETAILS`](/api-reference/payments/enums/transaction-event-report-error-code.mdx#incorrect-details) error will be raised
- **`AUTHORIZATION_SUCCESS` event already exists with different `amount` or with different `pspReference`**: [`ALREADY_EXISTS`](/api-reference/payments/enums/transaction-event-report-error-code.mdx#already-exists) error will be raised
  :::
...
- [`AUTHORIZATION_SUCCESS`](/api-reference/payments/enums/transaction-event-type-enum#authorization-success),
- [`AUTHORIZATION_ADJUSTMENT`](/api-reference/payments/enums/transaction-event-type-enum#authorization-adjustment),
- [`AUTHORIZATION_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum#authorization-request),
- [`CHARGE_ACTION_REQUIRED`](/api-reference/payments/enums/transaction-event-type-enum#charge-action-required),
- [`CHARGE_SUCCESS`](/api-reference/payments/enums/transaction-event-type-enum#charge-success),
- [`CHARGE_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum#charge-request),
- [`REFUND_SUCCESS`](/api-reference/payments/enums/transaction-event-type-enum#refund-success),
- [`REFUND_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum#refund-request),
- [`CANCEL_SUCCESS`](/api-reference/payments/enums/transaction-event-type-enum#cancel-success),
- [`CANCEL_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum#cancel-request).
...
- [`transactionRequestAction`](/api-reference/payments/mutations/transaction-request-action.mdx): will also create a new
  [`TransactionEvent`](/api-reference/payments/objects/transaction-event.mdx) with one of the request type ([`AUTHORIZATION_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum.mdx#authorization-request),
  [`CHARGE_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum.mdx#charge-request),
  [`REFUND_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum.mdx#refund-request),
  [`CANCEL_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum.mdx#cancel-request)),
  `amount` and the `owner` (User or App). Saleor will send a synchronous webhook dedicated to the action
  [`TRANSACTION_CHARGE_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-charge-requested),
  [`TRANSACTION_CANCELATION_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-cancelation-requested),
  [`TRANSACTION_REFUND_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-refund-requested)

- [`transactionRequestRefundForGrantedRefund`](/api-reference/payments/mutations/transaction-request-refund-for-granted-refund.mdx): will create a new [`TransactionEvent`](/api-reference/payments/objects/transaction-event.mdx) with
  [`REFUND_REQUEST`](/api-reference/payments/enums/transaction-event-type-enum.mdx#refund-request) type, `amount` and the `owner` (User or App).
  Saleor will send a synchronous webhook [`TRANSACTION_REFUND_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-refund-requested).
...
can accept action details in the response: [`TRANSACTION_CHARGE_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-charge-requested),
[`TRANSACTION_CANCELATION_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-cancelation-requested),
[`TRANSACTION_REFUND_REQUESTED`](/api-reference/webhooks/enums/webhook-event-type-sync-enum.mdx#transaction-refund-requested).
...
contain a `pspReference` field, which refers to the payment service provider reference. - The `pspReference` on `TransactionItem` identifies the overall payment. - The `pspReference` on `TransactionEvent` identifies specific actions that occurred within the `Transaction`.
...
When a `TransactionEvent` is reported: 1. **Event Type Check:** If the event type is `AUTHORIZATION_ACTION_REQUIRED`, `CHARGE_ACTION_REQUIRED`, or `INFO`, a new event is always created. 2. **Existing Event Check:** For other types, the system searches for an existing event with the same `type` and `pspReference` assigned to the `TransactionItem` requested in the input. - **Amount Match:** If an existing event is found, the system compares its amount to the reported event’s amount. - If the amounts match, the existing event is reused, and the `transactionEvent.alreadyProcessed` value is set to `True`.
For more information about this field, refer to the [Reporting actions for transactions](#reporting-actions-for-transactions). - If they do not match, an error is raised. - **No Existing Event:** If no matching event is found, a new one is created, except for `AUTHORIZATION_SUCCESS`, which must be unique, attempting to create a duplicate raises an error.
...
from all transactions connected to the `Checkout` covers the [`checkout.totalPrice`](/api-reference/checkout/objects/checkout.mdx#total-price).
...
See the [3.21 to 3.22 upgrade guide](/upgrade-guides/core/3-21-to-3-22.mdx#automatic-checkout-completion-behavior-change-for-checkouts-with-total-being-0) for more details.
:::
