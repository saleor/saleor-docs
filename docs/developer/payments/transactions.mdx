...
:::danger
**One this setting is enabled, you should verify if your connected services, including storefront, can handle this flow.**

Please remember, that if setting is enabled:
- `Checkout` will be deleted after checkout completion, which means any query or mutation relying on it will no longer work after that
