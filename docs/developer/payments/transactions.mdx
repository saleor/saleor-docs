...
The Checkout can be automatically converted into an [`Order`](/api-reference/orders/objects/order.mdx), once the [`TransactionItems`](/api-reference/payments/objects/transaction-item.mdx) attached to the Checkout are fully paid.

:::caution
This feature won't work with legacy [Payments API](/developer/extending/payment-gateways.mdx) that use [plugins](/developer/extending/plugins/overview.mdx) (instead of apps). For example this setting won't work with [Stripe plugin](/developer/app-store/plugins/stripe)
:::

A checkout is considered fully paid when the [`Checkout.authorizeStatus`](/developer/payments/lifecycle.mdx#authorize-status) 
is set to `FULL`.
...
:::danger
**Enabling automatic checkout might disrupt storefront functionality!**

Completing checkout deletes the `Checkout` object, which means any query or mutation relying on it will no longer work after completion.
:::


:::tip
Keep in mind that if `checkoutComplete` is called for a `Checkout` that has already been converted to an `Order`,
the corresponding `Order` instance will be returned.

This way you can get details of new `Order` object, after checkout it automatically completed.
:::

:::warning
Checkout will be automatically completed even if some Transactions are still pending and waiting for confirmation from payment provider. Checkouts are considered "paid" as long as the total amount is covered (see when `authorizeStatus` is set to `FULL` in [payment lifecycle docs](/developer/payments/lifecycle.mdx#authorize-status)).
