...
:::caution
This feature isn't compatible with [legacy Payments API](/developer/extending/payment-gateways.mdx) that uses [plugins](/developer/extending/plugins/overview.mdx) (instead of apps). For example this setting won't work with [Stripe plugin](/developer/app-store/plugins/stripe).
