...
#### Automatic Checkout Completion Settings Details

The `automatic_completion` field in `CheckoutSettingsInput` provides the following options for configuring automatic checkout completion:
...
- **`delay`**: An optional integer value (in minutes) that specifies the delay before a checkout is automatically completed. If not provided, the default delay is 30 minutes.
...
To enable automatic checkout completion, update the channel checkout settings.
For example with a `delay` and cut-off date:
```graphql
mutation {
  channelUpdate(
    id: "Q2hhbm5lbDox",
...
1. Checkouts that have never had an automatic completion attempt.
2. Checkouts with the oldest `last_change` field.

Checkouts modified more than 30 days ago are excluded from automatic completion.
:::


A checkout is considered fully covered when the [`Checkout.authorizeStatus`](/developer/payments/lifecycle.mdx#authorize-status) is set to `FULL`. This occurs when the total of `authorizedAmount`, `chargedAmount`, `authorizePendingAmount`, and `chargePendingAmount` from all transactions connected to the `Checkout` covers the [`checkout.totalPrice`](/api-reference/checkout/objects/checkout#checkoutsubtotalpricetaxedmoney---).
  ...
  "input": {
    "checkoutSettings": {
      "automaticCompletion": {
        "enabled": true,
        "delay": 60,
        "cutOffDate": "2025-11-19T00:00:00Z"
      }
    }
  }
}
...
:::danger
**Ensure your connected services, including storefronts, can handle this flow before enabling this setting.**

When enabled:
- The `Checkout` will be deleted after completion, making any queries or mutations relying on it invalid.
- Running `checkoutComplete` on an already completed `Checkout` will return the previously created `Order` object.
:::

:::caution
Checkouts will be automatically completed even if some transactions are still pending confirmation from the payment provider. Pending transactions might later be rejected, resulting in an unpaid order. Use the [Order Expiration feature](/developer/order/order-expiration.mdx) to handle unpaid orders.

Additionally, once a `Checkout` is converted into an `Order`, the `authorizeStatus` might change from `FULL` to `NONE` or `PARTIAL` due to differences in calculation logic between `Checkout` and `Order` statuses.
:::
