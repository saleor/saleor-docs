...
- **Per-line `reasonReference` is always optional** — for both staff and apps. Lines without it are stored with `null`. There is no inheritance from the global level.
- **Apps** can omit `reasonReference` at any level, even when configuration requires it.
- When `reasonReference` is provided (global or per-line), the referenced `Page` must belong to the `PageType` set in the `returnReasonReferenceType` setting. Otherwise, a `ValidationError` is raised.
