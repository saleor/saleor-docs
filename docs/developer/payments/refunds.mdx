...
mutation {
  transactionRequestAction(
    ...
    reasonReference: "xXyzaXdaxbX" # ID of a model representing the reason for the refund

  ) {
    transaction {
      events {
        id
        message # Will contain the "reason" value
        pspReference
        amount {
          amount
        }
...
- `reason` - Optional plain text reason for the refund. Will be populated to the `TransactionEvent.message` field.
- `reasonReference` - The ID of a model representing the reason for the refund. This is useful for tracking and reporting purposes, like aggregating refunds by specific type.
...
While `reason` field is optional and remains a plain text field, the `reasonReference` can be configured:
- Require staff user to pass `reasonReference` for all refunds.
- Make `reasonReference` optional

To change the settings, you can either use the Dashboard or a mutation

```graphql
mutation UpdateRefundSettings($id: ID) {
...
- input `reasonReferenceType` - the ID of a Model Type. All models that are instances of this type will be available to reference in the reasons.

Mutation returns `RefundSettings` object with `reasonReferenceType: PageType` if set.

Once `reasonReferenceType` is set, staff users will have to provide the `reasonReference` field when processing refunds.
The `reasonReference` will be validated against the provided type, and if it does not match, an error will be returned.

However, apps can still process refunds without providing the `reasonReference` field, as it is not a mandatory field for apps.

To disable the requirement, pass `reasonReferenceType: null` in the mutation input.