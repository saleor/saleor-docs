...
- `input`:
  - `lines` - List of lines related to the planned refund action:
    ...
    - `reasonReference` - ID of the `Page` representing the reason for the refund **for this specific line**. When provided, the Page must match the `PageType` set in refund settings. See [configuration](#refund-reasons-configuration) to learn more. Added in Saleor 3.23.
  ...
  - `reasonReference` - Order-level `Page` ID representing the reason for the refund. See [configuration](#refund-reasons-configuration) to learn more.
...
Both order-level and per-line `reasonReference` follow the same validation rules: required for staff users when the `refundReasonReferenceType` is configured, optional for apps. A line-level `reasonReference` overrides the order-level one for that specific line.
...
- `input`:
  - `addLines` - Lines that should be added to `OrderGrantedRefund`:
    ...
    - `reasonReference` - ID of the `Page` representing the reason for the refund **for this specific line**. Same validation rules as `orderGrantRefundCreate`. See [configuration](#refund-reasons-configuration) to learn more. Added in Saleor 3.23.
...
input OrderReturnProductsInput {
  ...
  refund: Boolean
  reason: String            # NEW in 3.22 - global return reason text
  reasonReference: ID       # NEW in 3.22 - global Page reference
}
...
input OrderReturnFulfillmentLineInput {
  ...
  quantity: Int!
  replace: Boolean
  reason: String            # NEW in 3.22 - per-line return reason text
  reasonReference: ID       # NEW in 3.22 - per-line Page reference
}
```
...
type Fulfillment {
  # ... existing fields ...
  reason: String            # NEW in 3.22
  reasonReference: Page     # NEW in 3.22
}
...
- `input`:
  - `reason` - Global free-text reason for the return. Stored on the resulting `Fulfillment`.
  - `reasonReference` - Global `Page` ID representing the reason for the return. Must match the `PageType` set in refund settings. Stored on the resulting `Fulfillment`. See [configuration](#refund-reasons-configuration) to learn more.
...
- When `refundReasonReferenceType` is configured and the caller is a **staff user**, `reasonReference` is required (either at global or per-line level).
- **Apps** can omit `reasonReference` even when configuration requires it.
- When `reasonReference` is provided, the referenced `Page` must belong to the `PageType` set in the `refundReasonReferenceType` setting. Otherwise, a `ValidationError` is raised.
- **When `refund` is `true`**, reason validation on the return input is skipped. In this case, the refund reason should be provided separately via `orderGrantRefundCreate` after the return is processed.

### Example: Return with refund (no reason validation)

When `refund: true`, the mutation processes a return and triggers a refund simultaneously. In this flow, reason references are **not validated** on the return input since the refund reason is expected to be tracked on the `OrderGrantedRefund` instead.
...
:::note
Available from 3.22. Per-line reason references for grant refund mutations and return/replace reasons added in 3.23.
:::
...
Now, every time a *staff user* tries to create a refund or return, they will be required to provide a reason reference. The existing free-text "reason" field will still be available, but it will be optional.
...
The following mutations support `reasonReference` when the refund reason type is configured:
