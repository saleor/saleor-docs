...
- When `refundReasonReferenceType` is configured and the caller is a **staff user**, the **global** `reasonReference` is required.
...
- When `reasonReference` is provided (global or per-line), the referenced `Page` must belong to the `PageType` set in the `refundReasonReferenceType` setting. Otherwise, a `ValidationError` is raised.
