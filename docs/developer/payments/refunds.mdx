...
```json
{
  ...
  "refundReasonReference": "UGFnZTox"  # ID of a model representing the reason for the refund
}
```
</TabItem>
</Tabs>
...
Note that permitted `App` will be able to omit this setting. For the *staff user*, the ValidationError will be raised if the reason reference is not provided. App can omit this field for more powerful workflows.
