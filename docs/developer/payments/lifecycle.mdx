...
:::caution
`paymentStatus` is a legacy field inherited from the old Payments API (payment plugins) 
It attempts to map `TransactionItem` state into a single combined field. 
It works correctly in most cases, but certain edge cases fall outside what a single field can accurately represent.

For a precise view of payment state, use [`authorizeStatus`](#authorize-status) and [`chargeStatus`](#charge-status) instead — they are calculated directly from `TransactionItem` amounts.
:::
...
[`Order.paymentStatus`](/api-reference/orders/objects/order.mdx#payment-status) attempts to consolidate both payment and refund state into a single field. It is calculated based on:

- `order.total` value
- status of legacy `Payment` objects assigned to the order
...
:::note
`Checkout` doesn't have a `paymentStatus` field.
:::
...
[`Checkout.authorizeStatus`](/api-reference/checkout/objects/checkout.mdx#authorize-status) depends on:
...
[`Order.authorizeStatus`](/api-reference/orders/objects/order.mdx#authorize-status) status depends on:
...
The status is calculated based on summing up all transactions assigned to the `Order` (`authorizedAmount` and `chargedAmount` on each `TransactionItem`) and by comparing it with the difference between [`total`](/api-reference/orders/objects/order.mdx#total) and [`totalGrantedRefund`](/api-reference/orders/objects/order.mdx#total-granted-refund).
...
[`Order.chargeStatus`](/api-reference/orders/objects/order.mdx#charge-status) depends on:
...
The status is calculated by summing up all transactions assigned to the `Order` (`chargedAmount` on each `TransactionItem`) and by comparing it with the difference between [`total`](/api-reference/orders/objects/order.mdx#total) and [`totalGrantedRefund`](/api-reference/orders/objects/order.mdx#total-granted-refund).
...
[`Checkout.chargeStatus`](/api-reference/checkout/objects/checkout.mdx#charge-status) depends on:
- sum of **charged** transactions (`TransactionItem.chargedAmount`)
- sum of transactions with **pending charge** (`TransactionItem.chargePendingAmount`)
- `checkout.totalPrice` value
