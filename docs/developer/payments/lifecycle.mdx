...
[`Order.paymentStatus`](/api-reference/orders/objects/order.mdx#paymentstatuspaymentchargestatusenum) represents the combined payment and refund status of the order. It's calculated based on:
...
[`Checkout.authorizeStatus`](/api-reference/checkout/objects/checkout#authorizestatuscheckoutauthorizestatusenum) depends on:
...
[`Order.authorizeStatus`](/api-reference/orders/objects/order#authorizestatusorderauthorizestatusenum) status depends on:
...
The status is calculated based on summing up all transactions assigned to the `Order` (`authorizedAmount` and `chargedAmount` on each `TransactionItem`) and by comparing it with the difference between [`total`](/api-reference/orders/objects/order.mdx#totaltaxedmoney) and [`totalGrantedRefund`](/api-reference/orders/objects/order.mdx#totalgrantedrefundmoney).
...
[`Order.chargeStatus`](/api-reference/orders/objects/order.mdx#chargestatusorderchargestatusenum) depends on:
...
[`Checkout.chargeStatus`](/api-reference/checkout/objects/checkout.mdx#chargestatuscheckoutchargestatusenum) depends on:
- sum of **charged** transactions (`TransactionItem.chargedAmount`)
- sum of transactions with **pending charge** (`TransactionItem.chargePendingAmount`)
- `checkout.totalPrice` value
