...
- when handling incorrect response from [sync webhook sent to payment app](/developer/extending/webhooks/synchronous-events/transaction.mdx)
...
- When app sends response to any of the [Transaction synchronous events](/developer/extending/webhooks/synchronous-events/transaction.mdx)
  - Saleor will either fill missing `pspReference` on the automatically created event that already exists
  - or will create a new event if it has different `type` (e.g. user requested `AUTHORIZATION`, but app returned `CHARGE_SUCCESS` - we'll have two events `AUTHORIZATION_REQUEST` with no `pspReference` and `CHARGE_SUCCESS`)
...
:::caution
Each event can be excluded from calculation if it has matching `_FAILURE` event with the same `pspReference` and newer `time`.
:::
...
- `authorizedAmount` - `AUTHORIZATION_SUCCESS` event amount
  ...
  - subtracting `canceledAmount`
- `authorizePendingAmount` - sum of `AUTHORIZATION_REQUEST` event amounts
- `chargedAmount` - sum of `CHARGE_SUCCESS` event amounts
  - and sum `REFUND_REVERSE` event amounts
  - subtracting all `CHARGE_BACK` events that lower this amount
  - subtracting `refundedAmount`
...
Each event is identified by `pspReference`. This field is used to de-duplicate events in case there were reported multiple times by the payment app.

For example if Saleor receives two `CHARGE_SUCCESS` events with the same `pspReference` and `amount` there will be only one event stored on Transaction. Second event will be treated as "already reported" and it won't be stored by Saleor.
...
:::tip
**Fix**: In order to change `authorizedAmount` value, payment app should send `AUTHORIZATION_ADJUSTMENT` event. If there are in fact two valid `AUTHORIZATION_SUCCESS` events, create new `TransactionItem`
:::
...
:::tip
**Fix**: Previous event must be invalidated via `_FAILURE` event, then report `_SUCCESS` event again
:::
