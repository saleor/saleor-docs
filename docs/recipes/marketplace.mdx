...
Saleor's permission system doesn't have a built-in vendor entity or vendor-based permissions, which means it has to be implemented separately. 

Our recommendation is to integrate OIDC provider with both Saleor and [vendor portals](#vendor-portal). You can integrate with services like Auth0 or KeyCloak. Choosing this approach has multiple advantages:

- **Single source of truth for permissions:** Saleor and portal users permissions will be managed with single service 
- **Single Sign-on (SSO):** using the same accounts to access Saleor Dashboard and portals
...
sequenceDiagram
    ...
    vendor_portal->>oidc: Checking vendor permissions
    oidc-->>vendor_portal: Permissions verified
    vendor_portal->>saleor: Product mutation using Vendor Portal token
