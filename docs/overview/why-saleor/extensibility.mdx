...
While Saleor's engine is written in Python, it makes almost no assumptions about your tech stack. Saleor's API and the extension mechanism utilize GraphQL transported over either HTTP or message queues. Hence, the only requirement we impose is that your code be able to make or receive HTTP requests.
...
Saleor's product type system allows you to create named schemas for different product types, each having a different set of fields of various types. This guarantees that your books always have a page count that is a number and that your drinks have a list of ingredients, each linked to its own entity.
...
It's unrealistic for Saleor to provide every possible feature and function. Conversely, it's very realistic for your business to have a unique need that is addressed by any of the available solutions.
