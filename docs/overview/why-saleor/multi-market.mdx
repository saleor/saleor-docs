...
Each channel can use a different currency to transact. Saleor supports all the ISO 4217 currency codes and knows that the Japanese _yen_ does not have a subunit (and thus no decimal places) and that the Iraqi _dinar_ splits into a thousand fils (and thus needs to be rounded to three decimal places).
