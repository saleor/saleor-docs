...
For example, you might want to combine independent vendors of `Content Management`, `Checkout API`, and `Order Management` that suit your business needs, commonly referred to as _best-of-breed_ architecture.
...
• Creation of order requires objects such as customers, products, and attributes to be present in the system.

• Creation of order has a rigid structure where status, date, or order number can not be altered.
...
• Price can not be calculated dynamically using external service; custom logic must be included in the backend as a plugin or extension.
...
• **Isolation of failures**. Issues of individual components less likely affect critical path.

### Our approach to composable

• **Batteries included.**
Even though our architecture interchangeable with other components, essential components for _Merchandising_ and _Operations_ included.

• **Balanced approach.** We strive to strike a balance between productivity and flexibility.
For example we provide opinionated product pricing model which aims to be useful for the majority of businesses as is, but you are also free to take full control over your pricing logic.
