---
api_reference: true
id: password-change
title: passwordChange
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import { useState } from "react";

export const Details = ({
  dataOpen,
  dataClose,
  children,
  startOpen = false,
}) => {
  const [open, setOpen] = useState(startOpen);
  return (
    <details
      {...(open ? { open: true } : {})}
      className="details"
      style={{
        border: "none",
        boxShadow: "none",
        background: "var(--ifm-background-color)",
      }}
    >
      <summary
        onClick={(e) => {
          e.preventDefault();
          setOpen((open) => !open);
        }}
        style={{ listStyle: "none" }}
      >
        {open ? dataOpen : dataClose}
      </summary>
      {open && children}
    </details>
  );
};

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

Change the password of the logged in user.

<Permissions
  permissions={"AUTHENTICATED_USER"}
  text={"Requires one of the following permissions"}
/>

```graphql
passwordChange(
  newPassword: String!
  oldPassword: String
): PasswordChange
```

<Details dataOpen="Hide return type" dataClose="Show return type">

```graphql
type PasswordChange {
  user: User
  accountErrors: [AccountError!]! @deprecated
  errors: [AccountError!]!
}
```

</Details>

### Arguments

#### [<code>newPassword</code>](#new-password)<Bullet />[<code>String!</code>](/api-reference/miscellaneous/scalars/string.mdx) \{#new-password\}

New user password.

#### [<code>oldPassword</code>](#old-password)<Bullet />[<code>String</code>](/api-reference/miscellaneous/scalars/string.mdx) \{#old-password\}

Current user password.

### Type

#### [<code>PasswordChange</code>](/api-reference/users/objects/password-change.mdx)

Change the password of the logged in user.

<Permissions
  permissions={"AUTHENTICATED_USER"}
  text={"Requires one of the following permissions"}
/>
