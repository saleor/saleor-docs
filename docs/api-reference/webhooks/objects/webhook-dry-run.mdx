...
#### [<code>payload</code>](#payload)<Bullet />[<code>JSONString</code>](/api-reference/miscellaneous/scalars/jsonstring.mdx) \{#payload\}

JSON payload, that would be sent out to webhook's target URL.

#### [<code>errors</code>](#errors)<Bullet />[<code>[WebhookDryRunError!]!</code>](/api-reference/webhooks/objects/webhook-dry-run-error.mdx) \{#errors\}
