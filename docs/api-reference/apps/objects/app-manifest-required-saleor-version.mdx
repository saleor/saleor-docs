---
api_reference: true
id: app-manifest-required-saleor-version
title: AppManifestRequiredSaleorVersion
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import { useState } from "react";

export const Details = ({
  dataOpen,
  dataClose,
  children,
  startOpen = false,
}) => {
  const [open, setOpen] = useState(startOpen);
  return (
    <details
      {...(open ? { open: true } : {})}
      className="details"
      style={{
        border: "none",
        boxShadow: "none",
        background: "var(--ifm-background-color)",
      }}
    >
      <summary
        onClick={(e) => {
          e.preventDefault();
          setOpen((open) => !open);
        }}
        style={{ listStyle: "none" }}
      >
        {open ? dataOpen : dataClose}
      </summary>
      {open && children}
    </details>
  );
};

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

No description

```graphql
type AppManifestRequiredSaleorVersion {
  constraint: String!
  satisfied: Boolean!
}
```

### Fields

#### [<code>constraint</code>](#constraint)<Bullet />[<code>String!</code>](/api-reference/miscellaneous/scalars/string.mdx) \{#constraint\}

Required Saleor version as semver range.

#### [<code>satisfied</code>](#satisfied)<Bullet />[<code>Boolean!</code>](/api-reference/miscellaneous/scalars/boolean.mdx) \{#satisfied\}

Informs if the Saleor version matches the required one.

### Member Of

[`Manifest`](/api-reference/apps/objects/manifest.mdx) <Badge class="badge badge--secondary badge--relation" text="object"/>
