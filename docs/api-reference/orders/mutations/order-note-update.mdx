---
api_reference: true
id: order-note-update
title: orderNoteUpdate
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import { useState } from "react";

export const Details = ({
  dataOpen,
  dataClose,
  children,
  startOpen = false,
}) => {
  const [open, setOpen] = useState(startOpen);
  return (
    <details
      {...(open ? { open: true } : {})}
      className="details"
      style={{
        border: "none",
        boxShadow: "none",
        background: "var(--ifm-background-color)",
      }}
    >
      <summary
        onClick={(e) => {
          e.preventDefault();
          setOpen((open) => !open);
        }}
        style={{ listStyle: "none" }}
      >
        {open ? dataOpen : dataClose}
      </summary>
      {open && children}
    </details>
  );
};

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

Updates note of an order.

<Permissions
  permissions={"MANAGE_ORDERS"}
  text={"Requires one of the following permissions"}
/>

```graphql
orderNoteUpdate(
  note: ID!
  input: OrderNoteInput!
): OrderNoteUpdate
```

<Details dataOpen="Hide return type" dataClose="Show return type">

```graphql
type OrderNoteUpdate {
  order: Order
  event: OrderEvent
  errors: [OrderNoteUpdateError!]!
}
```

</Details>

### Arguments

#### [<code>note</code>](#note)<Bullet />[<code>ID!</code>](/api-reference/miscellaneous/scalars/id.mdx) \{#note\}

ID of the note.

#### [<code>input</code>](#input)<Bullet />[<code>OrderNoteInput!</code>](/api-reference/orders/inputs/order-note-input.mdx) \{#input\}

Fields required to create a note for the order.

### Type

#### [<code>OrderNoteUpdate</code>](/api-reference/orders/objects/order-note-update.mdx)

Updates note of an order.

<Permissions
  permissions={"MANAGE_ORDERS"}
  text={"Requires one of the following permissions"}
/>
