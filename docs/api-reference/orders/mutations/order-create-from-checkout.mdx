...
#### [<code>removeCheckout</code>](#remove-checkout)<Bullet />[<code>Boolean</code>](/api-reference/miscellaneous/scalars/boolean.mdx) \{#remove-checkout\}

Determines if checkout should be removed after creating an order. Default true.

### Type

#### [<code>OrderCreateFromCheckout</code>](/api-reference/orders/objects/order-create-from-checkout.mdx)
