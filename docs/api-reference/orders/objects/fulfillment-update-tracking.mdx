---
api_reference: true
id: fulfillment-update-tracking
title: FulfillmentUpdateTracking
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

Updates a fulfillment for an order.

<Permissions
  permissions={"MANAGE_ORDERS"}
  text={"Requires one of the following permissions"}
/>

Triggers the following webhook events:

- FULFILLMENT_TRACKING_NUMBER_UPDATED (async): Fulfillment tracking number is updated.

```graphql
type FulfillmentUpdateTracking {
  fulfillment: Fulfillment
  order: Order
  orderErrors: [OrderError!]! @deprecated
  errors: [OrderError!]!
}
```

### Fields

#### [<code>fulfillment</code>](#fulfillment)<Bullet />[<code>Fulfillment</code>](/api-reference/orders/objects/fulfillment.mdx) \{#fulfillment\}

A fulfillment with updated tracking.

#### [<code>order</code>](#order)<Bullet />[<code>Order</code>](/api-reference/orders/objects/order.mdx) \{#order\}

Order for which fulfillment was updated.

#### [<code>errors</code>](#errors)<Bullet />[<code>[OrderError!]!</code>](/api-reference/orders/objects/order-error.mdx) \{#errors\}

<details class="graphql-markdown-details">
<summary>
<span class="graphql-markdown-details-label-closed">Show deprecated</span>
<span class="graphql-markdown-details-label-open">Hide deprecated</span>
</summary>

#### [<code>orderErrors</code>](#order-errors)<Bullet />[<code>[OrderError!]!</code>](/api-reference/orders/objects/order-error.mdx) \{#order-errors\}

:::warning[DEPRECATED]

Use `errors` field instead.

:::

</details>

### Returned By

[`orderFulfillmentUpdateTracking`](/api-reference/orders/mutations/order-fulfillment-update-tracking.mdx) <Badge class="badge badge--secondary badge--relation" text="mutation"/>
