---
api_reference: true
id: create-warehouse
title: createWarehouse
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

Creates a new warehouse.

<Permissions
  permissions={"MANAGE_PRODUCTS"}
  text={"Requires one of the following permissions"}
/>

```graphql
createWarehouse(
  input: WarehouseCreateInput!
): WarehouseCreate
```

<Details dataOpen="Hide return type" dataClose="Show return type">

```graphql
type WarehouseCreate {
  warehouseErrors: [WarehouseError!]! @deprecated
  errors: [WarehouseError!]!
  warehouse: Warehouse
}
```

</Details>

### Arguments

#### [<code>input</code>](#input)<Bullet />[<code>WarehouseCreateInput!</code>](/api-reference/products/inputs/warehouse-create-input.mdx) \{#input\}

Fields required to create warehouse.

### Type

#### [<code>WarehouseCreate</code>](/api-reference/products/objects/warehouse-create.mdx)

Creates a new warehouse.

<Permissions
  permissions={"MANAGE_PRODUCTS"}
  text={"Requires one of the following permissions"}
/>
