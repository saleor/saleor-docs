---
api_reference: true
id: collection-create
title: collectionCreate
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

Creates a new collection.

<Permissions
  permissions={"MANAGE_PRODUCTS"}
  text={"Requires one of the following permissions"}
/>

```graphql
collectionCreate(
  input: CollectionCreateInput!
): CollectionCreate
```

<Details dataOpen="Hide return type" dataClose="Show return type">

```graphql
type CollectionCreate {
  collectionErrors: [CollectionError!]! @deprecated
  errors: [CollectionError!]!
  collection: Collection
}
```

</Details>

### Arguments

#### [<code>input</code>](#input)<Bullet />[<code>CollectionCreateInput!</code>](/api-reference/products/inputs/collection-create-input.mdx) \{#input\}

Fields required to create a collection.

### Type

#### [<code>CollectionCreate</code>](/api-reference/products/objects/collection-create.mdx)

Creates a new collection.

<Permissions
  permissions={"MANAGE_PRODUCTS"}
  text={"Requires one of the following permissions"}
/>
