...
#### [<code style={{ fontWeight: 'normal' }}>menuItemCreate.<b>input</b></code>](#)<Bullet />[`MenuItemCreateInput!`](../../api-reference/inputs/menu-item-create-input) <Badge class="secondary" text="non-null"/> <Badge class="secondary" text="input"/>

> Fields required to update a menu item. Only one of `url`, `category`, `page`, `collection` is allowed per item.

### Type

#### [`MenuItemCreate`](../../api-reference/objects/menu-item-create) <Badge class="secondary" text="object"/>
