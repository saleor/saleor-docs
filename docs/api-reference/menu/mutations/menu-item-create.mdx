---
api_reference: true
id: menu-item-create
title: menuItemCreate
---

export const Bullet = () => (
  <>
    <span
      style={{
        fontWeight: "normal",
        fontSize: ".5em",
        color: "var(--ifm-color-secondary-darkest)",
      }}
    >
      &nbsp;●&nbsp;
    </span>
  </>
);

export const SpecifiedBy = (props) => (
  <>
    Specification
    <a
      className="link"
      style={{ fontSize: "1.5em", paddingLeft: "4px" }}
      target="_blank"
      href={props.url}
      title={"Specified by " + props.url}
    >
      ⎘
    </a>
  </>
);

export const Badge = (props) => (
  <>
    <span className={props.class}>{props.text}</span>
  </>
);

import FeaturePreview from "@site/components/FeaturePreview";
import Permissions from "@site/components/Permissions";

Creates a new menu item.

<Permissions
  permissions={"MANAGE_MENUS"}
  text={"Requires one of the following permissions"}
/>

Triggers the following webhook events:

- MENU_ITEM_CREATED (async): A menu item was created.

```graphql
menuItemCreate(
  input: MenuItemCreateInput!
): MenuItemCreate
```

<Details dataOpen="Hide return type" dataClose="Show return type">

```graphql
type MenuItemCreate {
  menuErrors: [MenuError!]! @deprecated
  errors: [MenuError!]!
  menuItem: MenuItem
}
```

</Details>

### Arguments

#### [<code>input</code>](#input)<Bullet />[<code>MenuItemCreateInput!</code>](/api-reference/miscellaneous/inputs/menu-item-create-input.mdx) \{#input\}

Fields required to update a menu item. Only one of `url`, `category`, `page`, `collection` is allowed per item.

### Type

#### [<code>MenuItemCreate</code>](/api-reference/menu/objects/menu-item-create.mdx)

Creates a new menu item.

<Permissions
  permissions={"MANAGE_MENUS"}
  text={"Requires one of the following permissions"}
/>

Triggers the following webhook events:

- MENU_ITEM_CREATED (async): A menu item was created.
