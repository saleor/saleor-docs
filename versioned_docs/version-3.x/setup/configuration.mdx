...
If deploying Saleor under a specific domain, like `api.example.com`, it's crucial to ensure the system is set up to generate URLs properly. Adjust the domain configuration by utilizing the GraphQL `shopDomainUpdate` mutation:

```graphql
mutation UpdateDomain {
  shopDomainUpdate(input: { domain: "api.example.com" }) {
    shop {
      domain {
        host
      }
...
:::caution
After applying this modification, remember to restart the Saleor instances. Since the shop data is cached in memory, this action is vital for the updated configuration to be applied effectively.
:::

Additionally, ensure the `ENABLE_SSL` environment variable is configured appropriately.

With these configurations in place, Saleor will effectively generate URLs tailored to its domain, enabling smooth bidirectional communication.
