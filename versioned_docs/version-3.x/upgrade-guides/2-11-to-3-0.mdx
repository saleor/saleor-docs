...
Besides the impact on fetching the data, you'll be required to specify the channels during the checkout creation process. Read more at the [Checkout](developer/checkout/overview.mdx) page.
...
- [User emails](developer/app-store/legacy-plugins/user-emails.mdx)
- [Admin emails plugin](developer/app-store/legacy-plugins/admin-emails.mdx)
...
Read documentation for the [ProductMedia](developer/products/configuration.mdx#media).
...
Error codes can be used for identifying the issue and displaying custom message for the customers.
...
- `Attribute.values` dropped in favor of paginated list called [`choices`](developer/attributes/api.mdx#fetching-all-attributes).
  AttributeValue type:
...
If you want to create access tokens, you can do that by creating an [App](developer/extending/apps/overview.mdx).

### Webhooks

Webhook payloads have changed. You can reference examples at our [documentation](developer/extending/webhooks/sample-webhook-payloads.mdx).
...
The full list of available events can be found [here](developer/extending/webhooks/asynchronous-events.mdx#available-webhook-events).
...
The signature of JWT tokens has been replaced from HS256 to RS256. This change will require providing an [RSA private key](setup/configuration.mdx#rsa_private_key) for the production environments.
