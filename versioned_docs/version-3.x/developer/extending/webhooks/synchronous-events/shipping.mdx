...
- `id` **(required)** is id of external shipping method e.g `dhl-exp`. It will be converted by Saleor to `app:<app-database-id>:<id-returned-by-webhook>` and decoded in Base64 format.
...
External shipping methods have an ID of the following format: `app:<app-database-id>:<id-returned-by-webhook>` decoded in Base64.
...
:::warning
Due to circular reference, subscription queries should not contain `checkout.shippingMethods` or `order.shippingMethods` fields.
:::
