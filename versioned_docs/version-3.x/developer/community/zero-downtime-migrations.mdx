...
Similarly to adding the index to the database, removing the index also needs `concurrently` option.
...
Please note that line `atomic=False` is needed to proceed with concurrent index removal.
