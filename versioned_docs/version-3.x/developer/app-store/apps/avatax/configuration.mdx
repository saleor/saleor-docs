...
:::caution DEPRECATED
Alternatively you can add `avataxCustomerCode` to `User` metadata. If the user is logged in, the AvaTax app will use the user's id as the customer code. If the user is not logged in a dummy customer code will be used. **Sending customer code as entry inside user metadata won't work for [CheckoutCalculateTaxes](../../../../api-reference/taxes/objects/calculate-taxes.mdx) webhook.**
:::

### Discounts

When the AvaTax app is handling checkout or order calculate taxes webhooks it is using [automatically distributed discounts](https://developer.avalara.com/erp-integration-guide/sales-tax-badge/transactions/discounts-and-overrides/discounting-a-transaction/). It means that Saleor sends information about total discount applied to order or checkout and Avalara is distributing discounts.

For order confirmed webhook, the AvaTax app is using [price reduction discounts flow](https://developer.avalara.com/erp-integration-guide/sales-tax-badge/transactions/discounts-and-overrides/discounting-a-transaction/). It means that Saleor is responsible for calculating discounts and AvaTax app is sending discounted prices to Avalara for tax calculation.

For more information about how discounts work in Saleor see discounts [guide](../../../../developer/discounts/overview) from our docs.
...
![](./avatax-dashboard-list.png)

:::note
The following screenshots were taken in Feburary 2024, it might not reflect the current state of AvaTax dashboard.
:::
...
![Line item details in AvaTax dashboard that includes flowchart of why taxes were calculated](./avatax-details-1.png)
...
![Review calculation page with information about tax details for the particular line item in order](./avatax-details-2.png)
