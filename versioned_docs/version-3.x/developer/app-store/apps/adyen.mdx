...
Saleor synchronous webhooks have a maximum response time limit of 20 seconds. To allow graceful error handling, app restricts Adyen response time to 15 seconds.
If Adyen surpass this limit, the App will return FAILURE status accompanied by an appropriate error message (Timeout Error).
