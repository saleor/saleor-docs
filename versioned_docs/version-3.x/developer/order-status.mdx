...
Besides the order status, several settings that can affect the behavior of the order. You can find them in the [`channel.orderSettings`](api-reference/miscellaneous/objects/order-settings.mdx) object.

## Order status

Below is a diagram that shows the possible states of an order and the transitions between them. Names of the states are drawn from the [`OrderStatus`](api-reference/orders/enums/order-status) enum. Each state is described in detail in the following sections.
