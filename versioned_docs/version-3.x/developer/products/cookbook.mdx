...
- [Stock bulk update](api-reference/products/mutations/stock-bulk-update) - useful when your external system doesn't provide events for individual stock updates and you must pull data in batches. It can also be used for massive data updates since bulk mutations are more efficient.

Tracking should be disabled in the [variant settings](developer/products/configuration#variant-settings) to prevent Saleor from decrementing stock levels automatically and creating race conditions.
