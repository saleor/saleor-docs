...
- Product level availability:
  ...
  - `Hide in product listings` can be used to hide the product from the category and search. Products will be visible in collections.
...
The media gallery is saved on a product level and shared across all variants, allowing you to upload a single image and assign it to multiple variants.

- **Thumbnails:** Saleor automatically generates different formats and sizes of images, see [thumbnail settings](developer/thumbnails.mdx).
- **Image attributes:** To add specific media slots to products or variants, see [attributes](developer/attributes/overview.mdx). For example, it can be useful if you want to have a dedicated image for the product description section.
...
Assign tax class that is used to calculate taxes for the product. Learn more about [Taxes](developer/taxes/).
...
**Stock** - Stock is set per warehouse and has limited public visibility. Learn more about [stock tracking](/docs/developer/stock-allocation.mdx).

**Track inventory** - Disable automated stock tracking, [see more](/docs/developer/stock-allocation.mdx). When disabled the stock needs to be updated via API.

**SKU** - Stock Keeping Unit, unique identifier for the variant that is used for inventory management. This field can follow any format.

**Pricing** - The price can be set at the channel level, which can have a dedicated currency. Prices can be set as Selling Prices intended for customers and Cost Prices, which are optional and only used for internal purposes.

**Default variant** - Setting default variant is optional, it is useful in some scenarios when storefronts need to display a specific variant by default.

**Sorting variants** - Variants can be sorted for cases when the order of variants is important in storefront.

**Variant attributes** - `Dropdown attribute` can be configured to be a variant selection, which means that the attribute will be mapped to a variant. Follow the [example steps](#example-steps) to see how to configure a variant selection attribute.
