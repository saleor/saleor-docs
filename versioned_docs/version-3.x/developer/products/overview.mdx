...
| **Variants**            | Variations of a product, such as color, size, material, or any user-defined dimension, are mapped to a Stock Keeping Unit or a unique slug. | Optional. A product can have one or many variants; each variant contains.                                                  |
...
Variants give the level of control of
...
**Material:** can be _Cotton_ or _Polyester_ and represents a dropdown attribute, that is assigned to a product type.

**Size:** of the shirt is a product option that can be selected by the customer; it comes with predefined sizes, so it will be of type _Dropdown_ and _Variant selection attribute_ [see more](#variant-selection-attributes).

**Length:** is an attribute specific to a variant. It is not something that customers can select, and it is unique to each product, so it will be of type _Numeric_.
