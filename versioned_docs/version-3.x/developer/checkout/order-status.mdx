...
- this state is the result of [`checkoutComplete`](../api-reference/checkout/mutations/checkout-complete.mdx) mutation, if [`channel.orderSettings.automaticallyConfirmAllNewOrders`](../api-reference/miscellaneous/objects/order-settings#ordersettingsautomaticallyconfirmallnewordersboolean--) is set to `False`.

#### Available transitions

- Confirm: the status will change to `UNFULFILLED`.
- Expire: the status will automatically change to `EXPIRED` if the order is older than [`channel.orderSettings.expireOrdersAfter`](../api-reference/miscellaneous/objects/order-settings#ordersettingsexpireordersafterminute-) minutes. This will happen automatically only if no transaction attempts were made.
