...
Shipping methods can be internal (defined in Saleor Dashboard) or [external](developer/extending/webhooks/synchronous-events/shipping.mdx) (coming from 3-rd party). Collection points are [warehouses](api-reference/products/objects/warehouse.mdx) that allow picking-up orders e.g for click & collect flow.
