...
Adding a channel creates a wide scope of customization options, bellow the degree of customization per entity:
...
- Serving different geographical markets

- Serving different types of customers, for example, consumers, businesses, wholesale, etc.

- Businesses that have different legal entities can restrict access to customer and order data per entity

- Serving physical and online channels

- Multi-brand businesses that wish to share customer data, loyalty programs, and products.

- A/B testing different pricing strategies for product and fulfilment.

- Creating dedicated channels for marketplaces or other external sales channels.

### Not ideal for

- Currency conversion. Each channel can have different currency, however `Checkout` belongs to a single channel so it would not be straightforward to convert to another channel. Reason for that is that each channel can have different stock, prices, taxes, etc. See

- Customer-specific pricing. Potentially, you could serve customers differently using channels, but keep in mind that each channel also adds configuration complexity, see [relationships and scope](#relationships-and-scope) for more details.;

- Supplier-specific pricing. If you wish to single product at a different price depending on the supplier, using channels would not work because each checkout is scoped to a single channel; you would have to implement price list functionality instead.

### Channels lifecycle

Out of the box your store will have a default channel, that can be deleted at least one channel is always required.
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/cd142bef-3876-4b58-acad-c0783ea64906/cfb61018-937b-468d-a4aa-60a670982836/Untitled.png)
